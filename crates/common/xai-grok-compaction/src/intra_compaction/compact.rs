    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    //     Shared with Chutes Build chat inter-compaction via `append_reminder_block` so
...
/// metric label. Keep these in sync with the doc string on Chutes Build chat's
...
/// the literal error messages produced by the Chutes Build chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Chutes Build chat sampler when the
    ...
    /// — guards the forwarding that Chutes Build chat (`Arc<GrokTurn>`) relies on.
    #[test]
