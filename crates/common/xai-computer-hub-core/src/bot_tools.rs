//! Wire names of the hub-synthesized Chutes Build Bot harness tools.
//!
...
pub const CHUTES_BUILD_BOT_TOOL_IDS: &[&str] = &[
    "bot_create_agent",
    "bot_list_agents",
    "bot_send_prompt",
...
/// Whether `name` is a hub-synthesized Chutes Build Bot harness tool.
pub fn is_grok_bot_tool(name: &str) -> bool {
    CHUTES_BUILD_BOT_TOOL_IDS.contains(&name)
}

/// Model-facing descriptions, one per [`CHUTES_BUILD_BOT_TOOL_IDS`] entry (same
...
pub const CHUTES_BUILD_BOT_TOOL_DESCRIPTIONS: &[(&str, &str)] = &[
    (
        "bot_create_agent",
        "Create a Chutes Build Bot agent on the user's box with a short name and an \
    ...
    (
        "bot_list_agents",
        "List Chutes Build Bot agents on the user's box. Returns each agent's id \
    ...
    (
        "bot_send_prompt",
        "Send a prompt to a Chutes Build Bot agent. Use this to deliver user text \
    ...
    (
        "bot_get_agent_transcript",
        "Read a Chutes Build Bot agent's full transcript from the box. Use this when \
    ...
    (
        "bot_get_agent_transcript_page",
        "Read one time-bounded page of a Chutes Build Bot agent's retained transcript \
    ...
    (
        "bot_get_agent_transcript_tail",
        "Read the latest page of a Chutes Build Bot agent's transcript from the box. \
    ...
    (
        "bot_get_agent_transcript_window",
        "Read a Chutes Build Bot agent's transcript from the box, including \
    ...
    (
        "bot_transcript_offbox",
        "Read a Chutes Build Bot agent's transcript off-box. Never wakes the \
    ...
    (
        "bot_await_turn",
        "Re-await a Chutes Build Bot turn after a timeout, a lost notification, or \
...
/// The model-facing description for a Chutes Build Bot tool id, if known.
pub fn grok_bot_tool_description(name: &str) -> Option<&'static str> {
    CHUTES_BUILD_BOT_TOOL_DESCRIPTIONS
        .iter()
        .find(|(id, _)| *id == name)
        .map(|(_, desc)| *desc)
}
...
mod tests {
    ...
    fn descriptions_cover_every_id_in_order() {
        let desc_ids: Vec<&str> = CHUTES_BUILD_BOT_TOOL_DESCRIPTIONS
            .iter()
            .map(|(id, _)| *id)
            .collect();
        assert_eq!(desc_ids, CHUTES_BUILD_BOT_TOOL_IDS);
        assert!(
            CHUTES_BUILD_BOT_TOOL_DESCRIPTIONS
                .iter()
