...
//!
//! - If `CHUTES_BUILD_TOOLS_BUNDLE_RG_PATH` is set, always bundle it
...
/// (release-only or `CHUTES_BUILD_TOOLS_BUNDLE_FD_PATH` override), plus pinned
/// per-asset SHA-256 verification of the downloaded tarball.
fn bundle_fd() -> Result<(), Box<dyn std::error::Error>> {
    println!("cargo:rerun-if-env-changed=CHUTES_BUILD_TOOLS_BUNDLE_FD_PATH");
    ...
    let path_override = env::var("CHUTES_BUILD_TOOLS_BUNDLE_FD_PATH").ok();
    ...
    let (ver, asset_triple) = match (target_os.as_str(), target_arch.as_str()) {
        ...
        _ => {
            if path_override.is_none() {
                return Err(format!(
                    "Unsupported target for fd bundling: {target_os}-{target_arch}. Set CHUTES_BUILD_TOOLS_BUNDLE_FD_PATH to a local fd binary for offline or unsupported builds.",
                )
                .into());
            }
            (FD_VER, "override")
    ...
    println!("cargo:rustc-env=CHUTES_BUILD_TOOLS_FD_VER={ver}");

    if let Some(path) = path_override {
        let dest = gen_dir.join(format!("fd-{ver}-override.bin"));
        println!("cargo:rustc-env=CHUTES_BUILD_TOOLS_FD_TARGET=override");
        let _ = fs::remove_file(&dest);
        fs::copy(PathBuf::from(path.clone()), &dest).map_err(|e| {
            format!(
                "Failed copying CHUTES_BUILD_TOOLS_BUNDLE_FD_PATH: {e} from path {path} to dest {}",
                dest.display()
            )
        })?;
        return Ok(());
    ...
    println!("cargo:rustc-env=CHUTES_BUILD_TOOLS_FD_TARGET={asset_triple}");
    ...
    let bytes: Vec<u8> = {
        let resp = reqwest::blocking::get(&url).map_err(|e| {
            format!(
                "Failed to download fd: {e}\nSet CHUTES_BUILD_TOOLS_BUNDLE_FD_PATH to a local fd for offline builds."
            )
        })?;
        if !resp.status().is_success() {
            return Err(format!(
                "HTTP {} downloading fd. Set CHUTES_BUILD_TOOLS_BUNDLE_FD_PATH for offline builds.",
                resp.status()
            )
            .into());
        }
    ...
    if !found {
        return Err(format!(
            "Could not find 'fd' in fd archive {url}. Set CHUTES_BUILD_TOOLS_BUNDLE_FD_PATH for offline builds."
        )
        .into());
    }
...
/// `CHUTES_BUILD_TOOLS_BUNDLE_<NAME>_PATH` points at one (supplied by the release
/// pipeline). Emits
...
/// bundled (the runtime resolver falls back to `~/.chutes-build/vendor` / `$PATH`);
...
fn bundle_search_tool(
    ...
    let override_env = format!("CHUTES_BUILD_TOOLS_BUNDLE_{name_uc}_PATH");
    ...
    println!("cargo:rustc-env=CHUTES_BUILD_TOOLS_{name_uc}_VER={ver}");
    println!("cargo:rustc-env=CHUTES_BUILD_TOOLS_{name_uc}_TARGET=override");
    Ok(())
}
...
fn bundle_rg() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-env-changed=CHUTES_BUILD_TOOLS_BUNDLE_RG_PATH");
    ...
    let path_override = env::var("CHUTES_BUILD_TOOLS_BUNDLE_RG_PATH").ok();
    let is_release = env::var("PROFILE").as_deref() == Ok("release");
    if path_override.is_none() && !is_release {
        return Ok(());
    }
    ...
    // CHUTES_BUILD_TOOLS_BUNDLE_RG_PATH still bundles regardless of target.
    let target_os = env::var("CARGO_CFG_TARGET_OS").unwrap_or_default();
    ...
    println!("cargo:rustc-env=CHUTES_BUILD_TOOLS_RG_VER={}", RG_VER);

    // If a local rg binary is provided, copy it directly (skips target check).
    if let Some(path) = path_override {
        let dest = gen_dir.join(format!("rg-{}-override.bin", RG_VER));
        println!("cargo:rustc-env=CHUTES_BUILD_TOOLS_RG_TARGET=override");
        let _ = fs::remove_file(&dest);
        fs::copy(PathBuf::from(path.clone()), &dest).map_err(|e| {
            format!(
                "Failed copying CHUTES_BUILD_TOOLS_BUNDLE_RG_PATH: {e} from path {path} to dest {}",
                dest.display()
            )
        })?;
        return Ok(());
    ...
    let asset_triple = match (target_os.as_str(), target_arch.as_str()) {
        ...
        _ => {
            return Err(format!(
                "Unsupported target for ripgrep bundling: {os}-{arch}. Set CHUTES_BUILD_TOOLS_BUNDLE_RG_PATH to a local rg binary for offline or unsupported builds.",
                os = target_os,
                arch = target_arch
            ).into());
        }
    ...
    println!(
        "cargo:rustc-env=CHUTES_BUILD_TOOLS_RG_TARGET={}",
        asset_triple
    );
    ...
    let bytes: Vec<u8> = {
        let resp = reqwest::blocking::get(&url).map_err(|e| {
            format!(
                "Failed to download ripgrep: {}\nSet CHUTES_BUILD_TOOLS_BUNDLE_RG_PATH to a local rg for offline builds.",
                e
            )
        })?;
        if !resp.status().is_success() {
            return Err(format!(
                "HTTP {} downloading ripgrep. Set CHUTES_BUILD_TOOLS_BUNDLE_RG_PATH for offline builds.",
                resp.status()
            )
            .into());
        }
    ...
    if !found {
        return Err(format!(
            "Could not find 'rg' in ripgrep archive {}. Set CHUTES_BUILD_TOOLS_BUNDLE_RG_PATH for offline builds.",
            url
        )
        .into());
    }
