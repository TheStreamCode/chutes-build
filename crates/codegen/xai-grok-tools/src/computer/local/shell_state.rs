...
const BASH_STATE_START_MARKER: &str = "__CHUTES_BUILD_BASH_STATE_START__";
const BASH_STATE_END_MARKER: &str = "__CHUTES_BUILD_BASH_STATE_END__";
const ZSH_STATE_START_MARKER: &str = "__CHUTES_BUILD_ZSH_STATE_START__";
const ZSH_STATE_END_MARKER: &str = "__CHUTES_BUILD_ZSH_STATE_END__";

/// Marker emitted by the init path to separate login-shell noise (MOTD, etc.)
/// from the actual state dump on stdout.
const INIT_STATE_MARKER: &str = "__CHUTES_BUILD_INIT_STATE_MARKER__";
...
pub fn shell_env_overrides() -> HashMap<String, String> {
    HashMap::from([
        ...
        // env cannot clear it.
        (
            crate::util::CHUTES_BUILD_AGENT_ENV.to_string(),
            crate::util::CHUTES_BUILD_AGENT_ENV_VALUE.to_string(),
        ),
    ])
}
...
dump_bash_state() {
  ...
  _emit_encoded() {
    ...
    local var_name="$2"
    if [[ -n "$content" ]]; then
      builtin printf 'grok_snap_%s=$(command base64 -d <<'"'"'CHUTES_BUILD_SNAP_EOF_%s'"'"'\n' "$var_name" "$var_name"
      command base64 <<<"$content" | command tr -d '\n'
      builtin printf '\nCHUTES_BUILD_SNAP_EOF_%s\n' "$var_name"
      builtin printf ')\n'
      builtin printf 'eval "$grok_snap_%s"\n' "$var_name"
    fi
  }

  _emit "__CHUTES_BUILD_BASH_STATE_START__"

  _emit "$PWD"

  local env_vars
  env_vars=$(builtin export -p 2>/dev/null | command grep -viE '_proxy=|CHUTES_BUILD_SANDBOX|CHUTES_BUILD_AGENT=|SUDO_ASKPASS|CHUTES_BUILD_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
  ...
  _emit "__CHUTES_BUILD_BASH_STATE_END__"
}
"##;
...
function dump_zsh_state() {
  ...
  _emit "__CHUTES_BUILD_ZSH_STATE_START__"

  _emit "$PWD"

  local env_vars
  env_vars=$(builtin typeset -xp 2>/dev/null | command grep -viE '_proxy=|CHUTES_BUILD_SANDBOX|CHUTES_BUILD_AGENT=|SUDO_ASKPASS|CHUTES_BUILD_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
  _emit_encoded "$env_vars" "ENV_VARS_B64"
  ...
  _emit "__CHUTES_BUILD_ZSH_STATE_END__"
}
"##;
    ...
    pub fn prepare_command(
        ...
        let wrapper = match self.shell {
            ShellKind::Bash => format!(
                ...
                // Re-export CHUTES_BUILD_AGENT=1 after snapshot eval so agent-definition
                ...
                "{dump_script} \
                 ...
                 builtin export CHUTES_BUILD_AGENT=1; \
...
mod tests {
    ...
    fn parse_dump_valid_bash() {
        let raw = "__CHUTES_BUILD_BASH_STATE_START__\n\
                    /home/user/project\n\
                    export FOO=bar\n\
                    # end of bash state dump\n\
                    __CHUTES_BUILD_BASH_STATE_END__\n";
    ...
    fn parse_dump_valid_zsh() {
        let raw = "__CHUTES_BUILD_ZSH_STATE_START__\n\
                    /tmp\n\
                    typeset -x FOO=bar\n\
                    # end of zsh state dump\n\
                    __CHUTES_BUILD_ZSH_STATE_END__\n";
    ...
    fn parse_after_marker_found() {
        let output =
            "Welcome to Ubuntu\nMOTD line\n__CHUTES_BUILD_INIT_STATE_MARKER__\nactual data\n";
        let result = parse_after_marker(output, "__CHUTES_BUILD_INIT_STATE_MARKER__");
    ...
    fn shell_state_update_from_dump() {
        ...
        let dump = "__CHUTES_BUILD_BASH_STATE_START__\n\
                     /new/dir\n\
                     export X=1\n\
                     # end of bash state dump\n\
                     __CHUTES_BUILD_BASH_STATE_END__\n";
    ...
    async fn test_prepare_command_and_roundtrip() {
        ...
        // Run "export CHUTES_BUILD_TEST_VAR=hello" and capture the new state
        let prep = state
            .prepare_command(
                "export CHUTES_BUILD_TEST_VAR=hello",
                None,
