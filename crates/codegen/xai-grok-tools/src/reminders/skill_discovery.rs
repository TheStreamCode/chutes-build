...
//! after every tool call to check for SKILL.md files in `.chutes-build/skills/`,
...
/// Directories that contain skill definitions (`.chutes-build/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".chutes-build", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.chutes-build/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.chutes-build/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.chutes-build/) are outside the git root so the walk breaks early.
