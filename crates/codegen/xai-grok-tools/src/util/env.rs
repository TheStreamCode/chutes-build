...
/// Note: the CLI also uses `CHUTES_BUILD_AGENT` as an
/// optional agent-definition selector for launching `chutes-build` itself; child terminal
/// processes only need the sentinel value `"1"`.
pub const CHUTES_BUILD_AGENT_ENV: &str = "CHUTES_BUILD_AGENT";

/// Sentinel value for [`CHUTES_BUILD_AGENT_ENV`] on agent tool terminals.
pub const CHUTES_BUILD_AGENT_ENV_VALUE: &str = "1";

/// Force `CHUTES_BUILD_AGENT=1` on an agent terminal child so request/login env cannot
/// clear the agent marker.
pub fn apply_grok_agent_marker(cmd: &mut tokio::process::Command) {
    cmd.env(CHUTES_BUILD_AGENT_ENV, CHUTES_BUILD_AGENT_ENV_VALUE);
}

/// Expand the four plugin-path tokens (`${CLAUDE_PLUGIN_ROOT}` / `${CHUTES_BUILD_PLUGIN_ROOT}`
/// and `${CLAUDE_PLUGIN_DATA}` / `${CHUTES_BUILD_PLUGIN_DATA}`) in `s`. Each pair is expanded
...
pub fn substitute_plugin_tokens(
    ...
    if let Some(root) = plugin_root {
        out = out
            ...
            .replace("${CHUTES_BUILD_PLUGIN_ROOT}", root);
    }
    if let Some(data) = plugin_data {
        out = out
            .replace("${CLAUDE_PLUGIN_DATA}", data)
            .replace("${CHUTES_BUILD_PLUGIN_DATA}", data);
    }
    out
}
...
#[cfg(test)]
mod tests {
    use super::{CHUTES_BUILD_AGENT_ENV, CHUTES_BUILD_AGENT_ENV_VALUE, substitute_plugin_tokens};

    const ALL_TOKENS: &str = "${CLAUDE_PLUGIN_ROOT}/a ${CHUTES_BUILD_PLUGIN_ROOT}/b ${CLAUDE_PLUGIN_DATA}/c ${CHUTES_BUILD_PLUGIN_DATA}/d";
    ...
    fn expands_only_root_when_data_none() {
        ...
        assert_eq!(
            out,
            "/root/a /root/b ${CLAUDE_PLUGIN_DATA}/c ${CHUTES_BUILD_PLUGIN_DATA}/d"
        );
    }

    #[test]
    fn agent_marker_constants_match_cursor_parity() {
        assert_eq!(CHUTES_BUILD_AGENT_ENV, "CHUTES_BUILD_AGENT");
        assert_eq!(CHUTES_BUILD_AGENT_ENV_VALUE, "1");
    }
}
