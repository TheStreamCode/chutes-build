...
/// should use `background: true`. Env override: `CHUTES_BUILD_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("CHUTES_BUILD_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`CHUTES_BUILD_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `CHUTES_BUILD_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
