...
impl SubagentLimits {
    ...
    fn from_lookup(lookup: impl Fn(&str) -> Option<String>) -> Self {
        ...
        let max_concurrent = parse_positive_env(
            "CHUTES_BUILD_MAX_CONCURRENT_SUBAGENTS",
            lookup("CHUTES_BUILD_MAX_CONCURRENT_SUBAGENTS"),
        )
        .unwrap_or(default.max_concurrent);
        let behavior = match lookup("CHUTES_BUILD_SUBAGENT_LIMIT_BEHAVIOR") {
            ...
            Some(value) => {
                tracing::warn!(
                    %value,
                    "CHUTES_BUILD_SUBAGENT_LIMIT_BEHAVIOR is neither `queue` nor `fail`; keeping `queue`"
                );
                LimitBehavior::Queue
            }
        };
...
impl AdmissionError {
    ...
    /// into `CHUTES_BUILD_SUBAGENT_LIMIT_BEHAVIOR=fail`.
