...
/// Short name of a tool id (`ChutesBuild:workflow` → `workflow`).
...
pub enum WorkflowSource {
    Name {
        #[schemars(
            description = "Name of a registered workflow (built-in, or discovered from the project `.chutes-build/workflows/` or user `~/.chutes-build/workflows/`)."
        )]
        name: String,
    },
    Script {
...
A started run gets a session-unique display name (e.g. `review-changes`, `review-changes-2`) — the handle to show the user and use with `/workflow pause|resume|stop <name>`; keep run IDs internal. Each launch persists an editable `script_path`; edit it and launch as a new run to iterate. Use the `resume` source only for a same-process paused run (process restarts are terminal); it reuses the run's original immutable source and args, and a budget-limited run resumes only with a higher `agent_budget`. Save reusable scripts to `.chutes-build/workflows/<name>.rhai`."##
    }
...
mod tests {
    ...
    fn workflow_id_matches_kind_or_short_name() {
        ...
        assert!(is_workflow_tool_id("ChutesBuild:workflow"));
        assert!(!is_workflow_tool_id("web_search"));
        assert!(is_workflow_tool(Some(ToolKind::Workflow), "anything"));
        assert!(is_workflow_tool(None, "ChutesBuild:workflow"));
