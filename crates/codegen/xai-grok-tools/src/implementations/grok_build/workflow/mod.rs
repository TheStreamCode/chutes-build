...
pub struct WorkflowToolInput {
    ...
    #[serde(default)]
    #[schemars(
        description = "Name of a registered workflow (built-in, or discovered from the project `.chutes-build/workflows/` or user `~/.chutes-build/workflows/`). Exactly one of `name`, `script`, or `script_path` must be set."
    )]
...
A started run gets a session-unique display name (e.g. `review-changes`, `review-changes-2`) — the handle to show the user and use with `/workflow pause|resume|stop <name>`; keep run IDs internal. Each launch persists an editable `script_path`; edit it and launch as a new run to iterate. Use `resume_from_run_id` only for a same-process paused run (process restarts are terminal); a budget-limited run resumes only with a higher `agent_budget`. Save reusable scripts to `.chutes-build/workflows/<name>.rhai`."##
    }
