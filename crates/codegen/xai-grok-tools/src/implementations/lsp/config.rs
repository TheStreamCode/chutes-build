//! LSP server configuration from `.chutes-build/lsp.json`.
...
/// This is the canonical merge function — both session startup and `chutes-build inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let project_path = cwd.join(".chutes-build").join("lsp.json");
...
/// Load LSP server configs from `~/.chutes-build/lsp.json` and `<cwd>/.chutes-build/lsp.json`.
