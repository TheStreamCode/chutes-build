...
pub enum SkillScope {
    /// cwd/.chutes-build/skills (highest priority)
    Local = 0,
...
mod tests {
    ...
    fn extracts_skill_name() {
        assert_eq!(
            skill_name_from_path("/home/user/.chutes-build/skills/deploy/SKILL.md"),
            Some("deploy"),
        );
    }
    ...
    fn nested_path() {
        assert_eq!(
            skill_name_from_path("/repo/.chutes-build/skills/my-skill/SKILL.md"),
            Some("my-skill"),
        );
    }
