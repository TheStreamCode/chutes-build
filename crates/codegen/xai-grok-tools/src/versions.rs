...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"ChutesBuild:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"ChutesBuild:run_terminal_cmd"`).
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("ChutesBuild:run_terminal_cmd", "legacy-0.4.10"),
            ("ChutesBuild:read_file", "legacy-0.4.10"),
            ("ChutesBuild:search_replace", "legacy-0.4.10"),
            ("ChutesBuild:get_task_output", "legacy-0.4.10"),
            ("ChutesBuild:kill_task", "legacy-0.4.10"),
            ("ChutesBuild:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("ChutesBuild:run_terminal_cmd", "current"),
            ("ChutesBuild:read_file", "current"),
            ("ChutesBuild:search_replace", "current"),
            ("ChutesBuild:list_dir", "current"),
            ("ChutesBuild:grep", "current"),
            ("ChutesBuild:kill_task", "current"),
            ("ChutesBuild:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "ChutesBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "ChutesBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "ChutesBuild:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn unmanaged_tool_returns_none() {
        let v = resolve_version("current", "ChutesBuild:web_search", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn override_on_unmanaged_tool_errors() {
        let err = resolve_version("current", "ChutesBuild:web_search", Some("legacy-0.4.10"))
            .unwrap_err();
    ...
    fn unknown_preset_errors() {
        let err = resolve_version("nonexistent", "ChutesBuild:run_terminal_cmd", None).unwrap_err();
    ...
    fn unknown_override_version_errors() {
        let err =
            resolve_version("current", "ChutesBuild:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("ChutesBuild:run_terminal_cmd"));
        assert!(is_version_managed("ChutesBuild:read_file"));
        assert!(is_version_managed("ChutesBuild:search_replace"));
        assert!(is_version_managed("ChutesBuild:list_dir"));
        assert!(is_version_managed("ChutesBuild:grep"));
        assert!(is_version_managed("ChutesBuild:kill_task"));
        // Not managed:
        assert!(!is_version_managed("ChutesBuild:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res =
            resolve_version_with_warnings("current", "ChutesBuild:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "ChutesBuild:run_terminal_cmd", None)
                .unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let (version, warnings) =
            validate_and_resolve("ChutesBuild:run_terminal_cmd", "current").unwrap();
        ...
        let err = validate_and_resolve("ChutesBuild:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unmanaged_returns_none_no_warnings() {
        let res = resolve_version_with_warnings("current", "ChutesBuild:web_search", None).unwrap();
    ...
    fn resolve_with_warnings_unsupported_version_errors_with_list() {
        let err =
            resolve_version_with_warnings("current", "ChutesBuild:grep", Some("legacy-0.4.10"))
                .unwrap_err();
        assert!(
            err.contains("is not supported for tool"),
