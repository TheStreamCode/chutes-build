...
/// Resolve max API retries: `CHUTES_BUILD_MAX_RETRIES` env > model config > default ([`DEFAULT_MAX_RETRIES`]).
pub fn resolve_max_retries(model_max_retries: Option<u32>) -> u32 {
    let env_override = std::env::var("CHUTES_BUILD_MAX_RETRIES").ok();
...
mod tests {
    ...
    fn classify_cloudflare_522_is_retryable() {
        let err = api_err(
            ...
            "Connection to Chutes Build timed out or was interrupted. (HTTP 522).",
        );
    ...
    fn classify_cloudflare_525_is_fatal_even_with_should_retry_true() {
        ...
        for should_retry in [None, Some(true)] {
            let err = SamplingError::Api {
                ...
                message: "Secure connection to Chutes Build failed. (HTTP 525).".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry,
                error_code: None,
