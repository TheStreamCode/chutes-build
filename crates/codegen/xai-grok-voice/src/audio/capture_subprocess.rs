...
//! run) — and can be forced with `CHUTES_BUILD_VOICE_CAPTURE=inprocess`.
...
/// Env escape hatch: `CHUTES_BUILD_VOICE_CAPTURE=inprocess` forces the legacy
/// in-process cpal backend (accepting its permanent footprint cost).
const CAPTURE_BACKEND_ENV: &str = "CHUTES_BUILD_VOICE_CAPTURE";
