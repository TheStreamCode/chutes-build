//! Voice input for Chutes Build CLI: an xAI streaming STT client and the
...
mod intercept_tests {
    ...
    fn capture_subcommand_matches_only_argv1() {
        ...
        assert!(!is_capture_subcommand(&argv(&["chutes-build"])));
        assert!(!is_capture_subcommand(&argv(&["chutes-build", "chat"])));
        assert!(!is_capture_subcommand(&argv(&[
            "chutes-build",
            "chat",
            "__mic-capture"
