//! Single source of truth for the Chutes Build home directory: `$CHUTES_BUILD_HOME` or
//! `<home>/.chutes-build`. Shared by `xai-grok-config` and `xai-fast-worktree`.
//!
...
//! - [`default_grok_home`]: the `<home>/.chutes-build` default, ignoring `$CHUTES_BUILD_HOME`, so callers can detect an override.
...
/// `<home>/.chutes-build`, canonicalized via `dunce` (not `std::fs::canonicalize`,
...
/// `$CHUTES_BUILD_HOME` verbatim when non-empty, else `<home>/.chutes-build`. The env value is
...
/// The default `<home>/.chutes-build`, used when `$CHUTES_BUILD_HOME` is unset.
