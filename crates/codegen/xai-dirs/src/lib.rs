...
//! grok-home (`$CHUTES_BUILD_HOME` or `<home>/.grok`). Shared by `xai-grok-config`
...
//! - [`default_grok_home`]: the `<home>/.grok` default, ignoring `$CHUTES_BUILD_HOME`, so callers can detect an override.
...
/// Where a resolved Chutes Build home came from, so "why did grok pick this
...
pub enum GrokHomeSource {
    /// A non-empty `$CHUTES_BUILD_HOME` override.
    EnvOverride,
...
/// resolves `~/.chutes-build` from the profile variable — mixing the two sources puts
...
/// `$CHUTES_BUILD_HOME` verbatim when non-empty, else `<home>/.grok`. The env value is
...
/// Resolve the Chutes Build home from the environment (fresh, no cache); `None` if neither resolves.
...
pub fn resolve_grok_home_with_source() -> Option<(PathBuf, GrokHomeSource)> {
    resolve_grok_home_from(
        std::env::var_os("CHUTES_BUILD_HOME").as_deref(),
        home_dir().as_deref(),
    )
}

/// The default `<home>/.grok`, used when `$CHUTES_BUILD_HOME` is unset.
pub fn default_grok_home() -> PathBuf {
    grok_home_in(&home_dir().unwrap_or_else(|| PathBuf::from(".")))
}

/// The Chutes Build home, created if missing and cached for the process; falls back to
/// [`default_grok_home`] when neither `$CHUTES_BUILD_HOME` nor a home resolves.
pub fn grok_home() -> PathBuf {
    static CHUTES_BUILD_HOME: OnceLock<PathBuf> = OnceLock::new();
    CHUTES_BUILD_HOME
        .get_or_init(|| {
            let home = resolve_grok_home().unwrap_or_else(default_grok_home);
            if let Err(err) = std::fs::create_dir_all(&home) {
                tracing::warn!(path = %home.display(), %err, "failed to create Chutes Build home");
            }
            home
        })
        .clone()
...
mod tests {
    ...
    fn default_grok_home_has_no_verbatim_prefix() {
        ...
        assert!(home.ends_with(".chutes-build"));
    }
