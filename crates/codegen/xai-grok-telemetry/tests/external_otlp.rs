//! Integration test verifying that the external OTEL stream is **deadened** in
//! Chutes Build: even when every env var is set to activate it, no exporter
//! runs, `is_active()` stays `false`, and no data reaches any collector.
...
#[test]
fn external_stream_is_deadened() {
    let collected = col::Collected::default();
    let endpoint = col::start_collector(collected.clone());

    // Resolve through the real config path (double opt-in, gates on) —
    // everything that upstream would need to start exporting.
    ...
    // init() is a compile-time no-op: the stream can never activate.
    ...
    // Give any (impossible) exporter time to fire, then verify silence.
    ...
    assert!(
        !raw.contains("sess-deadened"),
        "session id reached the wire"
    );

    // shutdown() must also be a safe no-op.
