...
//! over ACP (`chutes.ai/log` notifications); shell receives them in
...
pub const LOG_METHOD: &str = "chutes.build/log";
...
/// Wire format for the `chutes.ai/log` ACP notification params.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// `~/.chutes-build/logs/unified.jsonl` (those bursts inflate exactly the counters
...
/// healing a stale handle rather than re-resolving `$CHUTES_BUILD_HOME` — which also
...
///
/// Called by the `chutes.ai/log` notification handler. Entries from
...
mod tests {
    ...
    /// writer onto the developer's real `~/.chutes-build/logs/unified.jsonl`.
    #[ctor::ctor]
