...
//! file logger that writes to `~/.chutes-build/logs/hooks.log`.
//!
//! ## When to use
//!
...
//!
//! ```bash
//! CHUTES_BUILD_HOOKS_LOG=1 grok              # enable, write to ~/.chutes-build/logs/hooks.log
//! CHUTES_BUILD_HOOKS_LOG=/tmp/h.log grok     # write to custom path
//! CHUTES_BUILD_HOOKS_LOG=0 grok              # explicitly disable
//! tail -f ~/.chutes-build/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "CHUTES_BUILD_HOOKS_LOG";
...
///
/// Writes to `~/.chutes-build/logs/hooks.log` (or custom path via `CHUTES_BUILD_HOOKS_LOG`).
/// Filters to hooks (`xai_grok_hooks`) and plugins (`xai_grok_agent::plugins`) targets.
/// Set `CHUTES_BUILD_HOOKS_LOG=0` to disable, `CHUTES_BUILD_HOOKS_LOG=/path` to redirect.
