...
//! reverse `chutes.build/mcp/sdk_call` request and feeding the response back. This module
...
/// channel (`chutes.build/mcp/sdk_call`) and returns its JSON-RPC response. The `Err` string is
...
/// `chutes.build/mcp/sdk_call` and writes the server→client response back. The pump exits when
...
async fn read_requests(
    ...
    loop {
        ...
        // `chutes.build/mcp/sdk_call`s without a JSON-RPC id, so id-less messages (e.g. rmcp's
