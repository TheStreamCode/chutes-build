//! E2E path-deny and Chutes Build hook write-deny (subprocess; arm64-tagged).
...
const CHUTES_BUILD_HOME_ENV: &str = "SANDBOX_E2E_CHUTES_BUILD_HOME";
...
fn apply_fixture_env(cmd: &mut Command, home: &Path, grok_home: &Path, workspace: &Path) {
    cmd.env(WORKSPACE_ENV, workspace.as_os_str())
        ...
        .env(CHUTES_BUILD_HOME_ENV, grok_home.as_os_str())
        .env("HOME", home.as_os_str())
        .env("CHUTES_BUILD_HOME", grok_home.as_os_str());
}
...
fn subprocess_entry() {
    ...
    // Isolate HOME/CHUTES_BUILD_HOME before any config OnceLock init.
    let home = PathBuf::from(std::env::var(HOME_ENV).expect(HOME_ENV));
    let grok_home =
        PathBuf::from(std::env::var(CHUTES_BUILD_HOME_ENV).expect(CHUTES_BUILD_HOME_ENV));
...
fn subprocess_hook_write_deny_marker_spoof(_grok_home: &Path) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        unsafe {
            std::env::set_var("__CHUTES_BUILD_INSIDE_BWRAP", "1");
        }
...
/// Workspace-profile Chutes Build-owned hook write-deny probes (existing sources + first-run).
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    let home = PathBuf::from(std::env::var(CHUTES_BUILD_HOME_ENV).expect(CHUTES_BUILD_HOME_ENV));
    ...
    if first_run {
        ...
        eprintln!("OK: first-run Chutes Build hook slots denied");
    } else {
    ...
    // Parent grants remain creatable: Chutes Build runtime sibling, workspace, temp.
    assert_write_ok(
        "chutes-build runtime sibling",
...
/// Create isolated HOME + CHUTES_BUILD_HOME fixture dirs for a scenario.
fn fixture_homes(
    tag: &str,
) -> (
    PathBuf,
    ...
    // Empty global sandbox config under fixture CHUTES_BUILD_HOME so generic tests do
    // not inherit the developer/runner's ~/.chutes-build/sandbox.toml.
...
fn run_deny_case(
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".chutes-build")).expect("mkdir .chutes-build");
    fs::write(
        tmp.join(".chutes-build").join("sandbox.toml"),
        format!("[profiles.{profile}]\nextends = \"workspace\"\ndeny = [{deny_list}]\n"),
    )
    .expect("write sandbox.toml");

    // Ensure Chutes Build fixed slots exist so workspace-based custom profiles can
...
/// Workspace profile: Chutes Build-owned direct hook sources are write-denied but readable;
/// create / overwrite / unlink / rename / mkdir fail; absolute hooks-paths
/// targets are denied; parent rename is blocked; Chutes Build/CWD/temp siblings stay writable.
#[test]
fn workspace_protects_direct_hook_sources() {
    ...
    for needle in [
        ...
        "OK: chutes-build runtime sibling writable",
...
/// First-run: missing fixed slots are created as real Chutes Build state before apply,
...
fn workspace_protects_direct_hook_sources_first_run() {
    ...
    for needle in [
        "OK: first-run Chutes Build hook slots denied",
    ...
    // Post-exit host: Chutes Build slots exist and are valid; no vendor artifacts.
    assert!(
