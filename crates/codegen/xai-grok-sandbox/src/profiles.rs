...
//! `off`. Custom profiles via `~/.chutes-build/sandbox.toml` or `.chutes-build/sandbox.toml`.
...
/// Load sandbox config from `~/.chutes-build/sandbox.toml` and `.chutes-build/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.chutes-build/sandbox.toml
    let global_path = grok_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.chutes-build/sandbox.toml (additive only)
    let project_path = workspace.join(".chutes-build").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    ...
    let project =
        load_config_file(&workspace.join(".chutes-build").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // directories like ~/.chutes-build/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.chutes-build/sandbox.toml or .chutes-build/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
...
mod tests {
    ...
    /// Hosts with a retargetable `$CHUTES_BUILD_HOME/hooks` symlink (fail-closed under
