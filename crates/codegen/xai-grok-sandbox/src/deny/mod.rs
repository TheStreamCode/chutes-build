...
pub(crate) fn apply_write_deny_paths_to_capability_set(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        tracing::info!(
            count = entries.len(),
            "Applied Seatbelt write-deny for Chutes Build-owned direct hook sources"
        );
    }
    #[cfg(target_os = "linux")]
    {
...
mod tests {
    ...
    fn ancestors_pin_under_writable_root_not_home() {
        ...
        let grok = tmp.join("chutes-build");
        ...
        assert!(
            ...
            "must pin sessions under CHUTES_BUILD_HOME: {pin:?}"
        );
        assert!(
            pin.iter().any(|p| p == &grok),
            "must pin CHUTES_BUILD_HOME grant root: {pin:?}"
        );
        assert!(
