...
// ── Chutes Build state directory ────────────────────────────────────────────────────

/// Chutes Build state directory — always writable (`$CHUTES_BUILD_HOME` or `~/.chutes-build`).
...
/// Writable directory paths for the read-only profile (minimal: just ~/.chutes-build + temp).
/// Device files are handled separately via `allow_file` in `to_capability_set_with_config`.
