...
///
/// Cache is stored in: `~/.chutes-build/indexes/{url_encoded_cwd}/goto_index.bin`
...
mod tests {
    ...
    // which maps to `chutes.build/code/status` reporting `reason: notStarted`.
    ...
    /// In `chutes.build/code/status`, `resolve_index_handle()` calls
