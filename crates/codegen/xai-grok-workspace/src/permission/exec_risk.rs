...
mod tests {
    ...
    fn attached_and_chained_c_paths() {
        ...
        // POSIX-absolute `-C` path with no Windows drive letter; only
        // meaningful under Unix path semantics (`Path::is_absolute()` is
        // false for it on Windows, so it resolves relative to `base` there).
        #[cfg(unix)]
