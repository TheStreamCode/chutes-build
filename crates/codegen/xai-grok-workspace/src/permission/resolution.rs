...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    // `.chutes-build/config.toml` (same gate as project `.claude/settings.json`).
    if project_trusted {
...
///
/// `project_trusted` gates project-tier Claude settings and `.chutes-build/config.toml`
...
mod tests {
    ...
    /// Untrusted clone must not contribute project `.chutes-build/config.toml` [permission].
    ///
    ...
    fn untrusted_project_config_toml_permissions_are_not_honored() {
        ...
        // Global allow (survives untrusted project when CHUTES_BUILD_HOME resolves here).
        std::fs::write(
...
"#,
        ...
        if global_live {
            let untrusted = untrusted.expect("global rules present when CHUTES_BUILD_HOME is live");
            assert!(
                untrusted
                    .config
                    .rules
