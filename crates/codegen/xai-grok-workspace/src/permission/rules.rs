...
/// map onto `ToolFilter::Mcp` patterns over Chutes Build's qualified `<server>__<tool>`
...
pub fn parse_permission_rule(
    ...
    if let Some(open_paren) = find_first_unescaped(rule, b'(') {
        ...
        // spelling). Chutes Build qualifies MCP tools as `<server>__<tool>` with no
            ...
            && !rest.is_empty()
        {
            let pattern = if rest == "*" {
                ...
                // Already `<server>__<tool>` (or `<server>__*`): the Chutes Build
...
mod tests {
    ...
    /// `ToolFilter::Mcp` globs over Chutes Build's qualified `<server>__<tool>` names.
    #[test]
