...
const ENVRC_TIMEOUT_ENV: &str = "CHUTES_BUILD_ENVRC_TIMEOUT_SECS"; // seconds; 0 disables
...
const OUTPUT_SENTINEL: &str = "__CHUTES_BUILD_ENVRC_COMPLETE__";
...
mod tests {
    ...
    fn sentinel_named_variable_does_not_truncate_env() {
        ...
        fs::write(
            dir.path().join(".envrc"),
            "export __CHUTES_BUILD_ENVRC_COMPLETE__=decoy\nexport ZZ_AFTER_DECOY=survives\n",
        )
        .unwrap();

        let env = load_envrc_with_timeout(dir.path(), Duration::from_secs(10)).unwrap();
        assert_eq!(
            env.get("__CHUTES_BUILD_ENVRC_COMPLETE__"),
