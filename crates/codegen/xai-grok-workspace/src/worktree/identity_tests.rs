...
fn cwd_at_slug_level_or_at_worktrees_dir_itself_has_no_identity() {
    let worktrees = Path::new("/home/user/.chutes-build/worktrees");
    assert_eq!(
        worktree_identity_in(worktrees, "/home/user/.chutes-build/worktrees"),
        None
    );
    assert_eq!(
        worktree_identity_in(worktrees, "/home/user/.chutes-build/worktrees/xai"),
        None
    );
}
...
fn standalone_clone_behind_symlinked_worktrees_dir_reports_no_source() {
    ...
    let link_home = root.join("link-home");
    std::os::unix::fs::symlink(&real_home, &link_home).unwrap();
    let _env = LockedTestEnv::lock().set("CHUTES_BUILD_HOME", &link_home);
