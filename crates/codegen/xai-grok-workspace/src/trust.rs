...
//!
//! Persists per-folder trust decisions to `~/.chutes-build/trusted_folders.toml`.
...
//! a `./.chutes-build/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.chutes-build/`.
...
/// sharing `~/.chutes-build/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.chutes-build/trusted-hook-projects` file listed one canonical project
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.chutes-build");
        assert_eq!(
    ...
    fn workspace_key_collapses_linked_worktrees_onto_main_checkout() {
        // Every linked `chutes-build -w` worktree of a repo must share ONE trust key:
