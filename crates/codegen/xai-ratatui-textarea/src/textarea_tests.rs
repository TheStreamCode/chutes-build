...
fn screen_spans_of_range_rejects_non_char_boundaries() {
    // '├®' spans bytes 1..3; an endpoint inside it must yield no spans
        ...
        for _step in 0..60 {
            ...
            match rng.random_range(0..18) {
                ...
                15 => {
                    ...
                    {
                        let end = start + payload.len();
                        // Create an intersecting range [start-╬┤, end-╬┤2)
...
fn double_click_cursor_on_last_char_unicode() {
    ...
    // Double-click on "caf├®" (col 1 = 'a')
    ta.handle_mouse(mouse_down(1, 0), area, state);
    ta.handle_mouse(mouse_down(1, 0), area, state);

    assert_eq!(ta.selected_text(), Some("caf├®".to_string()));
    // '├®' is 2 bytes (0xC3 0xA9), so "caf├®" = [c(0), a(1), f(2), ├®(3,4)]
    // Last char '├®' starts at byte 3
    assert_eq!(
        ta.cursor(),
        3,
        "cursor should be on '├®' (byte 3), got {}",
        ta.cursor()
    );
}
...
fn click_after_scroll_places_cursor_at_clicked_line() {
    ...
    // Scroll down 3 ticks (3 lines ├ù 3 = 9 lines).
    for _ in 0..3 {
        ta.handle_mouse(mouse_scroll_down(0, 0), area, state);
        ratatui::widgets::StatefulWidgetRef::render_ref(&(&ta), area, &mut buf, &mut state);
    }
