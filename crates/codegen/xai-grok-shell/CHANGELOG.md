...
- **CHUTES_BUILD_CONFIG** and **CHUTES_BUILD_CONFIG_PATH** environment variables now let launchers override selected config settings without editing config.toml.
- **Worktrees** under ~/.chutes-build/worktrees are now automatically reclaimed when safe, with strong safeguards that never delete a user's last copy.
...
- **CHUTES_BUILD_FORCE_LOGIN_TEAM_ID** environment variable now lets launchers restrict interactive login to one or more teams.
...
- **chutes-build inspect** no longer crashes when its output is piped into a command that closes the pipe early.
...
- **Session search index** can now be disabled via CHUTES_BUILD_SESSION_SEARCH or [features] session_search for hosts sharing $CHUTES_BUILD_HOME.
...
- Tool commands and MCP servers now receive a CHUTES_BUILD_SESSION_ID environment variable matching the current session.
...
- **Keystrokes typed while Chutes Build is starting** are now preserved in the composer instead of being lost.
...
- **`chutes-build du`** and worktree commands now work on Windows when only USERPROFILE is set.
...
- **Subagent spawning** is dramatically faster when you have many sessions in ~/.chutes-build.
...
- New `chutes-build du` command shows disk usage of ~/.chutes-build including worktrees and sessions.
...
- **chutes-build trace** exports now bundle memory trace files for easier debugging.
...
- **Failed alpha/enterprise updates** now suggest the matching CHUTES_BUILD_CHANNEL reinstall command.
...
- **Non-interactive sessions** (`chutes-build -p`) no longer fail when the agent asks for user input or plan approval.
