...
use super::*;
/// Partition the AGENTS.md / Claude.md / `.chutes-build/rules/*.md` files returned
/// by `read_agents_config_with_paths` into "workspace" (cwd / repo root /
/// extra workspace user dir) and "user" (`~/.chutes-build/`, `~/.claude/`,
/// `~/.chutes-build/bundled/`) buckets, mirroring the split between
...
fn partition_rules_by_scope(
    ...
    let user_prefixes: Vec<String> = match home {
        Some(h) => vec![format!("{h}/.chutes-build/"), format!("{h}/.claude/")],
        None => vec![],
    };
