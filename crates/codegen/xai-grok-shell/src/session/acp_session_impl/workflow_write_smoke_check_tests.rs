...
fn last_smoke_check_keeps_final_edit_per_path() {
    let indices = last_smoke_check_indices([
        (0, Some(".chutes-build/workflows/a.rhai".to_owned())),
        (1, Some(".chutes-build/workflows/b.rhai".to_owned())),
        (2, Some(".chutes-build/workflows/a.rhai".to_owned())),
        (3, None),
    ]);
...
fn path_helper_matches_only_grok_workflow_rhai_files() {
    for path in [
        ".chutes-build/workflows/check.rhai",
        "project/.chutes-build/workflows/check.rhai",
        r"C:\project\.chutes-build\workflows\check.rhai",
    ] {
    ...
    for path in [
        ".chutes-build/check.rhai",
        ".chutes-build/workflows/check.txt",
        ".chutes-build/not-workflows/check.rhai",
...
async fn valid_written_workflow_needs_no_warning() {
    ...
    let path = directory.path().join(".chutes-build/workflows/valid.rhai");
...
async fn invalid_authored_workflow_returns_path_specific_warning() {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join(".chutes-build/workflows/broken.rhai");
    std::fs::create_dir_all(path.parent().unwrap()).unwrap();
    ...
    let snapshot = snapshot_authored_workflow(
        Some(ToolKind::Write),
        &serde_json::json!({ "file_path": ".chutes-build/workflows/broken.rhai" }),
        &path_keys(&["file_path"]),
        directory.path(),
        None,
        directory.path(),
    ...
    assert!(
        snapshot_authored_workflow(
            Some(ToolKind::Write),
            &serde_json::json!({ "path": ".chutes-build/workflows/broken.rhai" }),
            &path_keys(&["path"]),
            directory.path(),
            None,
            directory.path(),
    ...
    assert!(
        snapshot_authored_workflow(
            Some(ToolKind::Edit),
            &serde_json::json!({ "target_file": ".chutes-build/workflows/broken.rhai" }),
    ...
    assert!(
        snapshot_authored_workflow(
            Some(ToolKind::Write),
            &serde_json::json!({ "src": ".chutes-build/workflows/broken.rhai" }),
            &path_keys(&["src"]),
            directory.path(),
            None,
            directory.path(),
...
async fn nonterminating_workflow_times_out_promptly() {
    let snapshot = AuthoredWorkflowSnapshot {
        path: PathBuf::from(".chutes-build/workflows/loop.rhai"),
...
async fn irrelevant_tool_or_path_skips_smoke_check() {
    let directory = tempfile::tempdir().unwrap();
    let missing = serde_json::json!({ "file_path": ".chutes-build/workflows/missing.rhai" });
