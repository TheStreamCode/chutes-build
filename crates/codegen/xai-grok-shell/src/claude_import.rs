// claude_import.rs
// Scans Claude settings and generates TOML patches for .chutes-build/config.toml.
//
...
pub enum ImportScope {
    /// User-level: writes to `~/.chutes-build/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.chutes-build/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.chutes-build/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.chutes-build/config.toml`.
...
impl ImportPlan {
    ...
    pub fn summary(&self, cwd: &Path) -> String {
        ...
        if !self.global_items.is_empty() {
            out.push_str("\nGlobal (~/.chutes-build/config.toml):\n");
...
//
// The marker `[claude_compat] imported = true` in `~/.chutes-build/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.chutes-build/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.chutes-build/config.toml`.
///
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.chutes-build/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.chutes-build/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        ...
        // Hooks are written separately to ~/.chutes-build/hooks/imported-from-claude.json.
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".chutes-build").join("config.toml");
        ...
        let hooks_dir = project_root.join(".chutes-build").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .chutes-build/hooks/ JSON files in apply_hooks_to_dir,
...
/// (Claude-compatible). The native hooks loader scans `.chutes-build/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .chutes-build/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    impl Drop for MarkerGuard {
        fn drop(&mut self) {
            ...
            unsafe { std::env::remove_var("_CHUTES_BUILD_CLAUDE_MARKER_OVERRIDE") };
        }
    }
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.chutes-build) don't interfere.
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `CHUTES_BUILD_HOME` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        unsafe { std::env::set_var("_CHUTES_BUILD_CLAUDE_MARKER_OVERRIDE", "1") };
        let dir = tempfile::tempdir().unwrap();
        ...
        // managed settings, and the developer's real `~/.chutes-build/config.toml`.
        // We can't isolate `grok_home()` because it's `OnceLock`-cached.
        // Instead, assert on rule *provenance*: no rule should originate from
        // our tempdir's `.claude/settings.json`. The dev's real ~/.chutes-build
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.chutes-build/config.toml` may legitimately have the marker
