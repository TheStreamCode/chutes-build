...
fn memory_config_cli_flag_overrides_env_disable() {
    with_grok_memory(
        ...
        || {
            ...
            let mem = MemoryConfig::resolve(true, false, &config, None);
            assert!(
                mem.enabled, "CLI --experimental-memory should override CHUTES_BUILD_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_grok_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "--no-memory should override --experimental-memory, CHUTES_BUILD_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_defaults_are_correct() {
    without_grok_memory(|| {
        ...
        assert_eq!(
            ...
            Some(xai_grok_config_types::DEFAULT_EMBEDDING_MODEL)
        );
        assert_eq!(
            mem.embedding.dimensions,
            xai_grok_config_types::DEFAULT_EMBEDDING_DIMENSIONS
        );
        assert_eq!(
            mem.embedding.base_url.as_deref(),
            Some(xai_grok_config_types::DEFAULT_EMBEDDING_BASE_URL)
        );
...
fn subagents_config_env_var_disables_default() {
    with_grok_subagents(
        ...
        || {
            ...
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(
                ! sa.enabled, "CHUTES_BUILD_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_grok_subagents(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let sa = SubagentsConfig::resolve(true, &config, None);
            assert!(sa.enabled, "--subagents CLI flag should override CHUTES_BUILD_SUBAGENTS=0");
        },
    );
}
#[test]
...
fn roles_parse_from_toml() {
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".chutes-build/prompts/impl.md"));
}
#[test]
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".chutes-build/personas/concise.md"));
}
#[test]
...
cli_chat_proxy_base_url = "https://cli-chat-proxy.chutes-build.com/v1"
...
/// verdicts untouched). CHUTES_BUILD_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores CHUTES_BUILD_HOME even on panic). No user-global
/// `$CHUTES_BUILD_HOME/config.toml` is seeded: `grok_home()` is `OnceLock`-cached,
...
/// CHUTES_BUILD_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
