...
pub(crate) const ENV_DEFAULT_PERMISSION_MODE: &str = "CHUTES_BUILD_DEFAULT_PERMISSION_MODE";
...
mod tests {
    ...
    fn effective_auto_for_launch_cli_auto_not_yolo() {
        ...
        unsafe { std::env::set_var("CHUTES_BUILD_AUTO_PERMISSION_MODE", "1") };
        ...
        unsafe { std::env::remove_var("CHUTES_BUILD_AUTO_PERMISSION_MODE") };
    }
    ...
    /// With the gate forced OFF (`CHUTES_BUILD_AUTO_PERMISSION_MODE=0`), explicit
    ...
    fn effective_auto_for_launch_inert_when_gate_off() {
        let _g = crate::util::config::resolve::AUTO_PERMISSION_MODE_ENV_LOCK
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        unsafe { std::env::set_var("CHUTES_BUILD_AUTO_PERMISSION_MODE", "0") };
        assert!(
    ...
    fn interactive_default_slot() {
        ...
        for (raw, expected) in [
            ...
            assert_eq!(
                default_interactive_permission_mode(),
                expected,
                "CHUTES_BUILD_DEFAULT_PERMISSION_MODE={raw}"
            );
        }
