...
//! attempt (or an explicit `chutes.ai/auth/cancel`) cancels the previous one, so
...
//! scopes explicit cancels so a delayed `chutes.ai/auth/cancel` cannot tear down a
//! successor login.
...
pub(crate) struct AttemptChannels {
    /// Forwards pasted codes from `chutes.ai/auth/submit_code` to the flow.
    code_tx: tokio::sync::mpsc::Sender<String>,
    /// Yields the auth URL to `chutes.ai/auth/get_url`. `Option` so
