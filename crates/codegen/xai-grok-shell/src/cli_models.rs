...
/// Fetch model state via `chutes.build/models/list` over an initialized channel.
...
/// Parse a `chutes.build/models/list` payload; a handler error wins over a
/// missing result.
...
mod tests {
    ...
    fn resolve_oauth_session() {
        ...
        assert_eq!(
            ...
            AuthStatus::LoggedIn("chutes.ai".to_owned())
        );
    }
