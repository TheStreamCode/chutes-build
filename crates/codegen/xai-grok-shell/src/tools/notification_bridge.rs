...
mod tests {
    ...
    /// drain it). The pager's `chutes.ai/task_completed` notification still fires.
    #[tokio::test]
    ...
    async fn task_completed_notification_stamps_will_wake() {
        ...
        *config
        ...
        assert!(
            persisted,
            "declined admission must still persist chutes.ai/task_completed"
        );
    }
    ...
    async fn task_completed_notification_is_frame_bounded() {
        ...
        let params = params.expect("expected an chutes.ai/task_completed notification");
        assert!(
