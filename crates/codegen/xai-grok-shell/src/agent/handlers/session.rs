...
/// Router for chutes.build/session/* and chutes.build/session_summaries/* methods.
pub async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
...
/// `chutes.build/sessions/list` — the FleetView roster. Returns every
...
/// `chutes.build/sessions/changed` broadcast.
...
async fn handle_session_close(
    ...
    if existed {
        ...
        tracing::info!(session_id = %req.session_id, "session closed via chutes.build/session/close");
    } else {
