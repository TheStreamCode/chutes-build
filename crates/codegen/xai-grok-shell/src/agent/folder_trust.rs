...
mod tests {
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; CHUTES_BUILD_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because CHUTES_BUILD_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. CHUTES_BUILD_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because CHUTES_BUILD_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CHUTES_BUILD_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. CHUTES_BUILD_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because CHUTES_BUILD_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CHUTES_BUILD_HOME", home.path());
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). CHUTES_BUILD_HOME-isolated so the trust store is empty.
        use xai_grok_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CHUTES_BUILD_HOME", home.path());
        let _flag = EnvGuard::unset("CHUTES_BUILD_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. CHUTES_BUILD_HOME-isolated (empty store).
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + CHUTES_BUILD_HOME-isolated so the
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. CHUTES_BUILD_HOME-isolated so the store starts
        // empty; `#[serial]` because CHUTES_BUILD_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("CHUTES_BUILD_HOME", home.path());
        let tmp = repo_tmp();
