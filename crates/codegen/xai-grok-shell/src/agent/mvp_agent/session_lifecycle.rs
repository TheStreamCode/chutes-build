...
impl MvpAgent {
    ...
    /// session end — a terminal/explicit close (`chutes.build/session/close`). It must
    ...
    /// (`chutes.build/session/close`). Finalizes the cloud replica (genuine session
    ...
    /// `chutes.build/sessions/changed` notification with the session in `removed` so
    ...
    /// `chutes.build/sessions/changed` notification with the current entry in
    ...
    /// Emit an `chutes.build/sessions/changed` upsert for a resident session with an
    ...
    /// Fan an `chutes.build/sessions/changed` delta out to every attached client.
    ///
    ...
    /// the `chutes.build/sessions/changed` special-case in `leader/server.rs`).
