...
fn replay_line(record: &Value) -> String {
    serde_json::json!({ "method": "chutes.build/session/update", "params": record }).to_string()
}
...
async fn replay_leaves_other_oversized_records_alone() {
    ...
    let line = format!(r#"{{"method":"chutes.build/session/update","params":{recorded}}}"#);
    agent.forward_raw_replay_line(
        &line,
        /*persist_data*/ None,
        /*target_client_id*/ None,
...
async fn a_stale_task_completion_is_frame_bounded() {
    ...
    let line = format!(
        r#"{{"timestamp":1,"method":"chutes.build/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_backgrounded","task_id":"stale-1","command":"{}","cwd":"/tmp"}}}}}}"#,
        "c".repeat(64 * 1024)
    );
