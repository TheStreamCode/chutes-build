...
fn subagent_spawn_context_resolves_compaction_mode_like_parent() {
    ...
    let _mode = EnvGuard::unset("CHUTES_BUILD_COMPACTION_MODE");
    let _detail = EnvGuard::unset("CHUTES_BUILD_COMPACTION_DETAIL");
    ...
    let _env_mode = EnvGuard::set("CHUTES_BUILD_COMPACTION_MODE", "transcript");
    assert_eq!(
        ctx.resolve_compaction_mode(),
        CompactionMode::Transcript,
        "CHUTES_BUILD_COMPACTION_MODE must win over parent config and remote"
    );
}
#[test]
