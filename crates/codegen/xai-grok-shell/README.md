...
- **Reference**
  - [Introspection (`chutes-build inspect`)](#introspection)
...
- [Building with Chutes Build](#building-with-grok) — headless API, ACP SDK integration
...
[Releases](https://github.com/TheStreamCode/chutes-build/releases).
...
To update, re-run the install: Chutes Build does not update itself, and
`chutes-build update` reports where to look rather than fetching anything.
...
On first launch, Chutes Build opens your browser to authenticate with chutes.ai:

```bash
chutes-build
```

Credentials are stored in `~/.chutes-build/auth.json` and persist across sessions. Tokens expire after 7 days; Chutes Build will prompt you to re-authenticate when needed.
...
For CI/CD, automation, or environments without browser access, use an API key from [chutes.ai/app/api](https://chutes.ai/app/api):
...
Authenticate developers via your own Identity Provider (Okta, Azure AD, Auth0) instead of `accounts.chutes.ai`.
...
**3. Run `chutes-build`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.chutes-build/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Chutes Build is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.
...
4. **stdout** → captured by Chutes Build and saved to `~/.chutes-build/auth.json` as the access token
5. exit 0 → success; exit non-zero → Chutes Build falls through to interactive login
...
**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Chutes Build reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want Chutes Build to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Chutes Build when to proactively refresh. Without it, Chutes Build assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Chutes Build captures this)
...
export CHUTES_BUILD_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export CHUTES_BUILD_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Chutes Build cannot detect expiry proactively and will only refresh after a 401.
...
> **Windows:** write the path as a TOML *literal* string (single quotes) so backslashes survive: `auth_provider_command = 'C:\corp\grok-auth.exe'`. Inside a double-quoted TOML string `\t`, `\n`, `\r`, `\b` and `\f` are escape sequences, so `"C:\temp\auth.exe"` parses into a path containing a tab character and the provider fails to start — after which Chutes Build falls back to browser login as if the setting were ignored.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with chutes.ai". In headless mode (`chutes-build -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
Chutes Build runs your binary on two different contracts, and `CHUTES_BUILD_AUTH_EXPIRED` is how it tells them apart:

| | `CHUTES_BUILD_AUTH_EXPIRED=1` | unset |
|---|---|---|
| **What it is** | A headless refresh over a credential Chutes Build already holds — near-expiry rotation, or a token the server rejected | A sign-in: `chutes-build login`, the sign-in screen, or the escalation after a headless run couldn't mint |
...
| **Budget** | A few seconds (7s), then Chutes Build kills the process | 300s — enough for a browser round trip or a device code |
...
if [ "$CHUTES_BUILD_AUTH_EXPIRED" = "1" ]; then
    ...
    # lapsed, say — exit non-zero rather than block. Chutes Build then shows the
...
Exiting promptly on `CHUTES_BUILD_AUTH_EXPIRED=1` is what makes the handover to the sign-in screen fast: a binary that blocks instead pays the whole refresh timeout on every start with an expired token.

One case stays ambiguous, and only in **leader mode** (`--leader`, or `[cli] use_leader = true`; off by default): with no credential at all, the leader makes one extra attempt in the background just after startup, and that run has the variable unset, like a sign-in. A binary that mints without help (service account, keytab, mounted token) succeeds there and the session heals itself. One that must prompt just sits, up to the 300s sign-in ceiling — nothing waits on it, the sign-in screen is already up, and its stderr goes to `~/.chutes-build/leader.log` rather than to the user.

`CHUTES_BUILD_AUTH_EXPIRED` is optional — if your binary ignores it, Chutes Build still works. It just runs the same flow for both login and refresh, and a flow that prompts will be killed on the headless run before it can finish.

### Automatic Credential Refresh

Chutes Build supports automatic credential refresh for external auth providers and OIDC. When Chutes Build detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Chutes Build handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Chutes Build re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Chutes Build re-runs the binary and retries the request once.
- **When the refresh run can't mint:** refreshes are headless (no stdin, short timeout), so a binary that needs you to complete an SSO flow cannot succeed there. Chutes Build then stops treating the stored credential as usable and runs your binary in its interactive mode instead — at startup that is the same sign-in flow a machine with no credentials gets; mid-session the turn fails with a re-auth prompt and `/login` re-runs the binary.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Chutes Build silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Chutes Build refreshes tokens 5 minutes before expiry by default.
...
**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `chutes-build login` before starting — Chutes Build runs your binary automatically on first launch. You _can_ run `chutes-build login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Chutes Build tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
chutes-build --debug-file /tmp/grok-auth.log -p "hello"
...
|-------------|---------------|
| `auth: running external auth provider (headless refresh)` | Your binary is being called with `CHUTES_BUILD_AUTH_EXPIRED=1` and a few seconds to work |
| `auth: running external auth provider (interactive login)` | Your binary is being called on the sign-in contract: no `CHUTES_BUILD_AUTH_EXPIRED`, stderr shown, 300s |
...
| `auth: external auth provider timed out (likely needs interactive auth), killing` | Binary didn't exit before the 7s headless-refresh timeout and was killed. Exiting non-zero on `CHUTES_BUILD_AUTH_EXPIRED=1` avoids this wait entirely |
| `auth: failed to start external auth provider` | The command couldn't be spawned (e.g. binary not found) |

### Per-Model Auth Providers

`auth_provider_command` above replaces Chutes Build's *session* auth: it mints the token sent to xAI's backend. If you instead want xAI models on normal xAI login while **other models** route through a gateway (LiteLLM, corporate proxy) whose bearer tokens rotate, use a named auth provider — the rotating-token analogue of a per-model `api_key`/`env_key`.
...
- `CHUTES_BUILD_AUTH_EXPIRED=1` is set whenever Chutes Build re-mints over a token still cached in memory, whether from near-expiry rotation or a rejection. The first mint on a cold cache runs without it.

**Token lifecycle:**

- Tokens are cached in memory per provider and shared by every model referencing the provider; nothing is written to disk. The command is a credential helper: it owns durable storage and OAuth2 refresh (keychain, its own dotdir, etc.), exactly like `gcloud auth print-access-token` or a git credential helper. On an in-session re-mint the last credential is handed back via `CHUTES_BUILD_AUTH_PROVIDER_ACCESS_TOKEN` (and, when present, `CHUTES_BUILD_AUTH_PROVIDER_REFRESH_TOKEN` / `CHUTES_BUILD_AUTH_PROVIDER_EXPIRES_AT`), so a refresh-grant command can refresh instead of re-authenticating. The command must be non-interactive and fast; do any interactive login out of band, and Chutes Build re-runs the command on restart to re-mint.
- Chutes Build runs the command before a chat turn when the token is missing or within about a minute of expiring, and once more after the server rejects a token. A token rejected within 30 seconds of being fetched is not refetched again, so a broken helper surfaces one clear error instead of looping.
...
**Security:** provider commands execute code, so they are honored only from trusted config layers (`~/.chutes-build/config.toml`, managed config, requirements). A project's `.chutes-build/config.toml` can never define one. Whatever layer sets a model's `base_url` decides where that model's minted token is sent, and `base_url` (unlike the provider table) is not stripped from remote or campaign patches, the same as for a static `env_key`. Keep provider tables and the model `base_url` in layers you trust. The command inherits Chutes Build's environment (so it sees `PATH`, `HOME`, and any other secrets there), but Chutes Build's own first-party credentials (`CHUTES_API_KEY`, `CHUTES_BUILD_DEPLOYMENT_KEY`, and related keys) are removed so a BYOK helper never receives them; write helpers that read only what they need, and prefer the `CHUTES_BUILD_AUTH_PROVIDER_*` handback for the prior credential.

### Using auth.json for API Access

If you've authenticated with `chutes-build login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the chutes-build CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.chutes.ai/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.chutes.ai/sign-in".key' ~/.chutes-build/auth.json)" \
  -H "X-XAI-Token-Auth: xai-grok-cli" \
  -H "x-grok-model-override: chutes-build" \
  -d '{
    "model": "chutes-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.chutes-build/auth.json` (set by `chutes-build login`)                                                                                                                              |
| `X-XAI-Token-Auth: xai-grok-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-grok-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `chutes-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `chutes-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `chutes-build login` to refresh.
...
chutes-build --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
chutes-build --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
chutes-build --always-approve --prompt "Format all files"
```
...
Run Chutes Build non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `chutes-build`)               |
...
chutes-build -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
chutes-build -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
chutes-build -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
chutes-build -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
chutes-build -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
chutes-build -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
chutes-build -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
chutes-build -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
chutes-build -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
chutes-build --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
chutes-build -p "What does this project do?"

# Use a specific model
chutes-build -p "Optimize this function" -m chutes-build

# Get JSON output for parsing
chutes-build -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
chutes-build -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
chutes-build -p "Remember: the secret number is 42" --output-format json
chutes-build -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
chutes-build -p "Continue where we left off" -c

# Run in a different directory
chutes-build -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
chutes-build -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
chutes-build -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
chutes-build -p "Generate a README" > README.md

# Parse JSON output with jq
chutes-build -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
chutes-build -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | chutes-build -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  chutes-build -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
chutes-build -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run Chutes Build as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `chutes-build`)                           |
...
```bash
chutes-build agent headless --chutes-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`chutes-build ssh`)

Use `chutes-build ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.

```bash
# Basic usage (same args as ssh)
chutes-build ssh user@host

# With SSH flags
chutes-build ssh -t user@host
chutes-build ssh -L 8080:localhost:8080 user@host

# With remote command
chutes-build ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `chutes-build ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.

This runs entirely locally.

---

## Building with Chutes Build

Chutes Build can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `chutes-build -p` and parses JSON output.
...
class GrokChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["chutes-build", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="chutes-build", stream=False):
...
class GrokChat {
  ...
  async create(
    ...
    { model = "chutes-build", stream = false } = {},
  ) {
    const prompt =
      messages.length === 1
        ? messages[0].content
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("chutes-build", this.buildArgs(prompt, model, true));
...
class GrokACPChat {
  ...
  async init() {
    this.proc = spawn("chutes-build", ["agent", "stdio"]);
...
Chutes Build implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Chutes Build reads configuration from `~/.chutes-build/config.toml`. If the file doesn't exist, Chutes Build uses sensible defaults. You only need to specify values you want to override.
...
telemetry = false                      # anonymous usage telemetry (env: CHUTES_BUILD_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: CHUTES_BUILD_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: CHUTES_BUILD_RESPECT_GITIGNORE)
...
allowed_domains = ["docs.rs", "chutes.ai"]           # override the built-in ~84-domain allowlist
```
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: CHUTES_BUILD_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: CHUTES_BUILD_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: CHUTES_BUILD_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: CHUTES_BUILD_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: CHUTES_BUILD_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `CHUTES_BUILD_TELEMETRY_BUILD_EVENTS_URL`, `CHUTES_BUILD_TELEMETRY_BUILD_EVENTS_API_KEY`, and `CHUTES_BUILD_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Chutes Build can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Chutes Build language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Chutes Build can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Chutes Build can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Chutes Build looks for server definitions in:

- project config: `<repo>/.chutes-build/lsp.json`
- user config: `~/.chutes-build/lsp.json`
...
- LSP tools are enabled (`CHUTES_BUILD_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
CHUTES_BUILD_LSP_TOOLS=1 chutes-build
```
...
If LSP tools are enabled but no usable server config is found, Chutes Build emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Chutes Build does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `CHUTES_BUILD_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
name = "Chutes Build Latest (Proxy)"
...
With this config, `chutes-build` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Chutes Build reads these files and appends their contents to the system prompt.

Chutes Build scans for agent rules in this order:

1. `~/.chutes-build/` (global rules)
...
Within each directory, Chutes Build checks for these filenames:
...
Skills are reusable prompt packages that extend Chutes Build with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

Chutes Build discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.chutes-build/skills/`           | Local | Highest  |
| `<repo_root>/.chutes-build/skills/` | Repo  | Medium   |
| `~/.chutes-build/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.chutes-build/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Chutes Build decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Chutes Build invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Chutes Build automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Chutes Build discovers agent definitions from `.chutes-build/agents/` (project), `~/.chutes-build/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `CHUTES_BUILD_AGENT` env var
4. Default `chutes-build` agent
...
```bash
chutes-build --agent-profile ./my-agent.md
# or
export CHUTES_BUILD_AGENT="my-custom-agent"
```
...
```bash
export CHUTES_BUILD_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "chutes-build"              # route explore to a lighter model
```
...
model = "chutes-build"
prompt_file = ".chutes-build/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".chutes-build/personas/concise.md"  # or load from file
```

Both are also discovered from `.chutes-build/roles/*.toml` and `.chutes-build/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Chutes Build with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.chutes-build/plugins/`            | Project |
| `~/.chutes-build/plugins/`          | User    |
...
Chutes Build discovers hooks from `.chutes-build/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → cached `auth_provider` token (terminal: a cache miss resolves to no credential, never the session token) → session token → `CHUTES_API_KEY`. See [Per-Model Auth Providers](#per-model-auth-providers).

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Chutes Build falls back to built-in defaults for known models.
...
**How it works:** When you override a built-in model, Chutes Build starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `CHUTES_BUILD_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `chutes-build models` output). You also need a `[model.*]` entry so Chutes Build knows
...
>
> [model.my-custom-model]              # 2. tell Chutes Build how to reach it
...
chutes-build -p "Hello" -m my-model
...
Point Chutes Build at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `CHUTES_BUILD_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `CHUTES_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `CHUTES_BUILD_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export CHUTES_BUILD_MODELS_BASE_URL="https://api.acme.com/v1"
export CHUTES_API_KEY="cpk_..."
chutes-build
```

Chutes Build fetches the model list from `{CHUTES_BUILD_MODELS_BASE_URL}/models` on startup and sends inference requests to `CHUTES_BUILD_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `CHUTES_BUILD_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.chutes-build/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Chutes Build uses API key auth (`Authorization: Bearer`) instead of session auth. `chutes-build login` is not required — only the API key.

---

## MCP Servers

Extend Chutes Build's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.chutes-build/config.toml`:
...
MCP servers can also be configured per-project in `.chutes-build/config.toml`. Chutes Build walks from the current directory up to the git repo root, loading `.chutes-build/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.chutes-build/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.chutes-build/config.toml` | This repository   | ↑        |
| `<cwd>/.chutes-build/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.chutes-build/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.chutes-build/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.chutes-build/config.toml`. Other config sections (models, etc.) are only read from `~/.chutes-build/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `CHUTES_BUILD_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets Chutes Build remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.chutes-build/memory/`:
- **Global** (`~/.chutes-build/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.chutes-build/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.chutes-build/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
```bash
# Per-session flag
chutes-build --experimental-memory

# Environment variable (persists for the shell session)
export CHUTES_BUILD_MEMORY=1
chutes-build
...
At the end of each session, Chutes Build saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.chutes-build/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Chutes Build searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
chutes-build memory edit

# Open global MEMORY.md
chutes-build memory edit --global

# Show memory statistics: file count, chunk count, and index size
chutes-build memory stats
```

### Configuration reference

Key options under `[memory]` in `~/.chutes-build/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.chutes-build/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Chutes Build emits the `grok-shell-memory_injection`
...
Chutes Build can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.chutes-build/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.chutes-build/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.chutes-build/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.chutes-build/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.chutes-build/sandbox.toml` (global) or `.chutes-build/sandbox.toml` (per-project):
...
```bash
chutes-build --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire chutes-build process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Chutes Build logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.chutes-build/sandbox-events.jsonl`
...
Use `chutes-build inspect` to see everything Chutes Build discovers in the current directory:

```bash
chutes-build inspect          # human-readable output
chutes-build inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.chutes-build/skills/`, `~/.chutes-build/skills/`, plugins, and config paths
...
Chutes Build automatically discovers configuration from Claude Code directories alongside native `.chutes-build/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Chutes Build uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.chutes-build/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Chutes Build at runtime.

---

## Built-in Tools

Chutes Build includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `CHUTES_BUILD_WEB_FETCH=1`. 

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (Chutes, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `chutes.ai/company`).

---

## Session Persistence

Chutes Build automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.chutes-build/sessions/`, organized by URL-encoded working directory:

```
~/.chutes-build/sessions/<encoded-cwd>/<session-id>/
...
chutes-build -p "Hello"

# Create or resume a named session
chutes-build -p "Remember: X=42" -s my-session
chutes-build -p "What is X?" -s my-session

# Resume existing session (errors if not found)
chutes-build -p "Continue" -r my-session

# Continue most recent session in current directory
chutes-build -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
chutes-build -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.chutes-build/config.toml` | Configuration file                                  |
| `~/.chutes-build/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.chutes-build/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.chutes-build/memory/`     | Cross-session memory files and index                |
| `~/.chutes-build/skills/`     | User-scoped skill definitions                       |
| `~/.chutes-build/plugins/`    | User-scoped plugins                                 |
| `~/.chutes-build/agents/`     | User-scoped agent definitions                       |
| `.chutes-build/config.toml`   | Project-scoped config (MCP servers)                 |
| `.chutes-build/skills/`       | Project-scoped skill definitions                    |
| `.chutes-build/plugins/`      | Project-scoped plugins                              |
| `.chutes-build/agents/`       | Project-scoped agent definitions                    |
| `.chutes-build/hooks/`        | Project-scoped hooks                                |
| `.chutes-build/lsp.json`      | LSP server configuration                            |
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `CHUTES_API_KEY`         | API key from [chutes.ai/app/api](https://chutes.ai/app/api). Used for custom endpoint auth and API key login      |
| `CHUTES_BUILD_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.chutes.ai/v1`)                          |
| `CHUTES_BUILD_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `CHUTES_BUILD_MODELS_LIST_URL`          | Override the model list URL if it differs from `{CHUTES_BUILD_MODELS_BASE_URL}/models`                                              |
| `CHUTES_BUILD_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `CHUTES_BUILD_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `CHUTES_BUILD_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `CHUTES_BUILD_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `CHUTES_BUILD_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `CHUTES_BUILD_HOME`                     | Override config directory (default: `~/.chutes-build`)                                                           |
| `CHUTES_BUILD_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `CHUTES_BUILD_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `CHUTES_BUILD_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `CHUTES_BUILD_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `CHUTES_BUILD_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `CHUTES_BUILD_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `CHUTES_BUILD_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `CHUTES_BUILD_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `CHUTES_BUILD_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `CHUTES_BUILD_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.chutes-build/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `CHUTES_BUILD_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `chutes-build` commands and flags.
...
chutes-build completions bash > ~/.local/share/bash-completion/completions/grok
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Chutes Build-managed location):

```bash
mkdir -p ~/.chutes-build/completions/bash
chutes-build completions bash > ~/.chutes-build/completions/bash/grok.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.chutes-build/completions/bash/grok.bash" ]] && source "$HOME/.chutes-build/completions/bash/grok.bash"
```
...
chutes-build completions zsh > ~/.zsh/completions/_grok
```
...
```bash
mkdir -p ~/.chutes-build/completions/zsh
chutes-build completions zsh > ~/.chutes-build/completions/zsh/_grok
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.chutes-build/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `chutes-build` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `chutes-build --debug` or `CHUTES_BUILD_LOG_FILE` instead:

```bash
# Per-session debug log (~/.chutes-build/debug/<sessionId>.txt)
chutes-build --debug

# Log to a custom path
CHUTES_BUILD_LOG_FILE=/tmp/grok-debug.log chutes-build

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.chutes-build/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `CHUTES_BUILD_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
CHUTES_BUILD_LOG_FILE=/tmp/grok-debug.log RUST_LOG="info,xai_grok_shell::auth=debug" chutes-build
```
...
ls ~/.chutes-build/sessions/

# Read session metadata
cat ~/.chutes-build/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.chutes-build/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.chutes-build/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
