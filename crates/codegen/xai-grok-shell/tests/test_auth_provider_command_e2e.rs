...
//! a throwaway `CHUTES_BUILD_HOME`, no network, and a provider command that needs no
...
/// Point the process at a throwaway Chutes Build home. `grok_home()` memoizes into a
...
fn use_temp_grok_home(dir: &Path) {
    ...
    unsafe {
        std::env::set_var("CHUTES_BUILD_HOME", dir);
    }
}
