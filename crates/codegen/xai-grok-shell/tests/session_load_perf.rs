...
//! `CHUTES_BUILD_PERF_*`), or a real session dir via `CHUTES_BUILD_PERF_SESSION_SRC=<session-dir>`.
//!
...
/// real session; scale/override via `CHUTES_BUILD_PERF_*` (e.g. `CHUTES_BUILD_PERF_TURNS`,
/// `CHUTES_BUILD_PERF_SCALE`), or point `CHUTES_BUILD_PERF_SESSION_SRC` at a real session dir.
...
/// `CHUTES_BUILD_PERF_SESSION_SRC` set, copy a real session over a registered stub
...
async fn prepare_session(root: &Path, cwd: &Path, spec: &SessionSpec) -> (Info, PathBuf) {
    let Ok(src) = std::env::var("CHUTES_BUILD_PERF_SESSION_SRC") else {
