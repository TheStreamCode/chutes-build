...
async fn sleep_tool_records_multi_second_dispatch_duration() {
    ...
    assert_headless_success(&result, "chutes-build -p sleep timing smoke", Some(&server));
    assert_no_crashes(&result.stderr);

    // Session artifacts live under the sandbox CHUTES_BUILD_HOME.
    let home = sandbox.grok_home();
    let events_files = collect_events_jsonl(home);
    assert!(
        !events_files.is_empty(),
        "no events.jsonl under CHUTES_BUILD_HOME {}\nstderr:\n{}",
        home.display(),
