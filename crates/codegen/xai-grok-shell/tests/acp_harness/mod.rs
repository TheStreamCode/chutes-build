    ...
    C: acp::Client + 'static,
{
    ...
    let method = init
        ...
        .expect("chutes.api_key auth method not advertised");
    tokio::time::timeout(
        RPC_TIMEOUT,
...
fn set_test_env(grok_home: &std::path::Path, server_url: &str) {
    ...
    unsafe {
        ...
        std::env::set_var("CHUTES_BUILD_CLI_CHAT_PROXY_BASE_URL", server_url);
        std::env::set_var("CHUTES_BUILD_XAI_API_BASE_URL", server_url);
        ...
        std::env::set_var("CHUTES_BUILD_TURN_SUMMARY", "false");
    }
}

/// Runs `body` against a mock inference server with `CHUTES_BUILD_HOME` isolated to a
    ...
    Fut: std::future::Future<Output = ()>,
{
    ...
    let grok_home = tempfile::TempDir::new().expect("Chutes Build home");
