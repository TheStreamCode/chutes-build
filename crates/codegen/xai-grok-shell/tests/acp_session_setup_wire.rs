...
async fn resident_sessions(conn: &acp::ClientSideConnection) -> u64 {
    let resp = ext_method(conn, "chutes.build/debug/agent", json!({})).await;
    resp["result"]["registries"]["sessions"]
        .as_u64()
        .unwrap_or_else(|| panic!("chutes.build/debug/agent: no sessions count in {resp}"))
}
...
async fn assert_resume_does_not_replay(
    ...
    assert!(
        meta.contains_key("chutes.build/sessionConfig"),
