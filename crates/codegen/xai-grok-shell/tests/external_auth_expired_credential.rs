...
//!
//! One `#[test]`: the phases share one process-global `CHUTES_BUILD_HOME` and env, so
...
fn seed_credential(grok_home: &Path, expires_at: chrono::DateTime<chrono::Utc>) {
    ...
    std::fs::create_dir_all(grok_home).expect("create Chutes Build home");
    std::fs::write(
...
fn expired_external_credential_routes_to_the_provider_login_flow() {
    ...
    unsafe {
        ...
        std::env::set_var("CHUTES_BUILD_MODELS_BASE_URL", server.url());
        std::env::set_var("CHUTES_BUILD_AUTH_PROVIDER_COMMAND", &provider);
        std::env::set_var("CHUTES_BUILD_AUTH_PROVIDER_LABEL", PROVIDER_LABEL);
