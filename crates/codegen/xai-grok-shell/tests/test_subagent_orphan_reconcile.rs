...
//!
//! This test spawns a real `chutes-build agent stdio` process, seeds an orphaned
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // GrokStdioClient sets HOME=<temp>; the binary uses <HOME>/.chutes-build as CHUTES_BUILD_HOME.
        let grok_home = shared_home.path().join(".chutes-build");
