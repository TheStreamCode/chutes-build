...
//! script the test writes to disk). The harness's `CHUTES_API_KEY` stands in for
//! the session-tier credential.
//!
//! `#[ignore]` (needs a built binary). The CI lifecycle lanes run it against
//! the release artifact via `CHUTES_BUILD_BINARY`; run locally (auto-builds the pager):
//! ```bash
...
async fn provider_backed_model_sends_minted_token_on_the_wire() {
    ...
    sandbox.remove_env("CHUTES_BUILD_LEADER_SOCKET");
...
"#,
    ...
    // `test-key-for-ci` is the harness's CHUTES_API_KEY; it stands in for the
