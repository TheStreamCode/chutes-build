...
//!
//! Each test spawns a real `chutes-build agent stdio` process against a mock
...
/// Find `summary.json` for `session_id` under `<home>/.chutes-build/sessions/` and
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let grok_dir = home.path().join(".chutes-build");
        std::fs::create_dir_all(&grok_dir).expect("create .chutes-build dir");
        std::fs::write(
