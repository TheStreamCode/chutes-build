...
/// Per-registry entry counts. `chutes.ai/debug/agent` answers under the extension
...
async fn registry_counts(client: &mut LeaderClient, id: u64) -> serde_json::Value {
    ...
    assert!(
        counts.is_object(),
        "chutes.build/debug/agent returned no registries: {resp}"
    );
    counts
}
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    local
        .run_until(async {
            ...
            rpc(
                &mut bootstrap,
                r#"{"jsonrpc":"2.0","id":2,"method":"authenticate","params":{"methodId":"chutes.api_key","_meta":{"headless":true}}}"#.to_string(),
                2,
                "authenticate",
            )
            .await;
