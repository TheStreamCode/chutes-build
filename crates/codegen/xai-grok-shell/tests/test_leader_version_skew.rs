...
//! - `CHUTES_BUILD_BINARY_CLIENT` — the second client.
//!
            ...
            common::leader::run_with_cleanup(&fixture, &mut clients, |fixture, clients| {
                Box::pin(async move {
                    ...
                    let control = LeaderClient::connect(
                        sandbox.home().join(".chutes-build").join("leader.sock"),
                        "grok-pager-update",
                        ClientMode::Stdio,
            ...
            common::leader::run_with_cleanup(&fixture, &mut clients, |fixture, clients| {
                Box::pin(async move {
                    ...
                    let old_pid = wait_for_live_leader(sandbox.home(), Duration::from_secs(10))
                        .await
                        .expect("no live old leader");
                    let auth_path = sandbox.home().join(".chutes-build").join("auth.json");
