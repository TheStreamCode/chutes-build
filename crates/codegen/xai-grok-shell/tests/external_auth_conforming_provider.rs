...
//!
//! Operator binaries live outside this repo and read `CHUTES_BUILD_AUTH_EXPIRED=1` as
...
fn write_conforming_provider(home: &Path) -> String {
    ...
    std::fs::write(
        ...
        format!(
            "#!/bin/sh\n\
             echo \"expired=${{CHUTES_BUILD_AUTH_EXPIRED:-unset}}\" >> {log}\n\
...
async fn a_provider_that_declines_the_headless_run_can_still_sign_the_user_in() {
    let home = tempfile::tempdir().expect("Chutes Build home");
    ...
    unsafe {
        ...
        std::env::set_var("CHUTES_BUILD_CLI_CHAT_PROXY_BASE_URL", &dead);
        std::env::set_var("CHUTES_BUILD_XAI_API_BASE_URL", &dead);
