...
async fn test_default_model_uses_grok_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Chutes Build") || sys_prompt.contains("grok"),
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_grok =
            original_sys_prompt.contains("Chutes Build") || original_sys_prompt.contains("grok");
        let resumed_has_grok =
            resumed_sys_prompt.contains("Chutes Build") || resumed_sys_prompt.contains("grok");
        assert_eq!(
            original_has_grok,
            resumed_has_grok,
...
async fn test_grok_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            assert!(
            result.is_ok(),
            "prompt with CHUTES_BUILD_AGENT override failed: {:?}\nstderr:\n{}",
            result.err(),
            client.stderr()
        );
            let sys_prompt = server
            ...
            assert!(
            sys_prompt.contains("Chutes Build") || sys_prompt.contains("grok"),
            "CHUTES_BUILD_AGENT=grok-build should override catalog model agent_type\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
        })
        .await;
