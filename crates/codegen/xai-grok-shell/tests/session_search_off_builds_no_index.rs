//! Its own binary: the Chutes Build home resolves once per process.
...
async fn saved_session_is_neither_indexed_nor_found_with_search_off() {
    ...
    let root = dir.path();
    let _home = EnvGuard::set("CHUTES_BUILD_HOME", root);
    let _off = EnvGuard::set("CHUTES_BUILD_SESSION_SEARCH", "0");
    ...
    assert_eq!(
        search.off_reason(),
        Some("the CHUTES_BUILD_SESSION_SEARCH environment variable"),
