...
fn env_budget_ms(name: &str, default: Duration) -> (u64, &'static str) {
    ...
    let note = if raw.is_some() {
        ...
        " (default scaled by CHUTES_BUILD_TEST_TIMEOUT_SCALE)"
    };
...
async fn every_startup_phase_and_the_boot_total_fit_their_budgets() {
    ...
    assert!(
        ...
        "startup phase(s) over the {phase_budget_ms}ms budget \
         (CHUTES_BUILD_PERF_STARTUP_PHASE_BUDGET_MS overrides{phase_note}) in a {total_ms}ms boot: \
         {over:?}\n\
         all phases: {phases:?}",
    );
    ...
    assert!(
        ...
        "startup total {total_ms}ms is over the {total_budget_ms}ms budget \
         (CHUTES_BUILD_PERF_STARTUP_TOTAL_BUDGET_MS overrides{total_note}; {attributed}ms inside phases): \
         {phases:?}",
    );
}
