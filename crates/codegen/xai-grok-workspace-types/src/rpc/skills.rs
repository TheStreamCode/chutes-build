...
pub enum SkillScope {
    /// cwd/.chutes-build/skills
    Local,
    /// repo_root/.chutes-build/skills
    Repo,
    /// ~/.chutes-build/skills
    User,
    /// ~/.chutes-build/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.chutes-build/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.chutes-build/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.chutes-build/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.chutes-build/plugins/infra-plugin",
            "plugin_data": "/root/.chutes-build/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "grok-4",
            "effort": "high",
            "user_invocable": true,
