...
pub const REPOS_MANIFEST_RELATIVE_PATH: &str = ".chutes-build/repos.json";
...
impl RepoManifest {
    ...
    /// Parse bytes from `{workspace}/.chutes-build/repos.json`.
    ...
    pub fn materialized_mounts(&self, workspace_root: &std::path::Path) -> Vec<std::path::PathBuf> {
        ...
        for repo in &self.repos {
            ...
            // Confine to the workspace: a compromised/malicious `.chutes-build/repos.json`
