...
/// (`.chutes-build/`) so BYO remotes / user commits do not pick up machine state.
pub const DEFAULT_GITIGNORE: &str = "\
# Seeded by Chutes Build app workspaces. Secrets and machine state never belong in git;
...
mod tests {
    ...
    fn default_gitignore_excludes_secrets_and_deps() {
        ...
        assert!(DEFAULT_GITIGNORE.contains(".chutes-build/"));
