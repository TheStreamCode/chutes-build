...
// (`~/.chutes-build/trusted_folders.toml`) is the single authority for whether a repo's
...
///
/// Disabled hooks are listed in `$CHUTES_BUILD_HOME/disabled-hooks`, one hook name
/// per line.
...
/// Disable a hook by name (append to `$CHUTES_BUILD_HOME/disabled-hooks`).
...
/// Enable a hook by name (remove from `$CHUTES_BUILD_HOME/disabled-hooks`).
...
/// Returns the path to `$CHUTES_BUILD_HOME/disabled-hooks`, or `None` when no user grok
/// home resolves.
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let grok_dir = dir.join(".chutes-build");
