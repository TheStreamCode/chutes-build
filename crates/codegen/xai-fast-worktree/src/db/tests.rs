...
fn id_from_path_differs_for_same_basename_in_different_repos() {
    ...
    let a = id_from_path(Path::new(
        "/home/.chutes-build/worktrees/repo-a/session/wt-abc",
    ));
    let b = id_from_path(Path::new(
        "/home/.chutes-build/worktrees/repo-b/session/wt-abc",
    ));
    assert_ne!(
        a, b,
