...
mod links {
    ...
    fn emits_osc8_around_linked_cells() {
        ...
        assert!(
            out.contains("\x1b]8;;https://chutes.ai\x07"),
            "missing open: {out:?}"
        );
    ...
    fn emit_id_param_included() {
        let mut t = term(20, 3);
        let out = frame(&mut t, "AB", &[span(0, 2, "https://chutes.ai", Some(7))]);
        assert!(
            out.contains("\x1b]8;id=7;https://chutes.ai\x07"),
    ...
    fn nonzero_origin_viewport_maps_links() {
        ...
        t.set_frame_links(&[LinkSpan {
            ...
            col_start: 2,
            col_end: 4,
            url: "https://chutes.ai".into(),
            id: None,
        }]);
        t.flush_with_links().unwrap();
        let out = String::from_utf8(t.backend().buf.clone()).unwrap();
        assert!(
            out.contains("\x1b]8;;https://chutes.ai\x07AB\x1b]8;;\x07"),
