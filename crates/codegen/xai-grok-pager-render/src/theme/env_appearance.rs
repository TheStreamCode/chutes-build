...
//!
//! `LC_CHUTES_BUILD_APPEARANCE` is the SSH-surviving alias (`AcceptEnv LC_*`).
...
/// Ordered lookup: `CHUTES_BUILD_APPEARANCE`, then `LC_CHUTES_BUILD_APPEARANCE`, then `COLORFGBG`.
#[must_use]
...
pub fn detect_explicit_from_env_map(env: &HashMap<String, String>) -> Option<SystemAppearance> {
    parse_appearance_var(env_nonempty(env, "CHUTES_BUILD_APPEARANCE"))
        .or_else(|| parse_appearance_var(env_nonempty(env, "LC_CHUTES_BUILD_APPEARANCE")))
}
...
mod tests {
    ...
    fn grok_appearance_dark_and_light() {
        assert_eq!(
            detect_from_env_map(&env(&[("CHUTES_BUILD_APPEARANCE", "dark")])),
            Some(SystemAppearance::Dark)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("CHUTES_BUILD_APPEARANCE", "Light")])),
            Some(SystemAppearance::Light)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("CHUTES_BUILD_APPEARANCE", "night")])),
            Some(SystemAppearance::Dark)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("CHUTES_BUILD_APPEARANCE", "day")])),
            Some(SystemAppearance::Light)
        );
    }
    ...
    fn lc_alias_used_when_canonical_absent() {
        assert_eq!(
            detect_from_env_map(&env(&[("LC_CHUTES_BUILD_APPEARANCE", "light")])),
            Some(SystemAppearance::Light)
        );
    }
    ...
    fn canonical_wins_over_lc_and_colorfgbg() {
        assert_eq!(
            detect_from_env_map(&env(&[
                ("CHUTES_BUILD_APPEARANCE", "dark"),
                ("LC_CHUTES_BUILD_APPEARANCE", "light"),
                ("COLORFGBG", "0;15"),
            ])),
            Some(SystemAppearance::Dark)
        );
    ...
    fn unknown_or_empty_appearance_falls_through_to_colorfgbg() {
        assert_eq!(
            detect_from_env_map(&env(&[
                ("CHUTES_BUILD_APPEARANCE", "solarized"),
                ("COLORFGBG", "15;0"),
            ])),
            Some(SystemAppearance::Dark)
        );
    ...
    fn lc_wins_over_conflicting_colorfgbg() {
        ...
        assert_eq!(
            detect_from_env_map(&env(&[
                ("LC_CHUTES_BUILD_APPEARANCE", "dark"),
                ("COLORFGBG", "0;15"),
            ])),
            Some(SystemAppearance::Dark)
        );
    ...
    fn unknown_or_empty_lc_falls_through_to_colorfgbg_when_grok_absent() {
        assert_eq!(
            detect_from_env_map(&env(&[
                ("LC_CHUTES_BUILD_APPEARANCE", "solarized"),
                ("COLORFGBG", "15;0"),
            ])),
            Some(SystemAppearance::Dark)
        );
    ...
    fn colorfgbg_map_ignores_explicit_hints() {
        ...
        assert_eq!(
            detect_colorfgbg_from_env_map(&env(&[("CHUTES_BUILD_APPEARANCE", "dark")])),
            None
        );
    }
}
