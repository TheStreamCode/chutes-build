...
//! The default theme is ChutesNight (graphite surfaces with Chutes green accents).
//!
//! ## Color support
//!
//! ChutesNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::ChutesNight, ThemeKind::ChutesDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::ChutesNight => "chutesnight",
            Self::TokyoNight => "tokyonight",
            Self::ChutesDay => "chutesday",
    ...
    /// when quantized to 256 or 16 colors. ChutesNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::ChutesNight => false,
            Self::TokyoNight => true,
            Self::ChutesDay => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "chutesnight" | "chutes-night" | "dark" => Some(Self::ChutesNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "chutesday" | "chutes-day" | "light" | "day" => Some(Self::ChutesDay),
...
/// `"chutesnight"` → `"Chutes Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "chutesnight" => "Chutes Night",
        "chutesday" => "Chutes Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.chutes-build/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::ChutesNight => Self::chutesnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::ChutesDay => Self::chutesday(),
            ...
            // if reached, fall back to ChutesNight.
            ThemeKind::Auto => Self::chutesnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `CHUTES_BUILD_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on chutesnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., ChutesNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // ChutesNight bg field quantizes to Color::Black and these
            // become invisible.
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::ChutesNight.is_auto());
        assert!(!ThemeKind::ChutesDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::chutesday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::chutesnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::chutesday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. ChutesNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::ChutesNight => Theme::chutesnight(),
                ThemeKind::ChutesDay => Theme::chutesday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_chutesnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark ChutesNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::chutesnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("chutesnight"),
            Some(ThemeKind::ChutesNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::ChutesNight));
        assert_eq!(
            ThemeKind::from_name("chutesday"),
            Some(ThemeKind::ChutesDay)
        );
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::ChutesDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("chutesnight", ThemeKind::ChutesNight),
            ("chutes-night", ThemeKind::ChutesNight),
            ("dark", ThemeKind::ChutesNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("chutesday", ThemeKind::ChutesDay),
            ("chutes-day", ThemeKind::ChutesDay),
            ("light", ThemeKind::ChutesDay),
            ("day", ThemeKind::ChutesDay),
