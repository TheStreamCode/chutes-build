...
//! 2. Explicit env stamps — `CHUTES_BUILD_APPEARANCE` / `LC_CHUTES_BUILD_APPEARANCE`
...
/// 1. Desktop APIs
/// 2. Explicit wrap/SSH stamps (`CHUTES_BUILD_APPEARANCE` / `LC_CHUTES_BUILD_APPEARANCE`)
...
///
/// Used by `chutes-build wrap` to stamp the *local* OS theme into the child env
...
/// in `config.toml`. When `None`, defaults to `ChutesNight` / `ChutesDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::ChutesDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::ChutesNight),
    }
}
