...
//! lookup must be cheaper than re-loading from `~/.chutes-build/config.toml`.
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::ChutesNight as u8);
...
/// back to `ChutesNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::ChutesNight as u8 => ThemeKind::ChutesNight,
        x if x == ThemeKind::ChutesDay as u8 => ThemeKind::ChutesDay,
...
/// When `None`, `to_theme_kind()` defaults to `ChutesNight` / `ChutesDay`.
...
///
/// On the first call, reads from `~/.chutes-build/config.toml` (via the shell's
...
///
/// Precedence:
/// 1. Environment variable (`CHUTES_BUILD_THEME` / `LC_CHUTES_BUILD_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `ChutesNight`
#[must_use]
...
fn env_theme_name_from(env: &HashMap<String, String>) -> Option<&str> {
    for key in ["CHUTES_BUILD_THEME", "LC_CHUTES_BUILD_THEME"] {
        let Some(raw) = env
            .get(key)
            .map(String::as_str)
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::ChutesNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `ChutesNight`
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::ChutesNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::ChutesDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::ChutesNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::ChutesDay);
            assert_eq!(applied, ThemeKind::ChutesNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::ChutesNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::chutesday().text_primary,
                "must not serve the cached (ChutesDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // chutesday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::ChutesDay), false),
                ThemeKind::ChutesDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::ChutesNight);
        });
    }
    ...
    fn resolve_auto_dark_system_returns_groknight() {
        with_test_env(|| {
            ...
            assert_eq!(result, ThemeKind::ChutesNight);
        });
    }
    ...
    fn resolve_auto_light_system_returns_grokday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::ChutesDay);
        });
    }
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::ChutesDay), true);
            assert_eq!(result, ThemeKind::ChutesDay);
            assert!(
                !is_auto_mode(),
    ...
    fn env_theme_overrides_config() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(Some("chutesday"), Some(ThemeKind::TokyoNight), false),
                ThemeKind::ChutesDay
            );
            assert!(!is_auto_mode());
        });
    ...
    fn unknown_env_theme_falls_through_to_config() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(Some("not-a-theme"), Some(ThemeKind::ChutesDay), false),
                ThemeKind::ChutesDay
            );
        });
    }
    ...
    fn grok_theme_wins_over_lc_and_config() {
        with_test_env(|| {
            let env = theme_env(&[
                ("CHUTES_BUILD_THEME", "chutesday"),
                ("LC_CHUTES_BUILD_THEME", "tokyonight"),
            ]);
            assert_eq!(
    ...
    fn empty_or_absent_grok_theme_falls_through_to_lc() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[
                        ("CHUTES_BUILD_THEME", ""),
                        ("LC_CHUTES_BUILD_THEME", "chutesday")
                    ])),
                    Some(ThemeKind::TokyoNight),
                    false,
                ),
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("LC_CHUTES_BUILD_THEME", "chutesday")])),
                    Some(ThemeKind::TokyoNight),
                    true,
                ),
                ThemeKind::ChutesDay
    ...
    fn unknown_grok_theme_falls_through_to_valid_lc() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[
                        ("CHUTES_BUILD_THEME", "not-a-theme"),
                        ("LC_CHUTES_BUILD_THEME", "chutesday"),
                    ])),
                    Some(ThemeKind::TokyoNight),
                    false,
    ...
    fn env_theme_system_alias_arms_auto_on_both_resolve_paths() {
        with_test_env(|| {
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("CHUTES_BUILD_THEME", "system")])),
                    Some(ThemeKind::TokyoNight),
                    false,
                ),
                ThemeKind::ChutesDay
        ...
        with_test_env(|| {
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("LC_CHUTES_BUILD_THEME", "auto")])),
                    Some(ThemeKind::TokyoNight),
                    true,
                ),
                ThemeKind::ChutesNight
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::ChutesDay);
            assert_eq!(current_kind(), ThemeKind::ChutesDay);
        });
    }
