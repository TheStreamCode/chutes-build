...
/// A closed stdout (`chutes-build du | head`) is a clean stop, not a failure.
...
/// `~/.chutes-build` or `$CHUTES_BUILD_HOME`, decided by the resolved home rather than by
/// whether `CHUTES_BUILD_HOME` is set in the environment.
...
mod tests {
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        ...
        let full = format!("{home}/.chutes-build/memory/MEMORY.md");
    ...
    fn abbreviate_path_collapses_home_with_native_separator() {
        ...
        // Stay outside grok_home so this hits the $HOME branch, not ~/.chutes-build.
