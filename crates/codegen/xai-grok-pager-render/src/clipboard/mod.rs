...
pub const CHUTES_BUILD_COPY_FILE_ENV: &str = "CHUTES_BUILD_COPY_FILE";
...
///
/// Set `CHUTES_BUILD_CLIPBOARD_NO_OSC52` (any value) before starting Chutes Build. Presence
/// forces the OSC 52 leg off for the whole process — including Linux "always
/// emit", tmux, SSH, container, and `CHUTES_BUILD_OSC52_SINK` paths. Use this when the
/// host terminal paints OSC 52 payloads as visible garbage (e.g. OpenText
/// Exceed and other non-supporting emulators).
///
/// Same convention as `CHUTES_BUILD_CLIPBOARD_NO_DATA_CONTROL`: env presence enables
/// the kill switch; resolved once and cached for the process lifetime.
pub fn osc52_disabled() -> bool {
    static DISABLED: OnceLock<bool> = OnceLock::new();
    *DISABLED.get_or_init(|| std::env::var_os("CHUTES_BUILD_CLIPBOARD_NO_OSC52").is_some())
}
...
/// matters for non-tmux contexts — unless `CHUTES_BUILD_CLIPBOARD_NO_OSC52` is set,
...
///
/// Override with [`CHUTES_BUILD_COPY_FILE_ENV`] (supports `~`). Otherwise
...
pub fn default_copy_fallback_path() -> Option<std::path::PathBuf> {
    if let Ok(raw) = std::env::var(CHUTES_BUILD_COPY_FILE_ENV) {
...
/// prefix collapses to `~/.chutes-build` (or `$CHUTES_BUILD_HOME` when overridden), and a
...
/// `CHUTES_BUILD_COPY_FILE`) — the backup file is skipped rather than written to a
/// predictable temp location.
///
/// On Unix a missing parent directory is created `0700` (a custom
/// `CHUTES_BUILD_COPY_FILE` may point at a not-yet-created private directory;
/// `~/.chutes-build` normally already exists).
pub fn write_copy_fallback(text: &str) -> std::io::Result<std::path::PathBuf> {
    let Some(path) = default_copy_fallback_path() else {
        return Err(std::io::Error::new(
            ...
            "no home directory resolves; set CHUTES_BUILD_COPY_FILE to enable the copy backup file",
        ));
    };
    #[cfg(unix)]
...
/// clipboard over SSH (notably Apple Terminal without `chutes-build wrap`); a failed
...
mod tests {
    ...
    fn clipboard_route_no_osc52_kill_switch_forces_off() {
        // CHUTES_BUILD_CLIPBOARD_NO_OSC52 must win over Linux/tmux/SSH automatic emit.
        for ctx in [
            plain_terminal_ctx(),
            plain_tmux_ctx(),
            byobu_tmux_ctx(),
    ...
    /// the file must be owner-only (`0600`) — including when an older chutes-build
    ...
    fn default_copy_fallback_path_respects_grok_copy_file() {
        ...
        unsafe {
            std::env::set_var(CHUTES_BUILD_COPY_FILE_ENV, &custom);
        }
        let resolved = default_copy_fallback_path();
        unsafe {
            std::env::remove_var(CHUTES_BUILD_COPY_FILE_ENV);
        }
    ...
    fn default_copy_fallback_path_is_grok_home() {
        ...
        // Test envs always resolve a home (or set CHUTES_BUILD_HOME).
    ...
    /// the `CHUTES_BUILD_HOME`-override integration test in `xai-grok-pager`).
    #[test]
