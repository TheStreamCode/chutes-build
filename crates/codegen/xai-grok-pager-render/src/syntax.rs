...
//! Dark themes (ChutesNight, TokyoNight) share `grok-night.tmTheme`;
//! ChutesDay uses `grok-day.tmTheme` with deepened colors for light backgrounds.
//!
...
//! `current_kind()` is a nominal `ChutesNight` (so leftover kind-keyed paths
...
/// reports a nominal `ChutesNight`, so this returns the night theme. Token
