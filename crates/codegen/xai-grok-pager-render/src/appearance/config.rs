...
pub struct AnimationConfig {
    ...
    /// Also enabled by the `CHUTES_BUILD_FPS=1` env var. Default: false.
    pub show_fps: bool,
}
...
pub struct RawAnimationConfig {
    ...
    /// Requires a debug build. Also enabled by CHUTES_BUILD_FPS=1 env var. Default: false.
    pub show_fps: bool,
}
...
fn lookup_named_color(name: &str) -> Result<Color, String> {
    // Named colors use the ChutesNight RGB palette. They are quantized via
...
impl RawAppearanceConfig {
    pub fn to_toml_with_comments() -> String {
        ...
        let header = format!(
            "\
# Chutes Build Pager Appearance Configuration ({pager_path})
...
pub fn persist_respect_manual_folds(enabled: bool) -> std::io::Result<()> {
    ...
    if xai_grok_config::user_grok_home().is_none() {
        return Err(Error::new(
            ErrorKind::NotFound,
            "no user Chutes Build home resolved; refusing to write a cwd-relative pager.toml \
