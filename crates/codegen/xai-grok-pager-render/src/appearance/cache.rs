...
/// Default ON when unset. The `CHUTES_BUILD_PROMPT_SUGGESTIONS` env var overrides
...
/// first call. `CHUTES_BUILD_SCROLL_SPEED` overrides config.toml — matches the
/// legacy `appearance::persist::load_scroll_speed` behaviour.
pub fn load_scroll_speed() -> u8 {
    SCROLL_SPEED_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("CHUTES_BUILD_SCROLL_SPEED")
                .ok()
...
/// `CHUTES_BUILD_SCROLL_MODE` overrides `[ui].scroll_mode` (the `CHUTES_BUILD_SCROLL_SPEED`
...
pub fn load_scroll_mode() -> ScrollMode {
    SCROLL_MODE_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("CHUTES_BUILD_SCROLL_MODE")
                .ok()
...
/// `CHUTES_BUILD_INVERT_SCROLL` (`1`/`true`/`0`/`false`) overrides `[ui].invert_scroll`.
pub fn load_invert_scroll() -> bool {
    INVERT_SCROLL_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("CHUTES_BUILD_INVERT_SCROLL")
                .ok()
...
/// its own lines-per-tick. `CHUTES_BUILD_SCROLL_LINES` overrides `[ui].scroll_lines`.
pub fn load_scroll_lines() -> Option<u8> {
    SCROLL_LINES_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("CHUTES_BUILD_SCROLL_LINES")
                .ok()
