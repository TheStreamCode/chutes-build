//! Headless markdown analysis sharing Chutes Build's exact `pulldown-cmark` config.
//!
...
//! `xai-grok-markdown` uses the same options, so analysis matches what Chutes Build renders.
//!
//! After parsing, Chutes Build applies [`offset_events`]: only `~~…~~` counts as strikethrough.
...
/// The exact `pulldown-cmark` option set Chutes Build uses to render markdown.
///
...
/// Returns Chutes Build's parser events with source byte ranges, single-tilde strikethrough already demoted.
///
...
/// Parse `text` with Chutes Build's options; count elements and flag structural issues.
pub fn analyze(text: &str) -> MarkdownAnalysis {
    ...
    // `offset_events` attaches byte ranges and demotes single-tilde strike, so counts match what Chutes Build renders
