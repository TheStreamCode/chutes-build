...
//! feature set, shared with `xai-grok-markdown` so analysis matches what Chutes Build
//! Build actually renders 1:1.
//!
//! After parsing, Chutes Build applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// Offset event stream from Chutes Build's parser, with single-tilde strikethrough demoted.
///
