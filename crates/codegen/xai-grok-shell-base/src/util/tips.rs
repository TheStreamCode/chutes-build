...
//! persisted to `~/.chutes-build/tip_cursor.json`.
...
/// Load the cursor from `~/.chutes-build/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.chutes-build/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.chutes-build/tip_cursor.json`, so every session sees the next tip
