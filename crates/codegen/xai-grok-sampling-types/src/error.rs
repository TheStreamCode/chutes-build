...
/// Flat error from the Chutes Build proxy/gateway: `{"code": "...", "error": "..."}`.
...
pub fn status_user_message(status: StatusCode) -> String {
    match status.as_u16() {
        code @ 502..=504 => {
            format!(
                "Chutes Build is temporarily unavailable. Please try again in a moment. (HTTP {code})."
            )
        }
        // Upstream capacity, not an edge failure — see [`SamplingError::is_overloaded`].
        code @ 529 => {
            format!(
                "Chutes Build is temporarily overloaded. Please try again in a moment. (HTTP {code})."
            )
        }
        ...
        code @ 520..=524 | code @ 530 => {
            format!(
                "Connection to Chutes Build timed out or was interrupted. Please try again. (HTTP {code})."
            )
        }
        // Cloudflare origin TLS (handshake / invalid certificate) — not transient.
        code @ 525 | code @ 526 => {
            format!("Secure connection to Chutes Build failed. (HTTP {code}).")
        }
...
<html lang="en-US">
<head><title>chutes.ai | 524: A timeout occurred</title></head>
