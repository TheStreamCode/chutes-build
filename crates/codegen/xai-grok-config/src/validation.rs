...
/// with `CHUTES_BUILD_MANAGED_CONFIG_URL`; only applies to `requirements.toml`.
pub(crate) const FAIL_CLOSED_ENV: &str = "CHUTES_BUILD_MANAGED_CONFIG_FAIL_CLOSED";
...
impl RequirementsSource {
    ...
    /// id (`ai.x.chutes-build:…`). For diagnostics and matching only; the MDM layer has no
...
pub struct RequirementsLayer {
    ...
    /// not re-derive from the source (`CHUTES_BUILD_HOME`-influenced, could carry `..`).
    pub is_system: bool,
}
...
/// no-op (no cwd-relative `.chutes-build/requirements.toml` is read or enforced).
