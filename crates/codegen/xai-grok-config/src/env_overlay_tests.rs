...
fn resolve_overlay_over_cap_path_is_ignored() {
    // A `CHUTES_BUILD_CONFIG_PATH` file larger than the cap must be refused (no stall,
...
fn malformed_overlay_parse_errors_do_not_carry_the_value() {
    ...
    assert!(parse_overlay(&bad_toml, OverlayFormat::Toml, CHUTES_BUILD_CONFIG_PATH_ENV).is_none());

    let bad_json = format!("{{\"models\": \"{secret}\",}}");
    assert!(parse_overlay(&bad_json, OverlayFormat::Json, CHUTES_BUILD_CONFIG_ENV).is_none());
}
