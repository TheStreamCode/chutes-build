...
fn request(path: &Path, items: &[(&str, &str)]) -> ManagedConfigRequest {
    ManagedConfigRequest {
        ...
        namespace: "chutes-build doctor".to_owned(),
...
fn missing_empty_normal_no_final_newline_and_crlf_are_preserved() {
    ...
    let plan = ManagedConfig::plan(request(
        &missing,
        &[("terminal.ssh-wrap", "alias ssh='chutes-build wrap ssh'")],
    ))
    .unwrap();
    assert_eq!(
        plan.updated_bytes(),
        expected("alias ssh='chutes-build wrap ssh'", "\n").as_bytes()
    );
...
fn prose_and_exports_with_owned_words_and_chevrons_are_inert() {
    ...
    let content = [
        ...
        "# chutes-build doctor docs say >>> run this later <<<",
        "export NOTE='terminal.ssh-wrap >>> not a marker'",
        "printf '%s\\n' 'chutes-build doctor <<< prose >>>'",
...
fn invalid_inputs_and_all_marker_shapes_are_refused() {
    ...
    .unwrap();
    // Not `nul`: on Windows that name is the null device wherever it appears, so the
    // write goes nowhere, the file reads back empty, and the NUL-byte refusal this
