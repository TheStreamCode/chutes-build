...
/// Unsigned folder-trust store (`$CHUTES_BUILD_HOME/trusted_folders.toml`).
pub const TRUSTED_FOLDERS_FILENAME: &str = "trusted_folders.toml";

/// User-global sandbox profile definitions (`$CHUTES_BUILD_HOME/sandbox.toml`).
pub const SANDBOX_CONFIG_FILENAME: &str = "sandbox.toml";

/// Legacy project-hook trust list (`$CHUTES_BUILD_HOME/trusted-hook-projects`).
...
/// Plugin trust list (`$CHUTES_BUILD_HOME/trusted-plugins`).
...
pub enum HookProvenance {
    ...
    SystemManaged,
    /// `$CHUTES_BUILD_HOME/managed_config.toml` (server-synced, user-writable).
    Managed,
    /// System-tier `requirements.toml` (root-owned, e.g. `/etc/grok`).
    Requirements,
    /// `$CHUTES_BUILD_HOME/requirements.toml` (user-writable).
    UserRequirements,
...
impl HookProvenance {
    ...
    /// `$CHUTES_BUILD_HOME` tiers (`Managed`, `UserRequirements`) never qualify: the
    ...
    /// timeout/env. Root-owned tiers outrank `$CHUTES_BUILD_HOME` tiers.
    ///
...
mod tests {
    ...
    /// `$CHUTES_BUILD_HOME/requirements.toml` stamps `UserRequirements` (never the
