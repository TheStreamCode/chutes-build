//! `CHUTES_BUILD_CONFIG` / `CHUTES_BUILD_CONFIG_PATH` config overlay resolution.
...
pub const CHUTES_BUILD_CONFIG_ENV: &str = "CHUTES_BUILD_CONFIG";

/// Path to an additional JSON or TOML config-overlay file (read by extension).
pub const CHUTES_BUILD_CONFIG_PATH_ENV: &str = "CHUTES_BUILD_CONFIG_PATH";

/// Hard cap on a `CHUTES_BUILD_CONFIG_PATH` overlay read. Matches the untrusted-config
...
/// A resolved overlay for the merge and for `chutes-build inspect`.
#[derive(Debug, Clone)]
...
fn env_overlay_inputs() -> (Option<String>, Option<PathBuf>) {
    let inline = match std::env::var_os(CHUTES_BUILD_CONFIG_ENV) {
        Some(raw) => match raw.into_string() {
            Ok(s) => Some(s),
            Err(_) => {
                tracing::warn!("CHUTES_BUILD_CONFIG is not valid UTF-8; ignoring the overlay");
                None
            }
        },
        None => None,
    };
    let path = std::env::var_os(CHUTES_BUILD_CONFIG_PATH_ENV)
        .filter(|v| !v.is_empty())
        .map(PathBuf::from);
    (inline, path)
}
...
pub(crate) fn load_env_overlay() -> Option<toml::Value> {
    ...
    let source_label = match source {
        OverlaySource::Inline => CHUTES_BUILD_CONFIG_ENV,
        OverlaySource::Path(_) => CHUTES_BUILD_CONFIG_PATH_ENV,
    };
    tracing::trace!(
        source = source_label,
        ?sections,
        "resolved CHUTES_BUILD_CONFIG overlay"
    );
    Some(overlay)
}

/// The resolved overlay for `chutes-build inspect`.
...
fn resolve_overlay_detailed(
    ...
    // Inline (`CHUTES_BUILD_CONFIG`) wins only when it fully succeeds. When it is
    // absent, empty, or rejected at any stage (parse, `$VAR` expand,
    // version_overrides, normalize), fall through to the `CHUTES_BUILD_CONFIG_PATH`
...
fn resolve_inline_overlay(inline: &str) -> Option<(toml::Value, OverlaySource, Vec<String>)> {
    ...
    let overlay = parse_overlay(trimmed, OverlayFormat::Json, CHUTES_BUILD_CONFIG_ENV)?;
    finalize_overlay(overlay, CHUTES_BUILD_CONFIG_ENV, OverlaySource::Inline)
}

/// Read, parse, and run the full pipeline for the `CHUTES_BUILD_CONFIG_PATH` candidate.
fn resolve_path_overlay(path: &Path) -> Option<(toml::Value, OverlaySource, Vec<String>)> {
    ...
    let overlay = parse_overlay(&raw, format, CHUTES_BUILD_CONFIG_PATH_ENV)?;
    finalize_overlay(
        overlay,
        CHUTES_BUILD_CONFIG_PATH_ENV,
        OverlaySource::Path(path.to_path_buf()),
    )
}

/// Read `CHUTES_BUILD_CONFIG_PATH` with a hard byte cap ([`MAX_OVERLAY_BYTES`]). Returns
...
fn read_capped_overlay_file(path: &Path) -> Option<String> {
    ...
    let file = match std::fs::File::open(path) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(path = %path.display(), error = %e, "CHUTES_BUILD_CONFIG_PATH is unreadable; ignoring the overlay");
            return None;
        }
    };
    ...
    match file.metadata() {
        Ok(meta) if !meta.file_type().is_file() => {
            tracing::warn!(path = %path.display(), "CHUTES_BUILD_CONFIG_PATH is not a regular file; ignoring the overlay");
            return None;
        }
        Ok(_) => {}
        Err(e) => {
    ...
    if raw.len() as u64 > MAX_OVERLAY_BYTES {
        tracing::warn!(path = %path.display(), max = MAX_OVERLAY_BYTES, "CHUTES_BUILD_CONFIG_PATH exceeds the max overlay size; ignoring the overlay");
        return None;
    }
    Some(raw)
}
