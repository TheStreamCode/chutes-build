...
/// `~/.chutes-build/marketplace-cache`. `dunce` strips the prefix whenever the path
...
pub fn grok_home() -> PathBuf {
    CHUTES_BUILD_HOME
        .get_or_init(|| {
            let grok_home = if let Ok(v) = std::env::var("CHUTES_BUILD_HOME") {
                PathBuf::from(v)
            } else {
                default_grok_home()
            };
...
/// mistake a project's `.chutes-build` tree for the user-global one when no home resolves.
pub fn user_grok_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable =
        std::env::var_os("CHUTES_BUILD_HOME").is_some() || std::env::home_dir().is_some();
