//! Config file loading for Chutes Build.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/grok/managed_config.toml`
//! 2. `$CHUTES_BUILD_HOME/managed_config.toml`
//! 3. `$CHUTES_BUILD_HOME/config.toml`
//! 4. `$CHUTES_BUILD_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/grok/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.chutes-build`, admin-forced) — macOS only
//!
...
pub use env_overlay::{
    CHUTES_BUILD_CONFIG_ENV, CHUTES_BUILD_CONFIG_PATH_ENV, OverlaySource, ResolvedOverlay,
    resolved_env_overlay,
};
#[cfg(unix)]
pub use global_hook_sources::{
