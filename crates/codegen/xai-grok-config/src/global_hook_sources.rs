//! Chutes Build-owned direct global hook paths shared by shell discovery and sandbox
//! write-deny: `$CHUTES_BUILD_HOME/hooks`, `hooks-paths`, and absolute registry targets.
...
pub enum GlobalHookSourceKind {
    /// `$CHUTES_BUILD_HOME/hooks/` (discovered + protected).
    HookDirectory,
    /// `$CHUTES_BUILD_HOME/hooks-paths` (protected; never loaded as hook JSON).
    RegistryFile,
    /// Absolute registry target (must exist before sandbox apply).
    ConfiguredSource,
    /// `$CHUTES_BUILD_HOME` trust-boundary file (protected; never hook JSON / discovery).
    TrustBoundaryFile,
}
...
pub enum GlobalHookSourceError {
    ...
    #[error("symlinked CHUTES_BUILD_HOME is not allowed under sandbox write-deny: {path}")]
    ...
    #[error("Chutes Build hooks directory has wrong type (expected real directory): {path}")]
    InvalidHooksDir { path: PathBuf },
    #[error("Chutes Build hooks-paths registry has wrong type (expected real file): {path}")]
    InvalidRegistryFile { path: PathBuf },
    #[error("cannot create Chutes Build hooks directory {path}: {source}")]
    CreateHooksDir {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("cannot create Chutes Build hooks-paths registry {path}: {source}")]
    CreateRegistryFile {
        path: PathBuf,
        #[source]
        source: io::Error,
...
/// Ensure real `$CHUTES_BUILD_HOME/hooks` dir + `hooks-paths` file (create if missing).
...
/// `$CHUTES_BUILD_HOME` files that are always-trusted or trust-granting if writable.
...
/// Resolve `$CHUTES_BUILD_HOME` trust-boundary files (symlink-rejected for sandbox).
...
/// Resolve Chutes Build-owned direct global hook sources (`reject_symlinks` for sandbox).
