...
impl MemoryIndex {
    ...
    /// `chutes-build memory doctor` uses this to detect stuck states.
    ...
    ///
    /// `chutes-build memory doctor` uses this to detect orphaned chunks (chunks whose source file has since been deleted).
...
mod tests {
    ...
    fn test_open_or_create_uses_wal_on_local_fs() {
        // CHUTES_BUILD_SQLITE_JOURNAL_MODE overrides the journal-mode decision; skip when it is set
    ...
    /// 3. Run the same orphan-removal logic as `chutes-build memory reindex`:
    ...
    ///
    /// This proves that `chutes-build memory reindex`'s Phase 1 fixes the state that `chutes-build memory doctor` warns about.
    #[test]
    fn test_reindex_maintenance_removes_orphaned_chunks() {
        ...
        // Simulate `chutes-build memory reindex` Phase 1: compare indexed vs current.
    ...
    ///
    /// Verifies that `chutes-build memory reindex` Phase 0 bails when a live session holds a fresh claim.
    ...
    /// `chutes-build memory reindex` Phase 3 resets the stale reindex claim.
    ///
    /// Verifies that `release_claim()` clears `meta.reindex_claim` so that `chutes-build memory doctor` no longer reports a stale lock after reindex runs.
    #[test]
    fn test_reindex_maintenance_resets_stale_claim() {
        ...
        // Simulate `chutes-build memory reindex` Phase 3: release the claim.
        idx.release_claim();
