...
impl MemoryIndex {
    ...
    /// a stale one).  Used by `chutes-build memory doctor` to detect stuck states.
    ...
    ///
    /// Used by `chutes-build memory doctor` to detect orphaned chunks (chunks whose
...
mod tests {
    ...
    ///
    /// This proves that `chutes-build memory reindex`'s Phase 1 actually fixes the
    /// state that `chutes-build memory doctor` warns about.
    #[test]
    ...
    ///
    /// Verifies that `chutes-build memory reindex` Phase 0 correctly bails when a
    ...
    /// `chutes-build memory doctor` no longer reports a stale lock after reindex runs.
    #[test]
