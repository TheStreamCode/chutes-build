...
mod tests {
    ...
    async fn test_trusted_endpoint_prefers_session_credential() {
        ...
        let provider =
            build_embedding_provider(Some(&config), &scoped, None, "https://api.chutes.ai/v1")
                .await;
        assert!(
            provider.is_some(),
    ...
    fn endpoint_scoped_credentials_trust_gate_and_url_match() {
        ...
        assert!(!scoped.is_empty(), "trusted endpoint keeps the credential");
        assert!(
            scoped.approved_for("https://API.chutes.ai/v1"),
            "host casing normalizes"
        );
        assert!(
            !scoped.approved_for("https://api.chutes.ai/v2"),
