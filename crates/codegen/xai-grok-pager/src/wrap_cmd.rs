//! `chutes-build wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `chutes-build ssh` wrapper: spawns an arbitrary command inside a
...
fn derive_spawn(
    ...
    // (`chutes-build wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`chutes-build wrap "$PROG" ...` with `$PROG` unset)
...
///
/// Unlike `chutes-build ssh`, `chutes-build wrap` does not gate on the terminal brand: the user
