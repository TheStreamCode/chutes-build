//! `chutes-build wrap` runs any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `chutes-build ssh` wrapper: spawns an arbitrary command inside a local pseudo-terminal.
...
//! It also stamps `LC_CHUTES_BUILD_APPEARANCE` from the local OS theme so a remote `theme = "auto"` can resolve over SSH and tmux.
//!
//! Resolvable programs spawn directly.
//! On Unix, a command a direct spawn cannot run (a single shell-quoted string `chutes-build wrap "mycli ssh host"` or a shell alias) goes to `$SHELL -i -c`.
...
/// Run the `chutes-build wrap` command.
///
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("chutes-build wrap: no command given"))?;
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!(
                    "chutes-build wrap: wrapped mode failed, running without PTY wrapping: {e}"
                );
...
/// The program and argv `chutes-build wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // A single argument containing whitespace is a shell-quoted command line (`chutes-build wrap "mycli ssh host"`), not a program name
    ...
    // An empty one (`chutes-build wrap "$PROG" ...` with `$PROG` unset) must keep failing fast instead of silently running the tail
...
///
/// Unlike `chutes-build ssh`, `chutes-build wrap` does not gate on the terminal brand.
