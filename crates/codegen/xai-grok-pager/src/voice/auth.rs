...
//!
//! voice-api accepts both API keys and OAuth2 tokens directly at `api.chutes.ai` and attributes per-user billing for OAuth.
...
///
/// Works for every auth method: OAuth / chutes.ai / OIDC session tokens and `CHUTES_API_KEY` / per-model BYOK keys.
