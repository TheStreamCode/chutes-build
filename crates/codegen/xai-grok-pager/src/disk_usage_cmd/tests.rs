...
fn worktrees_dominate_at_half_of_total() {
    ...
    for case in cases {
        let report = DiskUsageReport {
            grok_home: "/home/user/.chutes-build".into(),
...
fn json_shape_is_frozen() {
    let report = DiskUsageReport {
        ...
        registry: RegistryState::Read,
        registry_path: "/home/user/.chutes-build/worktrees.db".into(),
        worktrees: vec![
            WorktreeUsage {
                last_modified_at: Some(1_700_005_000),
                path: "/home/user/.chutes-build/worktrees/xai/wt-1".into(),
                ..tracked_row(
                    90,
                    TrackedRow {
            ...
            WorktreeUsage {
                ...
                path: "/home/user/.chutes-build/worktree_pool/inst/wt-2".into(),
                ..untracked_row(10)
            },
        ],
    };
    assert_eq!(
        ...
        serde_json::json!({
            "schema_version": 1,
            "grok_home": "/home/user/.chutes-build",
            "total_bytes": 100,
            ...
            "worktrees_outside_managed_roots": 0,
            "registry": "read",
            "registry_path": "/home/user/.chutes-build/worktrees.db",
            "worktrees": [
                {
                    "bytes": 90,
                    "kind": "session",
                    ...
                    "repo_name": "xai",
                    "git_ref": "brian/fix",
                    "path": "/home/user/.chutes-build/worktrees/xai/wt-1",
                },
                {
                    "bytes": 10,
                    "kind": "pool",
                    ...
                    "repo_name": null,
                    "git_ref": null,
                    "path": "/home/user/.chutes-build/worktree_pool/inst/wt-2",
                },
            ],
        })
    );
...
fn missing_home_json_is_valid_and_empty() {
    let mut out = Vec::new();
    write_report(
        &empty_report(Path::new("/nonexistent/.chutes-build")),
        /*json*/ true,
        &mut out,
    )
    .unwrap();
    ...
    let cases = [
        ...
        Case {
            ...
            absent: &[
                "db rebuild",
                "damaged",
                "Remove $CHUTES_BUILD_HOME/worktrees.db",
            ],
        },
        Case {
...
fn reclaim_hint_names_a_sequence_that_frees_space() {
    const AGE: &str = "run `chutes-build worktree gc --max-age 7d --dry-run`";
    const RM: &str = "Remove one with `chutes-build worktree rm --dry-run <path>`";
...
fn symlinked_default_home_keeps_home_label() {
    ...
    std::os::unix::fs::symlink(&real_grok, fake_home.join(".chutes-build")).unwrap();
    let _home = crate::test_util::EnvVarGuard::set("HOME", &fake_home);

    let resolved = dunce::canonicalize(&fake_home)
        .unwrap()
        .join(".chutes-build");
