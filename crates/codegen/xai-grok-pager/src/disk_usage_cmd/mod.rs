//! `chutes-build du`: what the user Chutes Build home uses on disk. It creates no Chutes Build home,
...
`chutes-build worktree gc --max-age 7d --dry-run`: without `--max-age`, gc expires nothing, and it \
