...
//!
//! Env sandboxing follows this crate's `serial(CHUTES_BUILD_HOME)` idiom.
...
//! The harness mutates process-global env (proxy URLs, `CHUTES_API_KEY`, `CHUTES_BUILD_LEADER_SOCKET`, `CHUTES_BUILD_HOME`) for a real agent's whole lifetime.
...
/// The Chutes Build home the agent actually persisted under: `grok_home()` is process-cached, so an earlier test in this binary may have pinned it.
...
struct PagerLeaderCluster {
    ...
    /// `kill_leader` aborts and drains them so a respawn can never race a still-running old agent on the same CHUTES_BUILD_HOME.
...
impl PagerLeaderCluster {
    ...
    /// Callers MUST be `#[serial_test::serial(CHUTES_BUILD_HOME)]` (env mutation) and run inside a current-thread `LocalSet`.
    async fn start() -> Self {
        ...
        let env = vec![
            crate::test_util::EnvVarGuard::set("CHUTES_BUILD_HOME", grok_home.path()),
            crate::test_util::EnvVarGuard::set(
                "CHUTES_BUILD_CLI_CHAT_PROXY_BASE_URL",
                server.url(),
            ),
            crate::test_util::EnvVarGuard::set("CHUTES_BUILD_XAI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("CHUTES_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("CHUTES_BUILD_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("CHUTES_BUILD_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("CHUTES_BUILD_TRACE_UPLOAD", "false"),
    ...
    async fn kill_leader(&mut self) {
        ...
        // Without the drain an old agent task could still run against the same CHUTES_BUILD_HOME when the next generation's agent starts
    ...
    /// The socket is pinned via `CHUTES_BUILD_LEADER_SOCKET` and the cluster holds the flock, so reconnects always adopt the in-process server.
