...
//!
//! Env sandboxing follows this crate's `serial(CHUTES_BUILD_HOME)` idiom; note
...
//! mutates process-global env (proxy URLs, `CHUTES_API_KEY`,
//! `CHUTES_BUILD_LEADER_SOCKET`, `CHUTES_BUILD_HOME`) for a real agent's whole lifetime, and
...
/// The Chutes Build home the agent actually persisted under: `grok_home()` is
...
struct PagerLeaderCluster {
    ...
    /// agent on the same CHUTES_BUILD_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(CHUTES_BUILD_HOME)]`
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same CHUTES_BUILD_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `CHUTES_BUILD_LEADER_SOCKET`, flock held
