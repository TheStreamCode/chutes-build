...
struct AgentLoadOutcome {
    ...
    success: bool,
    /// `chutes.ai/runningPromptId` from the reload response: the turn another client is driving mid-reconnect.
    /// Adopted at finalize (mirrors the `SessionLoaded` adoption in `dispatch.rs`).
    running_prompt_id: Option<String>,
    /// `chutes.ai/schedulerBackgroundLoops` from the reload response.
...
pub(crate) async fn run(
    ...
    app.privacy_notice_rollout = xai_grok_config::env_bool("CHUTES_BUILD_PRIVACY_NOTICE_ROLLOUT")
        .or_else(|| {
            remote_settings
                .as_ref()
        ...
        .unwrap_or(false);
    app.privacy_banner_reshow_days = std::env::var("CHUTES_BUILD_PRIVACY_BANNER_RESHOW_DAYS")
        .ok()
    ...
    app.workspace_dashboard_enabled = xai_grok_config::env_bool("CHUTES_BUILD_WORKSPACE_DASHBOARD")
        .or_else(|| {
            remote_settings
                .as_ref()
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            // Voice inherits the same resolved endpoints base as chat (config > CHUTES_BUILD_XAI_API_BASE_URL env > default)
            let endpoints_base =
    ...
    // CHUTES_BUILD_SCROLL_SPEED/_MODE/_LINES and CHUTES_BUILD_INVERT_SCROLL env overrides apply on first load
    ...
    // `chutes-build dashboard` startup: open the dashboard view immediately
    // The CLI subcommand wrote a `CHUTES_BUILD_OPEN_DASHBOARD_AT_STARTUP=1` env var so we don't have to thread a flag through every arg struct
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice could not start. Restart Chutes Build.");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.chutes-build/config.toml [ui]` rather than `~/.chutes-build/pager.toml`
