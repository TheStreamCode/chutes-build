...
pub enum Action {
    ...
    /// Routed to the agent as `chutes.ai/queue/remove`; the resulting `chutes.ai/queue/changed` rebroadcast is the source of truth.
    QueueRemoveShared {
        id: String,
        expected_version: u64,
    },
    /// Reorder the server-authoritative (shared) queued prompts to match `ordered_ids`. Routed as `chutes.ai/queue/reorder`.
    QueueReorderShared {
    ...
    /// Routed to the agent as `chutes.ai/queue/edit`; the rebroadcast of `chutes.ai/queue/changed` is the source of truth.
    ...
    /// Routed as `chutes.ai/queue/interject`.
    /// The `chutes.ai/session/interjection` and `chutes.ai/queue/changed` rebroadcasts are the source of truth (no optimistic client-side block).
    ...
    RunEditedQueuedCommand {
        ...
        /// With nothing to remove, no versioned `chutes.ai/queue/remove` request is sent.
    ...
    ToggleMouseCapture,
    /// Toggle the scroll-diagnostics HUD (hidden `/scroll-debug` command, also `/debug scroll`; `CHUTES_BUILD_SCROLL_DEBUG=1` enables it from startup).
    ToggleScrollDebugHud,
    /// Toggle the release-safe FPS HUD (`/debug fps`).
    ToggleFpsHud,
    /// Toggle the scroll flight recorder at runtime (`/debug log`; `CHUTES_BUILD_SCROLL_LOG=1` enables it from startup).
    ToggleScrollLog,
    ...
    OpenTranscriptPager,
    /// Minimal mode (`chutes-build --minimal`): re-print the last committed folded block, fully expanded, into native scrollback below the conversation.
    ...
    /// Commit the active theme (canonical name, e.g. `"chutesnight"`, `"auto"`).
    SetTheme(String),
...
/// `chutes.ai/yolo_mode_changed` notifications.
...
pub enum Effect {
    ...
    FetchSessionList {
        ...
        generation: u64,
        /// Text search pushed down to `chutes.ai/session/list` as `query` (chat mode: forwarded to the backend conversations search).
        ...
        /// When set, stamped as `_meta["chutes.ai/facetFilters"].kind`.
    ...
    /// Fetch the leader session roster (FleetView dashboard) via `chutes.ai/sessions/list`.
    /// Only issued in leader mode while the dashboard is open.
    FetchRoster,
    /// Fetch the local on-disk session list (dormant/idle sessions) for the dashboard via `chutes.ai/session/list`.
    ...
    /// Persist a typed setting to `~/.chutes-build/config.toml`. On failure,
    ...
    /// Remove a server-owned queued prompt: fire-and-forget `chutes.ai/queue/remove`.
    ...
    /// Clear the caller's server-owned queued prompts: fire-and-forget `chutes.ai/queue/clear`.
    QueueClear { session_id: acp::SessionId },
    /// Replace the text of a server-owned queued prompt in place: fire-and-forget `chutes.ai/queue/edit`.
    /// The session actor's serialized mailbox makes this last-writer-wins for concurrent edits.
    /// The rebroadcast of `chutes.ai/queue/changed` is the truth signal.
    QueueEdit {
        session_id: acp::SessionId,
        id: String,
        new_text: String,
    },
    /// Hold a server-owned row out of combine-on-promote while the composer edits it: fire-and-forget `chutes.ai/queue/hold_edit`.
    QueueHoldEdit {
        session_id: acp::SessionId,
        id: String,
    },
    ...
    /// Interject a server-owned queued prompt into the running turn: fire-and-forget `chutes.ai/queue/interject`.
    ...
    /// Install a plugin from the inline CTA via `chutes.ai/marketplace/action`, reported back via `TaskResult::CtaPluginInstallDone`.
    ...
    /// Reload plugins after a CTA install via `chutes.ai/plugins/action` (`PluginsAction::Reload`), reported back via `TaskResult::CtaPluginReloadDone`.
    /// Modal-independent.
    ReloadPluginsForCta {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    /// Read the MCP server list after a CTA install via `chutes.ai/mcp/list`, reported back via `TaskResult::PluginCtaMcpsLoaded`.
    /// Modal-independent.
    FetchPluginCtaMcps {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    /// Sleeps, then runs the same `chutes.ai/mcp/list` fetch as `FetchPluginCtaMcps`, reported back via `TaskResult::PluginCtaMcpsLoaded`.
    RetryPluginCtaMcps {
        agent_id: AgentId,
        session_id: acp::SessionId,
        plugin_name: String,
    ...
    /// Request a session recap via the chutes.ai/recap ext method.
    ...
    SendInterject {
        ...
        session_id: acp::SessionId,
        text: String,
        /// Client-minted id echoed back on the `chutes.ai/session/interjection` broadcast so the originator can dedup its optimistic local block.
    ...
    /// Log out via `chutes.ai/auth/logout` (shell clears auth.json and in-memory state).
    Logout,
    ...
    /// Delete a session's stored data (local and remote) via `chutes.ai/session/delete`.
    DeleteSession {
        source: String,
        session_id: String,
        cwd: String,
    ...
    /// Call `chutes.ai/session/fork` to create a peer session that resumes from `parent_session_id` in the same cwd (no worktree).
    ...
    /// Send an ACP `chutes.ai/suggest` request to the shell.
    ...
    /// Send an ACP `chutes.ai/suggestPrompt` request to the shell.
...
/// Wire params for `chutes.ai/session/rename`.
...
/// Outcome of an `chutes.ai/subagent/cancel` request, telling dispatch whether the pager must finalize the subagent row itself.
#[derive(Debug)]
...
pub enum TaskResult {
    ...
    SessionCreated {
        ...
        /// Read from the response `_meta["chutes.ai/schedulerBackgroundLoops"]`.
    ...
    SessionLoaded {
        ...
        restore_summary: Option<String>,
        restore_degree: Option<xai_grok_workspace::session::git::RestoreDegree>,
        /// The session's in-flight running prompt id (from the load response `_meta["chutes.ai/runningPromptId"]`).
    ...
    SessionListLoaded {
        ...
        /// A degraded conversations lane (`_meta["chutes.ai/partial"]`), shown as an actionable picker notice instead of a silent empty list.
    ...
    /// `chutes.ai/recap` request acknowledged (fire-and-forget).
    ...
    AuthCancelComplete,
    /// Shell responded to `chutes.ai/auth/check_subscription`.
    ...
    /// `chutes.ai/session/fork` completed (no-worktree path).
    ...
    /// Shell suggestions loaded from ACP `chutes.ai/suggest`.
