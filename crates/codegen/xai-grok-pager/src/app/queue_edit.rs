...
pub enum PromptMode {
    ...
    EditingQueued {
        ...
        /// `chutes.ai/queue/changed` rebroadcast for the visual result) instead
...
impl AgentView {
    ...
    pub(super) fn enter_queue_edit(&mut self, id: u64, is_server: bool, row: Option<QueueRowRef>) {
        ...
        // it. Ignore until the confirming `chutes.ai/queue/changed` lands (mirrors
    ...
    fn interject_edited_queued(
        ...
        match server_id {
            Some(server_id) => {
                // Server rows: the queue wire (`chutes.ai/queue/interject` newText)
