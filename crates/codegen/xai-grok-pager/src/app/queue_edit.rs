...
pub enum PromptMode {
    ...
    EditingQueued {
        ...
        /// The `chutes.ai/queue/changed` rebroadcast paints the result.
...
impl AgentView {
    ...
    pub(super) fn enter_queue_edit(&mut self, id: u64, is_server: bool, row: Option<QueueRowRef>) {
        ...
        // Toast instead of silently dropping the keypress, and wait for the confirming `chutes.ai/queue/changed` before allowing the edit
    ...
    fn interject_edited_queued(
        ...
        match server_id {
            Some(server_id) => {
                // Server rows: the queue wire (`chutes.ai/queue/interject` newText) is text-only, so composer images can't ride along
