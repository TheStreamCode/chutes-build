...
/// Chutes Build home for the replay path. In production this is just `grok_home()`; the
...
#[cfg(test)]
thread_local! {
    static REPLAY_CHUTES_BUILD_HOME: std::cell::RefCell<Option<std::path::PathBuf>> =
        const { std::cell::RefCell::new(None) };
}
/// Override Chutes Build home for disk-replay unit tests (thread-local).
#[cfg(test)]
pub(crate) fn set_replay_grok_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_CHUTES_BUILD_HOME.with(|h| *h.borrow_mut() = home);
}
#[cfg(test)]
fn effective_grok_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_CHUTES_BUILD_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
    xai_grok_shell::util::grok_home::grok_home()
}
