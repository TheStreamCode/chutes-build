...
//! `chutes.build/interject` effect, and prompt-history recording. Split out of
...
/// prompt, and fires the `chutes.build/interject` ext method carrying a client-minted
/// id.
///
/// The shell broadcasts `chutes.build/session/interjection` to every attached pane so
...
pub(super) fn dispatch_interject(
    ...
    // record its id so the broadcast echo (`chutes.build/session/interjection`) is
