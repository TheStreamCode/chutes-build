//! Mid-turn interjection dispatch: optimistic local echo, the `chutes.ai/interject` effect, and prompt-history recording.
...
/// Fires the `chutes.ai/interject` ext method carrying a client-minted id.
///
/// The shell broadcasts `chutes.ai/session/interjection` to every attached pane so other clients viewing the same session render it too.
...
pub(super) fn dispatch_interject_on(
    ...
    // Record its id so the broadcast echo (`chutes.ai/session/interjection`) is deduped instead of rendering a second copy on this pane
