...
/// The shell owns this setting and stores it in auth metadata (persists via ACP ext-request, NOT `~/.chutes-build/config.toml`).
...
pub(super) fn dispatch_manage_billing(app: &mut AppView) -> Vec<Effect> {
    ...
    super::router::dispatch(
        crate::app::actions::Action::OpenUrl("https://chutes.ai/?_s=usage".to_string()),
        app,
    )
}
