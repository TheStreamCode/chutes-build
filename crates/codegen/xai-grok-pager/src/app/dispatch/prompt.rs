...
/// Start (if needed) and submit the initial prompt from `chutes-build "<prompt>"`.
///
...
pub(super) fn dispatch_send_prompt_inner(
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        // The agent appends it to its authoritative `pending_inputs` (turn starts never overlap) and drives the drain via `chutes.ai/queue/changed`
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
            ...
            && agent.session.current_prompt_id.as_deref() != Some(response_pid)
        {
                ...
                && crate::app::acp_handler::is_server_initiated_prompt(response_pid)
            {
                ...
                // Retire its optimistic echo so a later `chutes.ai/queue/changed` broadcast can't re-pin a stale placeholder and reorder the queue
        ...
        if let Some((kind, body)) = notification {
            ...
            if kind != NotificationEventKind::TurnComplete || queue_empty {
                ...
                let notif_title = session_name
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| "Chutes Build".into());
