...
fn cancel_agent_turn(
    ...
    // Its `chutes.ai/queue/changed` rebroadcast (carrying `running_prompt_id`) is the source of truth
...
/// Grace window between a driver-side `chutes.ai/session/prompt_complete` broadcast and that turn's `session/prompt` RPC response.
...
/// Finish turns whose end was announced by `chutes.ai/session/prompt_complete` but whose `session/prompt` RPC response never arrived.
///
