...
pub(crate) fn commit_feedback(
    ...
    agent.scrollback.push_block(RenderBlock::system(
        if displaced {
            ...
            "Thanks for the feedback! The Chutes Build team is on it."
        }
        .to_string(),
    ));
...
/// Send a raw remember note for LLM-powered rewriting via `chutes.ai/memory/rewrite`.
...
/// Bypasses the prompt queue, so it works even while the agent is mid-turn.
/// Fires the `chutes.ai/recap` ext method; the recap arrives asynchronously as a `SessionRecap` notification (rendered in scrollback).
///
...
pub(super) fn dispatch_send_recap(app: &mut AppView, auto: bool) -> Vec<Effect> {
    ...
    // Skip client requests entirely when the feature is off so we never hit `chutes.ai/recap`
