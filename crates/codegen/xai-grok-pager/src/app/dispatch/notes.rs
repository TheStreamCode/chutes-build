...
pub(crate) const FEEDBACK_QUESTION_LABEL: &str = "How can we improve Chutes Build?";
...
/// agent is mid-turn. Fires the `chutes.ai/recap` ext method; the recap arrives
...
pub(super) fn dispatch_send_recap(app: &mut AppView, auto: bool) -> Vec<Effect> {
    ...
    // entirely when the feature is off so we never hit `chutes.ai/recap`.
