...
/// immediately, before the confirming `chutes.build/queue/changed` broadcast.
...
/// that bridges the round-trip before the confirming `chutes.build/queue/changed`
...
pub(crate) fn apply_turn_start_shim(
    ...
    // seen ring, so a buffer-replayed `chutes.build/follow_ups` for an older response
...
mod tests {
    ...
    /// `chutes.build/follow_ups` for a PRIOR turn's response stays rejected (its
