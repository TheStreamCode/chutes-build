...
/// The queue pane then renders it immediately, before the confirming `chutes.ai/queue/changed` broadcast.
...
/// The only client-side queue state is the optimistic echo that bridges the round-trip before the confirming `chutes.ai/queue/changed` broadcast.
...
/// Goes out as `chutes.ai/interject`, never send-now (cancel semantics).
///
...
pub(crate) fn apply_turn_start_shim(
    ...
    // A buffer-replayed `chutes.ai/follow_ups` for an older response then stays rejected (no stale revival)
...
pub(super) fn dispatch_run_edited_queued_command(
    ...
    let sends = match gate {
        EditedCommandGate::Run { session_id } => {
            ...
            match server {
                // Server rows are never mutated client-side; the `chutes.ai/queue/changed` rebroadcast is the visual result
...
mod tests {
    ...
    /// Via the shim: after starting a new turn, a buffer-replayed `chutes.ai/follow_ups` for a prior turn's response stays rejected (no stale revival).
