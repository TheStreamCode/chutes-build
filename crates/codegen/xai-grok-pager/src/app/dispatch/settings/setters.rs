...
/// The `CHUTES_BUILD_PROMPT_SUGGESTIONS` env var overrides the effective value.
...
pub(in crate::app::dispatch) fn set_default_model_inner(
    ...
    // Without this mirror, that new session (and the welcome card it commits) would show the previous default until the next `chutes.ai/models/update` roundtrip
