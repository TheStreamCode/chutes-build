...
/// `dispatch(Action::SetTheme("chutesday"), &mut app)` emits exactly one `Effect::PersistSetting` and mutates `app.current_ui.theme`.
/// It also fires a toast and toggles AUTO_MODE off (kind is concrete).
///
/// The test persists `chutesday` (a non-truecolor theme) because the asserted payload is the registry's CANONICAL, not the live theme cache.
/// `clamp_to_terminal` might fold the cache to ChutesNight in non-truecolor test environments.
...
/// Uses `chutesday` to avoid the `clamp_to_terminal` ambiguity in non-truecolor test envs.
...
///
/// Scenario: `theme="chutesnight"` (concrete) while the system is Dark, and the user commits `auto_dark_theme="chutesday"`.
/// The setting is dormant (parent theme is concrete, not auto), so the live display must stay on ChutesNight.
/// Uses `chutesday` to avoid `clamp_to_terminal` ambiguity in non-truecolor envs.
#[test]
...
///
/// Uses `ChutesDay` (non-truecolor-requiring) for the dark-mode fixture: the test environment may not report truecolor support.
/// `Theme::apply_kind` clamps truecolor-only themes (TokyoNight, RosePineMoon) to ChutesNight, so a non-truecolor theme avoids the clamp.
...
/// Uses `chutesnight` for the auto_dark_theme value to avoid `clamp_to_terminal` ambiguity.
/// We want a concrete kind that's clearly different from ChutesDay, the active resolved theme.
#[test]
...
/// Uses a non-truecolor theme (`chutesnight`) for the same clamp reason as the dark variant.
#[test]
...
///
/// Uses the non-truecolor themes `chutesday` and `chutesnight` to avoid the `clamp_to_terminal` interaction in test envs without truecolor support.
#[test]
