...
fn dispatch_open_settings_focus_reopens_when_already_open() {
    ...
    let _ = dispatch(Action::OpenSettingsFocus { key: "scroll_mode" }, &mut app);
    let agent = app.agents.get(&AgentId(0)).unwrap();
...
/// Chooser when editable, browse row when locked — and `coding_data_sharing`
/// is locked in every state, because Chutes Build has no such control to offer
/// (`CodingDataSharingLock::ProductPolicy`). Upstream's matrix over ZDR and team
/// role is kept, asserting that none of those states can unlock it: the floor is
/// not a lock a team admin can lift.
#[test]
fn dispatch_open_settings_focus_skips_the_chooser_only_when_locked() {
    ...
    assert!(
        matches!(
            open_focused(&mut app, "scroll_mode"),
            SettingsModalMode::PickingEnum { .. }
        ),
        "an editable setting opens its chooser"
    );
    for (label, prepare) in [
        ("default", (|_: &mut AppView| {}) as fn(&mut AppView)),
        ("ZDR", |app: &mut AppView| app.is_zdr = true),
        ("team member", |app: &mut AppView| {
            app.team_name = Some("acme".to_string());
            app.team_role = Some("member".to_string());
        }),
        ("team admin", |app: &mut AppView| {
            app.team_name = Some("acme".to_string());
        ...
        assert!(
            ...
            "{label}: the coding-data row must stop at the row that says why"
        );
    }
}
...
fn dispatch_open_settings_focus_sets_close_on_picker_exit_when_chooser_opens() {
    ...
    // The locked arm needs a locked row, and `coding_data_sharing` is locked
    // outright here — no ZDR or team state to arrange.
    let mut app = test_app_with_agent();
    let _ = dispatch(
...
/// Full path: a deep-link focus open → Enter commits and dismisses. Upstream
/// drove this through `coding_data_sharing`, which no longer enters a chooser
/// (`CodingDataSharingLock::ProductPolicy`); the mechanic under test is the
/// deep-link commit, so any editable enum row exercises it.
#[test]
fn open_settings_focus_enter_closes_settings_modal() {
    ...
    assert!(
        matches!(
            outcome,
            InputOutcome::Action(_) | InputOutcome::ActionPair(_, _)
        ),
        "deep-link Enter must commit the focused setting, got {outcome:?}"
    );
}
...
fn open_settings_enter_picker_esc_stays_open_in_browse() {
    ...
    let _ = dispatch(Action::OpenSettings, &mut app);
    {
        ...
        assert!(state.focus_key("scroll_mode"));
...
fn clear_default_model_persists_but_keeps_live_current() {
    ...
    let info = acp::ModelInfo::new(id.clone(), "Chutes Build Test".to_string());
...
fn set_default_model_idempotent_when_already_current() {
    ...
    let info = acp::ModelInfo::new(id.clone(), "Chutes Build Already".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
/// `clamp_to_terminal` might fold to ChutesNight in non-truecolor
...
fn set_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        ...
        let effects = dispatch(Action::SetTheme("chutesday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*key, "theme");
                assert_eq!(*value, SettingValue::Enum("chutesday"));
                assert_eq!(*rollback_value, SettingValue::Enum("chutesnight"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(app.current_ui.theme.as_deref(), Some("chutesday"));
        assert!(
...
/// for the auto-dark sibling. Uses `chutesday` to avoid the
...
fn set_auto_dark_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoDarkTheme("chutesday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
        ...
        assert_eq!(app.current_ui.auto_dark_theme.as_deref(), Some("chutesday"));
    });
}
#[test]
fn set_auto_light_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoLightTheme("chutesnight".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*value, SettingValue::Enum("chutesnight"));
                assert_eq!(*rollback_value, SettingValue::Enum("chutesday"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(
...
///
/// Scenario: `theme="chutesnight"` (concrete) + system=Dark. User
/// commits `auto_dark_theme="chutesday"`. The setting is dormant
/// (parent theme is concrete, not auto), so the live display
/// must stay on ChutesNight even though we're committing a
/// different theme. Uses `chutesday` to avoid `clamp_to_terminal`
...
fn set_auto_dark_theme_does_not_apply_when_theme_is_not_auto() {
    with_theme_test_env(|| {
        ...
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("chutesnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::ChutesNight,
        );
        let _ = dispatch(Action::SetAutoDarkTheme("chutesday".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::ChutesNight,
...
///
/// Uses `ChutesDay` (non-truecolor-requiring) for the dark-mode
...
/// ChutesNight. Using a non-truecolor theme avoids the clamp
...
fn set_auto_dark_theme_applies_when_theme_is_auto_and_system_is_dark() {
    with_theme_test_env(|| {
        ...
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::ChutesDay,
...
/// Uses `chutesnight` for the auto_dark_theme
/// value to avoid `clamp_to_terminal` ambiguity (we want a
/// concrete kind that's clearly different from ChutesDay, the
...
fn set_auto_dark_theme_does_not_apply_when_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoDarkTheme("chutesnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::ChutesDay,
...
/// (`chutesnight`) for the same clamp reason as the dark variant.
#[test]
fn set_auto_light_theme_applies_when_theme_is_auto_and_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoLightTheme("chutesnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::ChutesNight,
...
fn set_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("chutesday".into()), &mut app);
        let toast = read_toast(&app);
        assert!(
        ...
        assert!(
            toast.contains("Chutes Day"),
            "toast must use display name `Chutes Day`, not canonical `chutesday`, got: {toast:?}",
        );
...
fn set_auto_dark_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Chutes Day"));
        assert!(toast.contains('\u{2713}'));
    });
}
#[test]
fn set_auto_light_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Chutes Night"));
    });
}
...
///
/// Uses non-truecolor themes (`chutesday` ↔ `chutesnight`) to avoid
...
fn rollback_theme_reverts_current_ui_and_cache() {
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                key: "theme",
                rollback_value: SettingValue::Enum("chutesnight"),
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                ...
                rollback_value: SettingValue::Enum("chutesday"),
                error: "disk full".into(),
            }),
            &mut app,
        );
