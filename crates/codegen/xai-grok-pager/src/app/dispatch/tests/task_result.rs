...
fn doctor_planning_opens_refuses_remote_and_rejects_stale_identity() {
    ...
    dispatch_task_result(
        TaskResult::DoctorFixPlanned {
            ...
            result: Ok(crate::app::actions::DoctorPlanningOutcome::RunLocally(
                "chutes-build doctor fix ssh-wrap".to_owned(),
            )),
        },
        &mut app,
    );
    assert!(
        last_system_text(&app, id)
            .contains("On your local computer, run: chutes-build doctor fix ssh-wrap")
    );
...
fn stale_auth_copy_timeout_does_not_clear_newer_feedback() {
    ...
    app.auth_state = AuthState::Authenticating {
        request_seq: 1,
        handle: None,
        auth_url: Some("https://chutes.ai/auth".to_owned()),
...
fn wrap_host_image_request_eligible_covers_full_miss_and_attachment_error_only() {
    ...
    // That request is how `chutes-build wrap` pastes images over headless SSH
...
/// Regression: a failed `chutes.ai/subagent/cancel` RPC must NOT finalize the row; the subagent may still be running.
...
fn switch_to_non_reasoning_model_clears_persisted_effort() {
    ...
    app.agents
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "Chutes Build".to_string()),
        );
    app.agents
        .get_mut(&id)
        .unwrap()
...
fn same_agent_type_switch_no_modal() {
    ...
    agent.session.models.available.insert(
        model_a.clone(),
        acp::ModelInfo::new(model_a.clone(), "Chutes Build A".to_string()),
    );
    agent.session.models.set_current(model_a, None);
    agent.session.models.available.insert(
        model_b.clone(),
        acp::ModelInfo::new(model_b.clone(), "Chutes Build B".to_string()),
    );
    agent.session.model_switch_pending = true;
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    app.gate = Some(xai_grok_shell::auth::GateInfo {
        ...
        url: Some("https://chutes.ai/supergrok".into()),
