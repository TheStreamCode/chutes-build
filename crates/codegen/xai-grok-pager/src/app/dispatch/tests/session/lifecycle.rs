...
fn worktree_session_preserves_subdirectory_offset() {
    ...
    let id = AgentId(0);
    let worktree_root = PathBuf::from("/home/user/.chutes-build/worktrees/repo/pager-123");
...
/// The test is CHUTES_BUILD_HOME-isolated so the write hits a temp store, not the real one.
...
mod welcome_workspace_mode {
    ...
    fn welcome_new_session_sets_own_override() {
        let _ack = xai_grok_test_support::EnvGuard::set(
            crate::app::session_startup::CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ACK_ENV,
            "1",
        );
