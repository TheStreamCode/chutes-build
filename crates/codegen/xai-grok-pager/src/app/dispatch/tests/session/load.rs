...
fn load_session_marks_standalone_worktree_cwd() {
    ...
    assert!(
        ...
        "resume into a standalone chutes-build worktree must set session.is_worktree"
    );
...
/// The fast foreign fetch has landed (its rows hidden behind the Chutes Build-default CTA) while the native fetch is still in flight.
