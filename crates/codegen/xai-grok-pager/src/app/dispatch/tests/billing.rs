...
fn upsell_non_max_upgrade_url_is_supergrok() {
    ...
    assert!(url.contains("referrer=chutes-build"), "got: {url}");
}
...
/// The opener failure is simulated via a broken `CHUTES_BUILD_TEST_OPEN_URL_FILE` path.
...
fn open_url_shows_manual_url_when_browser_unavailable() {
    // Point `CHUTES_BUILD_TEST_OPEN_URL_FILE` at a path whose parent dir does not exist so the write fails and `open_url` returns false (BrowserUnavailable)
...
/// A successful open (the `CHUTES_BUILD_TEST_OPEN_URL_FILE` write succeeds) must not spam a fallback system message.
#[serial_test::serial(CHUTES_BUILD_TEST_OPEN_URL_FILE)]
#[test]
