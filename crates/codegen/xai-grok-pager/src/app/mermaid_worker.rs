...
thread_local! {
    ...
    /// `subagent::REPLAY_CHUTES_BUILD_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `CHUTES_BUILD_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    /// hermetic, with no process-global `CHUTES_BUILD_HOME` mutation. The `TempDir` lives
