...
thread_local! {
    ...
    /// `CHUTES_BUILD_HOME` (whose `grok_home()` value is cached first-write-wins, an
    /// isolation hazard under the full parallel suite; PNGs could land in the
    /// real `~/.chutes-build`). Thread-local, so each parallel test is independent; the
    /// `TempDir` guard lives here so the dir outlives the view. Mirrors the
    /// `subagent::REPLAY_CHUTES_BUILD_HOME` test override. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test override: a hermetic per-test cache dir (no `CHUTES_BUILD_HOME` mutation)
        #[cfg(test)]
...
mod tests {
    ...
    /// Point this test's session `mermaid/` cache dir at a private tempdir: hermetic, with no process-global `CHUTES_BUILD_HOME` mutation.
    ...
    fn mermaid_is_rendering_matches_by_source_across_theme_width_change() {
        ...
        let click_key =
            MermaidCacheKey::derive(src, ThemeKind::ChutesNight, 80, MermaidRenderQuality::Open);
        let mut rt = MermaidRuntime::new();
        rt.pending.push(PendingMermaidAction {
            key: click_key.clone(),
            action: MermaidClickAction::Open,
        ...
        let live_key =
            MermaidCacheKey::derive(src, ThemeKind::ChutesDay, 240, MermaidRenderQuality::Open);
        assert_ne!(
            click_key, live_key,
