...
pub enum AgentCommand {
    ...
    /// Drives the spinner shown on the placeholder agent while the `chutes.ai/session/fork` request is in flight.
    ForkSession,
}
impl AgentCommand {
...
pub struct AgentSession {
    ...
    /// Prompt history for the current session, fetched from ACP (`chutes.ai/prompt_history` scoped via `filter_session_id`). Most-recent-first.
    /// Fetched on session create/load; prompts sent in this session are additionally front-inserted locally on send.
    pub prompt_history: Vec<String>,
    /// True until the session's startup/load `chutes.ai/prompt_history` fetch completes.
