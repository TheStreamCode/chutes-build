...
pub(crate) fn title_miss_hint(arg: &str) -> String {
    format!(
        "no session id or title matched {arg:?} for this directory; \
         try `chutes-build sessions search {arg:?}`"
    )
}
        ...
        _ => {
            ...
            anyhow::bail!(
                "Multiple sessions match title {:?}:\n{listing}\n\
                 Resume by session id instead: chutes-build --resume <session-id>",
                arg.trim()
            );
        }
    }
...
pub(crate) fn presandbox_resume_target(
    ...
    let summaries = xai_grok_shell::session::persistence::local_summaries_for_cwd_sync(
        cwd,
        xai_grok_shell::session::persistence::RecentSessionSelection::Any,
    )
    .map_err(|e| {
