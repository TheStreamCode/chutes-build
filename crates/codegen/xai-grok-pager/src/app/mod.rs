...
/// Whether the opt-in mouse-reporting toggle feature is enabled (`[ui] mouse_reporting_toggle` / `CHUTES_BUILD_MOUSE_REPORTING_TOGGLE`).
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  chutes-build --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  chutes-build --resume {}", info.session_id);
    }
}
...
mod tests {
    ...
    fn print_exit_resume_hint_includes_minimal_flag() {
        ...
        assert_eq!(
            String::from_utf8(buf).unwrap(),
            "\nResume this session with:\n  chutes-build --minimal --resume sess-abc\n"
        );
    }
    #[test]
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  chutes-build --resume sess-abc\n"));
    }
    #[test]
