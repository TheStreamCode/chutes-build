...
pub(super) fn render(failure: &StartupFailure) -> String {
    ...
    let mut report = match &failure.reason {
        Reason::TimedOut { waited, timings } => {
            ...
            format!(
                "Couldn't start Chutes Build: startup timed out after {}.\n\n{explanation}",
...
impl Advice {
    fn explanation(&self) -> String {
        ...
        if let Some(earlier) = self.earlier {
            ...
            let _ = write!(
                explanation,
                " Chutes Build spent the first {} on the {target}.",
...
impl NextStep {
    fn text(self) -> &'static str {
        match self {
            Self::Retry => "Start Chutes Build again.",
            Self::CheckNetworkThenRetry => {
                "Check your network connection, then start Chutes Build again."
            }
            Self::RestartSharedLeader => {
                "Stop it with the command below, which also stops any other Chutes Build \
                 session using it, then start Chutes Build again."
            }
        }
    }
    ...
    fn command(self) -> Option<&'static str> {
        match self {
            ...
            Self::RestartSharedLeader => Some("chutes-build leader kill"),
        }
    }
}
