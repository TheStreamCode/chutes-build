...
/// Cap for the `CHUTES_BUILD_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `CHUTES_BUILD_ESC_DOUBLE_PRESS_MS`-overridable (read once, bounded).
...
pub struct AppView {
    ...
    /// Release-safe FPS HUD (`/debug fps`; `CHUTES_BUILD_FPS` env on release builds, where the dev overlay is compiled out); see the module doc.
    ...
    /// Env `CHUTES_BUILD_PLUGIN_CTA` overrides `RemoteSettings.plugin_cta` (remote settings); defaults to `false`.
    ...
    /// The Agent Dashboard entry points (`/dashboard`, `Ctrl+\`, `chutes-build dashboard`, the startup hook) are gated on this flag.
    ...
    /// Populated from `chutes.ai/sessions/list` polls and `chutes.ai/sessions/changed` broadcasts.
    ...
    /// We fetch the same `chutes.ai/session/list` the resume picker uses and render those as idle rows.
    ...
    /// Reconciled from `chutes.ai/queue/changed` broadcasts so every client renders the same ordered queue (including prompts queued by other clients).
    ...
    /// The confirming `chutes.ai/queue/changed` broadcast has not yet arrived. Keyed by `sessionId`.
    ...
    /// Gated by `CHUTES_BUILD_SESSION_PICKER_GROUPED` env var or remote settings `session_picker_grouped`; defaults to `false`.
    ...
    /// Gated by `CHUTES_BUILD_CANCEL_REWIND` env / `[features] cancel_rewind` config / remote settings flag.
    ...
    /// Stamps `_meta["chutes.ai/session"].kind = "chat"` and omits Build agent profiles on create/load while set.
    ...
    /// Precedence: `CHUTES_BUILD_CONTEXTUAL_HINTS` (master) > `[ui.contextual_hints]` user config > remote tier > default.
    ...
    /// One-shot gate for the SSH `chutes-build wrap` tip: set after the first evaluation at a stable agent-view draw.
    ...
    /// Loaded once on startup from `~/.chutes-build/config.toml`.
    ...
    /// When false (remote kill switch or `CHUTES_BUILD_VOICE_MODE=0`) the STT pipeline is not started and session voice mode cannot turn on.
...
impl AppView {
    ...
    /// Mirrors [`Self::apply_voice_mode_enabled`].
    ///
    /// Called from [`Self::apply_auth_meta`] (startup / login) and from the `chutes.ai/settings/update` handler when the subscription tier changes.
    ...
    /// Reconcile the shared prompt queue for a session from a `chutes.ai/queue/changed` broadcast.
    ...
    /// The row is keyed by `prompt_id` so the authoritative `chutes.ai/queue/changed` broadcast replaces it (matched by `id`) rather than duplicating it.
    ...
    /// Rows the dev `CHUTES_BUILD_FPS` overlay occupies (0 in non-dev builds), so runtime debug overlays stack below instead of overpainting it.
...
impl AppView {
    ...
    /// One-shot SSH `chutes-build wrap` tip trigger, run at the top of every `draw` right after [`Self::maybe_trigger_small_screen_tip`].
