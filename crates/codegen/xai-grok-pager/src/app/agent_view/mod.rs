...
impl McpInitProgress {
    ...
    ///   `chutes.ai/mcp_initialized` clears the progress.
...
/// (`chutes.ai/follow_ups`). Streaming-only: never persisted, does not survive a
...
pub struct AgentView {
    ...
    /// (optimistic echo). The shell broadcasts `chutes.ai/session/interjection` to
    ...
    /// MCP servers, cleared when `chutes.ai/mcp_initialized` arrives.
    ...
    /// Running agent definition for this session (`chutes.ai/session/info` `agentName`).
    ...
    /// refreshed by `chutes.ai/settings/update`: the fire side is pinned for the
    ...
    /// Armed when an `chutes.ai/session/prompt_complete` broadcast arrives for the
    ...
    /// `chutes.ai/queue/changed` broadcast has confirmed the row yet. Inserted by
    ...
    /// optimistic echo. Firing `chutes.ai/queue/interject` then would race the
    ...
    /// Parked here and fired from the confirming `chutes.ai/queue/changed`
    ...
    /// Follow-up suggestion chips for the latest assistant response
    /// (`chutes.ai/follow_ups`). `None` when no chips are shown. Set by
    ...
    /// Stamped `chutes.ai/follow_ups` that arrived for a turn that is NOT yet the
...
pub(crate) mod test_fixtures {
    ...
    /// After adopting a NEW turn, a buffer-replayed `chutes.ai/follow_ups`
    ...
    /// Stamped path: a LATE FIRST-TIME (never-seen) `chutes.ai/follow_ups`
    ...
    /// upstream by the `_meta["chutes.ai/replayed"]` gate); it is pinned here so the
    ...
    /// Buffer-before-adoption: a stamped `chutes.ai/follow_ups` for a turn that
