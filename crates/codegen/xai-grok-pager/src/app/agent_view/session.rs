...
impl AgentView {
    ...
    /// Adopt the in-flight turn another client is driving, conveyed by the `session/load` response meta (`chutes.ai/runningPromptId`).
    ...
    /// ACP `kind` for `chutes.ai/session/rename`: which list (Chat or Build) this session opened on.
