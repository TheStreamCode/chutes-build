...
impl AgentView {
    ...
    ///
    /// The browse reads that list alone, and the `chutes.ai/prompt_history` fetch delivers an empty list when it fails.
    ...
    /// The pty-only `CHUTES_BUILD_ESC_DOUBLE_PRESS_MS` override can exceed this; no pty case mashes Esc across a cancel.
...
mod prompt_suggestion_key_tests {
    ...
    /// Idle agent with the gate open and a loaded suggestion: the state right after a turn ends with `chutes.ai/suggestPrompt` resolved.
