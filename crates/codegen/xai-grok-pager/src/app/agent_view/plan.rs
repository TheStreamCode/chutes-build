...
mod plan_approval_enter_tests {
    ...
    #[test]
    fn empty_enter_with_pending_comments_requests_changes() {
        let mut agent = agent_with_revise_prompt();
    ...
    fn a_on_empty_revise_prompt_approves() {
        let mut agent = agent_with_revise_prompt();
        let outcome =
            agent.handle_plan_feedback_key(&KeyEvent::new(KeyCode::Char('a'), KeyModifiers::NONE));
        assert!(matches!(outcome, InputOutcome::Changed));
        assert!(agent.plan_approval_view.is_none());
        assert_ne!(
            agent.toast.as_ref().map(|(msg, _)| msg.as_str()),
    ...
    #[test]
    fn a_with_pending_comments_is_typed_instead_of_approving() {
        let mut agent = agent_with_revise_prompt();
        if let Some(ref mut pav) = agent.plan_approval_view {
            pav.comments.push(PlanComment {
                id: 1,
