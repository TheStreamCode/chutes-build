...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain Chutes Build session whose first prompt resumes a foreign tool session.
    ForeignResume {
...
/// Build `chutes.ai/session/fork` params shared by TUI effects and headless.
///
...
/// Parse `newSessionId` from an `chutes.ai/session/fork` ACP response body.
...
/// Mode defaults to `own` unless `CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_MODE` or an attach server id is set.
#[cfg(feature = "local-workspace")]
pub const CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ENV: &str = "CHUTES_BUILD_CHAT_LOCAL_WORKSPACE";
#[cfg(feature = "local-workspace")]
pub const CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_CWD_ENV: &str = "CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_CWD";
#[cfg(feature = "local-workspace")]
pub const CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_MODE_ENV: &str =
    "CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_MODE";
#[cfg(feature = "local-workspace")]
pub const CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV: &str =
    "CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_SERVER_ID";
#[cfg(feature = "local-workspace")]
pub const CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV: &str =
    "CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ALLOW_HOME";
/// Skip interactive first-run confirm (still prints the banner).
#[cfg(feature = "local-workspace")]
pub const CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ACK_ENV: &str = "CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ACK";
...
pub const LOCAL_WORKSPACE_ATTACH_NEEDS_SERVER_ID: &str = "local-workspace attach requires --local-workspace-attach=<server_id> \
     (or CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_SERVER_ID)";
#[cfg(feature = "local-workspace")]
pub const LOCAL_WORKSPACE_REQUIRES_CHAT: &str = "local-workspace flags/env require --chat";
#[cfg(feature = "local-workspace")]
pub const LOCAL_WORKSPACE_HOME_DENIED: &str = "local-workspace cwd may not be / or $HOME unless CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ALLOW_HOME=1";
#[cfg(feature = "local-workspace")]
...
#[cfg(feature = "local-workspace")]
pub const LOCAL_WORKSPACE_ACK_REQUIRED: &str = "local-workspace requires interactive confirm, CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ACK=1, or an ack file";
/// Declared advertised tool ids for attach FS-only check (comma-separated).
#[cfg(feature = "local-workspace")]
pub const CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV: &str =
    "CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS";
#[cfg(feature = "local-workspace")]
...
pub fn resolve_local_workspace_config(
    ...
    let env_enable = env_truthy(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ENV);
    let env_mode = env_nonempty(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_MODE_ENV);
    let env_server_id = env_nonempty(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    let env_cwd =
        env_nonempty(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_CWD_ENV).map(std::path::PathBuf::from);
    ...
    let mode = if cli_attach.is_some() {
        ...
        match m.as_str() {
            ...
            other => {
                anyhow::bail!(
                    "invalid {CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_MODE_ENV}={other:?}; expected own|attach"
                )
            }
        }
...
pub fn validate_local_workspace_cwd(path: &std::path::Path) -> anyhow::Result<std::path::PathBuf> {
    ...
    if env_truthy(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV) {
        return Ok(canon);
    }
...
///
/// Skip confirm only with `CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ACK=1` or a prior ack file.
...
pub fn local_workspace_ack_satisfied() -> bool {
    if env_truthy(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ACK_ENV) {
        return true;
    }
...
/// via `CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS` (comma-separated ids).
...
pub fn probe_advertised_tool_ids() -> Option<Vec<String>> {
    let raw = env_nonempty(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV)?;
...
pub enum MaterializedStartup {
    ...
    Resume {
        ...
        /// Pre-TUI conversation-only remote restore: the follow-up `LoadSession` must send `chutes.ai/restore_code: false`.
...
mod tests {
    ...
    fn advertised_tools_env() -> xai_grok_test_support::EnvGuard {
        xai_grok_test_support::EnvGuard::set(
            CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV,
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[test]
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[serial_test::serial(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_SERVER_ID)]
    #[test]
    fn resolve_local_workspace_empty_cli_attach_falls_back_to_env() {
        let _env = advertised_tools_env();
        let _sid = xai_grok_test_support::EnvGuard::set(
            CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV,
            "srv-from-env",
        );
        let tmp = tempfile::tempdir().unwrap();
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_CWD)]
    #[serial_test::serial(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE)]
    #[serial_test::serial(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_MODE)]
    #[serial_test::serial(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_SERVER_ID)]
    #[test]
    fn resolve_local_workspace_cwd_only_is_not_a_request() {
        let tmp = tempfile::tempdir().unwrap();
        let _cwd = xai_grok_test_support::EnvGuard::set(
            CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_CWD_ENV,
            tmp.path().to_str().unwrap(),
        );
        let _enable = xai_grok_test_support::EnvGuard::unset(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ENV);
        let _mode =
            xai_grok_test_support::EnvGuard::unset(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid =
            xai_grok_test_support::EnvGuard::unset(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    fn resolve_local_workspace_own_env_defaults() {
        let _env = advertised_tools_env();
        let _enable =
            xai_grok_test_support::EnvGuard::set(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ENV, "1");
        let _mode =
            xai_grok_test_support::EnvGuard::unset(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid =
            xai_grok_test_support::EnvGuard::unset(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS)]
    #[serial_test::serial(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ALLOW_HOME)]
    ...
    fn resolve_local_workspace_defaults_cwd_and_denies_home() {
        ...
        let _allow = xai_grok_test_support::EnvGuard::unset(
            CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV,
        );
    ...
    #[cfg(feature = "local-workspace")]
    #[serial_test::serial(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ACK)]
    #[serial_test::serial(CHUTES_BUILD_HOME)]
    #[test]
    fn local_workspace_non_tty_requires_ack() {
        let _ack =
            xai_grok_test_support::EnvGuard::unset(CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ACK_ENV);
        let home = tempfile::tempdir().unwrap();
