...
/// unless `CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_MODE` / attach server id is set.
#[cfg(feature = "local-workspace")]
...
fn local_workspace_ack_path() -> Option<std::path::PathBuf> {
    let home = std::env::var("CHUTES_BUILD_HOME")
        .ok()
...
pub enum MaterializedStartup {
    ...
    Resume {
        ...
        /// must send `chutes.ai/restore_code: false` so agent `[cli] restore_code`
