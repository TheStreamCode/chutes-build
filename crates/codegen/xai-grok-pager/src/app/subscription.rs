...
/// Floor for the `CHUTES_BUILD_SUBSCRIPTION_WATCH_INTERVAL_SECS` env override (for tests and power users; deliberately below the server floor).
...
impl AppView {
    ...
    pub fn subscription_watch_interval(&self) -> Option<std::time::Duration> {
        if let Ok(v) = std::env::var("CHUTES_BUILD_SUBSCRIPTION_WATCH_INTERVAL_SECS")
