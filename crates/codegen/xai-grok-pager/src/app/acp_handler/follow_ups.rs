...
/// Deserialize shape of the `chutes.ai/follow_ups` params emitted by the shell translator: `{ response_id, suggestions: [{ label, .. }] }`.
...
pub(super) struct FollowUpsParams {
    ...
    /// Carries the reserved `"chutes.ai/replayed"` marker, which the shell never sets in v1.
...
/// A single `chutes.ai/follow_ups` suggestion.
...
/// Handle `chutes.ai/follow_ups`: render follow-up suggestion chips for the latest assistant response.
///
/// The keying that lets the newest response win lives in [`AgentView::apply_follow_ups`].
/// The reserved `_meta["chutes.ai/replayed"] == true` marker suppresses rendering (it is absent today and treated as optional).
...
pub(super) fn handle_follow_ups(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if params
        .meta
        .as_ref()
        .and_then(|m| m.get("chutes.ai/replayed"))
