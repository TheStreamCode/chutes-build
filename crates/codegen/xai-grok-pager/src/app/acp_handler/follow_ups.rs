...
pub(super) struct FollowUpsParams {
    ...
    /// change. Parsed loosely as a JSON value to read the `"chutes.build/replayed"`
...
/// reserved `_meta["chutes.build/replayed"] == true` marker suppresses rendering (it
...
pub(super) fn handle_follow_ups(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if params
        .meta
        .as_ref()
        .and_then(|m| m.get("chutes.build/replayed"))
