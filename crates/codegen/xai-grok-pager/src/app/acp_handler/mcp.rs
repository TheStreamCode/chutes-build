...
/// Default `true`; set `CHUTES_BUILD_MCP_PUSH_SERVER_STATUS=0` to disable.
...
pub(super) fn handle_mcp_tools_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    // Both `chutes.ai/mcp_initialized` and (newer shell) `chutes.ai/mcp/tools_changed` carry `sessionId`
    ...
    if method == "chutes.ai/mcp_initialized"
        ...
        match serde_json::from_value::<Vec<McpToolEntry>>(raw) {
            ...
            Err(e) => {
                tracing::warn!(
                    server = %payload.name,
                    error = %e,
                    "chutes.ai/mcp/server_status: tools field present but not Vec<McpToolEntry>; status still applied"
                );
                None
            }
        }
...
/// Handle `chutes.ai/mcp/elicit_complete`: dismiss the matched agent's URL-mode elicitation card that is still waiting on this `elicitation_id`.
