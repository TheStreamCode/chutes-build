...
/// `CHUTES_BUILD_MCP_PUSH_SERVER_STATUS=0` to disable.
...
pub(super) fn handle_mcp_tools_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if method == "chutes.build/mcp_initialized"
        ...
        match serde_json::from_value::<Vec<McpToolEntry>>(raw) {
            ...
            Err(e) => {
                tracing::warn!(
                    server = %payload.name,
                    error = %e,
                    "chutes.build/mcp/server_status: tools field present but not Vec<McpToolEntry>; status still applied"
                );
                None
            }
        }
