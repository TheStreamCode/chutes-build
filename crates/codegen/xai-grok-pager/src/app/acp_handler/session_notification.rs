...
/// Handle `chutes.ai/session_notification` and replay-path `chutes.ai/session/update`.
///
    ...
    let matched = match find_session_match(app, &session_notif.session_id) {
        ...
        None => {
            tracing::debug!(
                ...
                "load-race: chutes.ai/session_notification DROPPED — no agent matches session_id"
            );
            return false;
        }
    };
