...
/// Handle `chutes.build/session_notification` and replay-path `chutes.build/session/update`.
///
...
pub(super) fn handle_session_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let matched = match find_session_match(app, &session_notif.session_id) {
        ...
        None => {
            tracing::debug!(
                ...
                "load-race: chutes.build/session_notification DROPPED — no agent matches session_id"
            );
            return false;
        }
    };
        ...
        })
    {
        tracing::debug!(
            ...
            "chutes.build/session update DROPPED by dedup highwater (event_seq <= last_applied)"
        );
        return false;
    }
