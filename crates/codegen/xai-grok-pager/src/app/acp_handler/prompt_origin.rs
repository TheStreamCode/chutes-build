...
/// Unlike wake turns they run through `MvpAgent::prompt()` and emit the `chutes.ai/session/prompt_complete` turn-end signal.
...
/// That is safe only when the turn will emit a terminal `chutes.ai/session/prompt_complete`, the only non-interactive way a viewer leaves `TurnRunning`.
