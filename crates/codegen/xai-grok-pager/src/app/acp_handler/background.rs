...
/// Handle `chutes.ai/monitor_event`: a background task or monitor emitted new output.
...
pub(super) fn handle_scheduled_task_inject_prompt(
    ...
    let Some(session_id) = payload["sessionId"].as_str() else {
        tracing::warn!("chutes.ai/scheduled_task_inject_prompt: missing or non-string sessionId");
        return false;
    };
    let Some(prompt) = payload["prompt"].as_str().filter(|s| !s.is_empty()) else {
        tracing::warn!("chutes.ai/scheduled_task_inject_prompt: missing or empty prompt");
        return false;
    };
