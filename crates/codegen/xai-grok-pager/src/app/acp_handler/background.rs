...
/// Handle `chutes.build/task_backgrounded` — a bash command transitioned to background.
///
...
pub(super) fn handle_task_backgrounded(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let Ok(session_notif) = serde_json::from_str::<SessionNotification>(notif.params.get()) else {
        tracing::warn!("Failed to parse chutes.build/task_backgrounded");
        return false;
    };
...
/// Handle `chutes.build/monitor_event` — background task or monitor emitted new output.
...
pub(super) fn handle_scheduled_task_inject_prompt(
    ...
    let payload: serde_json::Value = match serde_json::from_str(notif.params.get()) {
        Ok(v) => v,
        Err(e) => {
            tracing::warn!(error = %e, "Failed to parse chutes.build/scheduled_task_inject_prompt");
            return false;
        }
    };
    let Some(session_id) = payload["sessionId"].as_str() else {
        tracing::warn!(
            "chutes.build/scheduled_task_inject_prompt: missing or non-string sessionId"
        );
        return false;
    };
    let Some(prompt) = payload["prompt"].as_str().filter(|s| !s.is_empty()) else {
        tracing::warn!("chutes.build/scheduled_task_inject_prompt: missing or empty prompt");
        return false;
    };
    ...
    // `chutes.build/scheduled_task_inject_prompt` notification is routed by the leader
...
pub(super) fn handle_task_completed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let Ok(session_notif) = serde_json::from_str::<SessionNotification>(notif.params.get()) else {
        tracing::warn!("Failed to parse chutes.build/task_completed");
        return false;
    };
