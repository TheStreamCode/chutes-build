...
/// Handle `chutes.ai/task_backgrounded` — a bash command transitioned to background.
///
...
pub(super) fn handle_task_backgrounded(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let Ok(session_notif) = serde_json::from_str::<SessionNotification>(notif.params.get()) else {
        tracing::warn!("Failed to parse chutes.ai/task_backgrounded");
        return false;
    };
...
/// Handle `chutes.ai/monitor_event` — background task or monitor emitted new output.
...
pub(super) fn handle_scheduled_task_inject_prompt(
    ...
    let payload: serde_json::Value = match serde_json::from_str(notif.params.get()) {
        Ok(v) => v,
        Err(e) => {
            tracing::warn!(error = %e, "Failed to parse chutes.ai/scheduled_task_inject_prompt");
            return false;
        }
    };
    ...
    // `chutes.ai/scheduled_task_inject_prompt` notification is routed by the leader
...
pub(super) fn handle_task_completed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let Ok(session_notif) = serde_json::from_str::<SessionNotification>(notif.params.get()) else {
        tracing::warn!("Failed to parse chutes.ai/task_completed");
        return false;
    };
