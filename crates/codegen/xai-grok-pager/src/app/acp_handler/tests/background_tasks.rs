#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// notifications through the generic `chutes.ai/session/update` envelope. A
    ...
    fn ext_session_update_replay_restores_bg_task() {
        ...
        handle(
            make_ext_session_notification_with_method("sess-1", "chutes.build/session/update", update),
            &mut app,
        );
    ...
    fn task_backgrounded_with_wire_description_still_drains_deferred_tool() {
        ...
        let notif = acp::ExtNotification::new("chutes.build/task_backgrounded", raw.into());
    ...
    fn completed_before_backgrounded_does_not_resurrect_running() {
        ...
        let raw = serde_json::value::to_raw_value(&notif).unwrap();
        let late = acp::ExtNotification::new("chutes.build/task_backgrounded", std::sync::Arc::from(raw));
