#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Regression (resume sync): the on-disk replay stream re-emits persisted notifications through the generic `chutes.ai/session/update` envelope.
    ...
    fn ext_session_update_replay_restores_bg_task() {
        ...
        handle(
            make_ext_session_notification_with_method("sess-1", "chutes.ai/session/update", update),
            &mut app,
        );
    ...
    fn task_backgrounded_with_wire_description_still_drains_deferred_tool() {
        ...
        let notif = acp::ExtNotification::new("chutes.ai/task_backgrounded", raw.into());
    ...
    fn completed_before_backgrounded_does_not_resurrect_running() {
        ...
        let raw = serde_json::value::to_raw_value(&notif).unwrap();
        let late = acp::ExtNotification::new("chutes.ai/task_backgrounded", std::sync::Arc::from(raw));
