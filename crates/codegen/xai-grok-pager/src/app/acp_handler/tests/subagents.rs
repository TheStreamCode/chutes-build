#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn ext_session_notification_and_update_equivalent_for_subagent_spawned() {
        ...
        let (spawn_notif, finish_notif) =
            run_subagent_lifecycle_via_method("chutes.build/session_notification", child_sid);
        let (spawn_update, finish_update) =
            run_subagent_lifecycle_via_method("chutes.build/session/update", child_sid);
    ...
    fn ext_session_update_malformed_params_returns_false() {
        ...
        let request = acp::ExtNotification::new("chutes.build/session/update", raw.into());
