#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    mod eviction_and_rebuild {
        ...
        fn child_compaction_started_line(child_sid: &str) -> String {
            format!(
                r#"{{"method":"_chutes.build/session/update","params":{{"sessionId":"{child_sid}","update":{{"sessionUpdate":"auto_compact_started","tokens_used":9000,"context_window":10000,"percentage":90,"reason":"threshold"}}}}}}"#
            )
        }

        fn child_compaction_completed_line(child_sid: &str) -> String {
            format!(
                r#"{{"method":"_chutes.build/session/update","params":{{"sessionId":"{child_sid}","update":{{"sessionUpdate":"auto_compact_completed","tokens_after":100,"elapsed_ms":5}}}}}}"#
            )
        }
    ...
    fn both_session_transports_produce_the_same_subagent_row() {
        ...
        let (spawn_notif, finish_notif) =
            run_subagent_lifecycle_via_method("chutes.ai/session_notification", child_sid);
        let (spawn_update, finish_update) =
            run_subagent_lifecycle_via_method("chutes.ai/session/update", child_sid);
    ...
    fn a_malformed_session_notification_is_ignored() {
        ...
        let request = acp::ExtNotification::new("chutes.ai/session/update", raw.into());
