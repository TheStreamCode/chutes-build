#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// The pager reconciles the authoritative shared prompt queue from the `chutes.ai/queue/changed` broadcast, and an empty broadcast clears it.
    #[test]
    ...
    fn queue_changed_adopts_running_prompt_id_only_when_unset() {
        ...
        let notif = acp::ExtNotification::new("chutes.ai/queue/changed", raw.into());
    ...
    /// The shell then broadcasts `chutes.ai/queue/changed` carrying that synthetic id as `runningPromptId`.
    ...
    fn viewer_does_not_enter_turn_running_for_server_initiated_turn() {
        // A server-initiated auto-wake turn runs inside the actor and emits NO `chutes.ai/session/prompt_complete`
    ...
    fn viewer_enters_turn_running_for_scheduler_fired_cron_turn() {
        ...
        // UNLIKE auto-wake turns it is client-driven via `MvpAgent::prompt()` and DOES emit `chutes.ai/session/prompt_complete`
    ...
    fn viewer_prompt_complete_finishes_turn() {
        // A viewer in TurnRunning receives chutes.ai/session/prompt_complete for its session and runs finish_turn: state Idle, current_prompt_id cleared
        let mut app = make_app_with_agent("sess-view");
        app.agents.get_mut(&AgentId(0)).unwrap().attached_as_viewer = true;
        let _ = handle(
