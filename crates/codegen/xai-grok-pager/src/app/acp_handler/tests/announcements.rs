#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.chutes-build` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.chutes-build` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.chutes-build` reads).
    #[test]
