...
/// Reads `CHUTES_BUILD_ENVRC_TIMEOUT_SECS` in this process; the agent inherits the same environment.
...
/// Run the `chutes.ai/mcp/list` read after a CTA install and map it into a `TaskResult::PluginCtaMcpsLoaded`.
...
/// CANONICAL wire parser for `LoadSessionResponse._meta["chutes.ai/runningPromptId"]`.
///
...
pub(crate) fn parse_session_load_running_prompt_id(
    ...
    resp_meta
        .and_then(|m| m.get("chutes.ai/runningPromptId"))
        .and_then(|v| v.as_str())
        .map(String::from)
}
...
/// `_meta["chutes.ai/session"].kind` is stamped `"chat"` so the shell takes the `require_gateway` / thin profile.
...
pub(crate) struct SessionFlags {
    ...
    /// Injected as `chutes.ai/restore_code` into `LoadSession` meta, or passed
...
impl SessionFlags {
    ...
    ///
    /// Returns `None` for the default `chutes-build` profile (no `_meta` needed; it already includes TaskTool).
    ...
    /// `--no-ask-user` always forces `askUserQuestion: false` into the meta, even when paired with `CHUTES_BUILD_AGENT`.
    /// The env var chooses the *agent*, but the tool-strip is independent.
    /// Chat mode additionally stamps `chutes.ai/session.kind`.
    pub(crate) fn to_meta(&self) -> Option<acp::Meta> {
        ...
        if self.chat_mode {
            meta.insert("chutes.ai/session".into(), serde_json::json!({ "kind": "chat" }));
            #[cfg(feature = "local-workspace")]
...
///
/// `chutes.ai/cloud_existing_workspace` is intentionally omitted: scrub keeps it only when `chutes.ai/local_workspace.mode == "attach"`.
#[allow(dead_code)]
pub(super) const CHAT_FORBIDDEN_WORKSPACE_BIND_KEYS: &[&str] = &[
    "envId",
    "chutes.ai/cloud_server_id",
];
...
pub(super) fn stamp_local_workspace_meta(
    ...
    meta.insert("chutes.ai/local_workspace".into(), serde_json::Value::Object(local));
    tracing::info!(
        ...
        "stamped chutes.ai/local_workspace onto session meta"
    );
    if cfg.mode == LocalWorkspaceMode::Attach && let Some(ref sid) = cfg.server_id {
        ...
        meta.insert(
            "chutes.ai/cloud_existing_workspace".into(),
...
///
/// Narrow scrub exception: keep `chutes.ai/cloud_existing_workspace` when local intent is **attach**.
/// Own stamps intent only (shell mints `server_id`).
/// Never keep `envId` or Direct hub `chutes.ai/cloud_server_id`.
pub(super) fn scrub_chat_workspace_bind_meta(meta: &mut Option<acp::Meta>) {
    ...
    #[cfg(feature = "local-workspace")]
    {
        let allow_existing_attach = obj
            .get("chutes.ai/local_workspace")
            .and_then(|v| v.get("mode"))
            .and_then(|m| m.as_str()) == Some("attach");
        if !allow_existing_attach {
            obj.remove("chutes.ai/cloud_existing_workspace");
        }
    }
    {
        obj.remove("chutes.ai/cloud_existing_workspace");
...
pub(crate) fn reject_non_fs_only_advertised_tools(
    ...
    let Some(ids) = advertised_tool_ids else {
        return Err(
            "operator attestation CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS is unset \
    ...
    if ids.is_empty() {
        return Err(
            "operator attestation CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS is empty \
    ...
    if forbidden.is_empty() {
        ...
        Err(
            ...
            "operator attestation lists tools outside the FS-only allowlist: {}. \
             ...
             CHUTES_BUILD_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS or restart workspace_server \
...
/// Degraded conversations lane on `chutes.ai/session/list`, parsed from the response's `_meta["chutes.ai/partial"]` envelope.
...
/// Read `_meta["chutes.ai/partial"]` from a session-list payload.
...
pub(super) fn parse_session_list_partial(
    ...
    let partial = payload.get("_meta")?.get("chutes.ai/partial")?;
...
/// Reads `_meta["chutes.ai/listScope"]` from a session-list payload.
pub(super) fn parse_session_list_scope(payload: &serde_json::Value) -> ListScope {
    match payload
        .get("_meta")
        .and_then(|m| m.get("chutes.ai/listScope"))
        .and_then(|v| v.as_str())
    {
...
/// Parse the `chutes.ai/session/list` response payload (the unwrapped `{ "sessions": [...] }` object) into [`SessionPickerEntry`] rows.
///
    ...
    entries
        ...
        .filter_map(|v| {
            ...
            let is_conversation = v
                .get("_meta")
                .and_then(|m| m.get("chutes.ai/session"))
...
/// Best-effort `chutes.ai/auth/cancel`: stops the shell's device/loopback wait so a later login is single-flight.
...
pub(super) async fn send_check_subscription(
    ...
    let req = acp::ExtRequest::new(
        "chutes.ai/auth/check_subscription",
        serde_json::value::to_raw_value(&serde_json::json!({}))
...
/// 2. Fire ACP `chutes.ai/yolo_mode_changed` (gated on disk success for `WithRollback`; always for `BestEffort`).
...
/// Whether to fire the ACP `chutes.ai/yolo_mode_changed` notification.
...
/// Extract the typed kill outcome from an `chutes.ai/task/kill` ext response.
///
...
/// Map an `chutes.ai/subagent/cancel` response (payload under `result`) to a kill outcome.
...
/// Fetch the user's auto top-up rule via the `chutes.ai/auto-topup-rule` extension.
...
/// Map an `chutes.ai/auto-topup-rule` payload to an [`AutoTopupFetch`].
