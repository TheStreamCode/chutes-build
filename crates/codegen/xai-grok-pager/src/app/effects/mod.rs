    ...
    match effect {
                    ...
                    match acp_send(request, &tx).await {
                        Ok(resp) => {
                            ...
                            match parsed {
                                ...
                                None => {
                                    tracing::warn!("failed to parse chutes.ai/sessions/list response");
        ...
        Effect::SetAdvisorModel(model) => {
            tasks.spawn(async move {
                let to_persist = if model.is_empty() {
                    None
                } else {
                    Some(model.clone())
                };
        ...
        Effect::ShowSessionInfo { agent_id, session_id, show_resolved_model, nonce } => {
            let is_api_key_auth = session_flags.is_api_key_auth;
            let api_key_env_set = xai_grok_shell::agent::auth_method::has_chutes_api_key_env();
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    match fetch_session_info(&session_id, &tx).await {
...
/// Fetch session info from ACP via `chutes.ai/session/info`.
...
fn format_auth_lines(is_api_key_auth: bool, api_key_env_set: bool) -> String {
    if is_api_key_auth {
        let method = if api_key_env_set {
            "  Auth method: API key (CHUTES_API_KEY)\n"
        } else {
            "  Auth method: API key\n"
        };
        return format!(
            "{method}  Run `chutes-build login` to use your SuperGrok subscription instead.\n"
        );
    }
