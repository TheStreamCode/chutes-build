    ...
    match effect {
        ...
        Effect::LoadSession { agent_id, session_id, session_cwd, chat_kind } => {
            ...
            if let Some(true) = session_flags.restore_code {
                meta.get_or_insert_with(acp::Meta::new)
                    .insert("chutes.build/restore_code".into(), serde_json::Value::Bool(true));
            }
                    ...
                    match acp_send(request, &tx).await {
                        Ok(resp) => {
                            ...
                            match parsed {
                                ...
                                None => {
                                    tracing::warn!(
                                        "failed to parse chutes.build/sessions/list response"
                                    );
...
/// Fetch session info from ACP via `chutes.build/session/info`.
...
/// Build the `chutes.build/interject` params. The optional structured `content`
