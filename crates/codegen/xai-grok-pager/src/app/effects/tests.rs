...
/// Text-only interjections must omit the `content` key entirely; the legacy `chutes.ai/interject` wire shape stays byte-identical.
#[test]
...
fn picker_keeps_conversation_with_empty_cwd_and_missing_updated_at() {
    let payload = serde_json::json!({
            "sessions": [{
                ...
                "source": "conversation",
                "_meta": { "chutes.ai/session": { "kind": "chat" } }
            }]
        });
...
fn session_list_partial_parses_reasons() {
    let payload = |reason: &str| {
        serde_json::json!({
                "sessions": [],
                "_meta": { "chutes.ai/partial": { "conversations": true, "reason": reason } }
            })
    };
    assert_eq!(
...
fn session_list_partial_absent_for_healthy_or_meta_less_responses() {
    let healthy = serde_json::json!({
            "sessions": [],
            "_meta": { "chutes.ai/partial": { "conversations": false } }
        });
...
/// Spawn a fake ACP agent that counts `chutes.ai/yolo_mode_changed` notifications.
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let xai_acp_lib::AcpAgentMessage::ExtNotification(args) = msg {
                if args.request.method.as_ref() == "chutes.ai/yolo_mode_changed" {
...
/// `FetchSessionList` wire shape: search sends `query` (no `allowRelax`); browse opts into `allowRelax` and parses `chutes.ai/listScope`.
...
async fn fetch_session_list_pushes_query_and_echoes_seq() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "chutes.ai/session/list");
                ...
                let body = if fail {
                    ...
                    serde_json::json!({
                            "result": {
                                "sessions": [],
                                "_meta": { "chutes.ai/listScope": "repo" },
                            }
                        })
                } else {
    ...
    match tasks.join_next().await.expect("task").expect("no panic") {
        TaskResult::SessionListLoaded { scope, seq, query, .. } => {
            ...
            assert!(
                    scope.is_relaxed(),
                    "_meta[\"chutes.ai/listScope\"] must parse into the task result"
                );
        }
        other => panic!("expected SessionListLoaded, got {other:?}"),
    }
...
async fn fetch_session_list_sends_kind_facet_filter() {
    ...
    assert_eq!(
            captured[0]["_meta"]["chutes.ai/facetFilters"]["kind"],
...
async fn fetch_workflows_list_sends_session_id() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "chutes.ai/workflows/list");
                let params: serde_json::Value = serde_json::from_str(
                        args.request.params.get(),
                    )
                    .expect("params JSON");
...
async fn deep_search_sessions_echoes_routing_and_policy() {
    ...
    let captured = captured.lock().unwrap();
    assert_eq!(captured.len(), 1);
    assert_eq!(captured[0].0, "chutes.ai/session/search");
...
/// Neutralize `CHUTES_BUILD_AGENT` for the profile-matrix tests below.
...
fn to_meta_chat_mode_stamps_kind_and_omits_agent_profile() {
    ...
    assert_eq!(meta["chutes.ai/session"]["kind"], "chat");
    assert!(
...
fn assert_chat_meta_has_no_workspace_bind_keys(meta: &serde_json::Value) {
    ...
    assert!(
            meta.get("chutes.ai/cloud_existing_workspace").is_none(),
...
fn chat_load_meta_never_includes_workspace_bind_keys() {
    ...
    apply_chat_kind_meta(&mut meta);
    {
        ...
        obj.insert("chutes.ai/cloud_server_id".into(), serde_json::json!("srv-poison"));
        obj.insert(
...
fn scrub_chat_workspace_matrix_attach_exception() {
    ...
    let mut meta = Some(acp::Meta::new());
    {
        ...
        obj.insert("chutes.ai/cloud_server_id".into(), serde_json::json!("hub-x"));
        obj.insert(
            "chutes.ai/cloud_existing_workspace".into(),
    ...
    assert!(scrubbed.get("chutes.ai/cloud_server_id").is_none());
    assert!(scrubbed.get("chutes.ai/cloud_existing_workspace").is_none());
    let mut meta = Some(acp::Meta::new());
    ...
    );
    {
        ...
        obj.insert("chutes.ai/cloud_server_id".into(), serde_json::json!("hub-must-go"));
    }
    scrub_chat_workspace_bind_meta(&mut meta);
    let scrubbed = meta.as_ref().unwrap();
    assert!(scrubbed.get("envId").is_none(), "envId must stay scrubbed");
    assert!(
            scrubbed.get("chutes.ai/cloud_server_id").is_none(),
            "Direct hub must stay scrubbed"
        );
    assert_eq!(
            scrubbed["chutes.ai/cloud_existing_workspace"]["server_id"],
            "srv-dogfood"
        );
    assert_eq!(scrubbed["chutes.ai/local_workspace"]["mode"], "attach");
    assert_eq!(scrubbed["chutes.ai/local_workspace"]["server_id"], "srv-dogfood");
    assert_eq!(scrubbed["chutes.ai/local_workspace"]["cwd"], "/tmp/repo");
}
#[cfg(feature = "local-workspace")]
#[test]
fn to_meta_chat_attach_stamps_local_and_existing() {
    ...
    assert_eq!(meta["chutes.ai/session"]["kind"], "chat");
    assert_eq!(meta["chutes.ai/local_workspace"]["mode"], "attach");
    assert_eq!(meta["chutes.ai/cloud_existing_workspace"]["server_id"], "srv-1");
    assert!(meta.get("envId").is_none());
    assert!(meta.get("chutes.ai/cloud_server_id").is_none());
}
#[cfg(feature = "local-workspace")]
#[test]
fn to_meta_chat_own_stamps_intent_without_existing() {
    ...
    let meta = flags.to_meta().expect("meta");
    assert_eq!(meta["chutes.ai/local_workspace"]["mode"], "own");
    assert_eq!(meta["chutes.ai/local_workspace"]["cwd"], "/tmp/repo-own");
    assert!(meta["chutes.ai/local_workspace"].get("server_id").is_none());
    assert!(
            meta.get("chutes.ai/cloud_existing_workspace").is_none(),
...
fn finalize_chat_session_meta_stamps_attach_on_worktree_path() {
    ...
    assert_eq!(meta["chutes.ai/session"]["kind"], "chat");
    assert_eq!(meta["chutes.ai/local_workspace"]["mode"], "attach");
    assert_eq!(meta["chutes.ai/cloud_existing_workspace"]["server_id"], "srv-wt");
    assert!(meta.get("envId").is_none());
}
#[test]
