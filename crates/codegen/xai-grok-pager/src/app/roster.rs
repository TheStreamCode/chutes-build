...
//!
//! - Request/response `chutes.ai/sessions/list` parses into [`RosterListResponse`].
//! - Broadcast notification `chutes.ai/sessions/changed` parses into [`RosterChanged`].
//!
...
mod tests {
    ...
    /// Round-trip for `chutes.ai/sessions/changed`: serialize the agent's `RosterChanged` as `emit_roster_changed` does (bare params, no envelope).
