//! `chutes-build mcp`: manage MCP server configurations from the command line.
...
Examples:
  ...
  chutes-build mcp add xcode -- xcrun mcpbridge
  ...
  # Add to the project config (./.chutes-build/config.toml) instead of ~/.chutes-build/config.toml
  chutes-build mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.chutes-build/config.toml`, available in all your projects
    User,
    /// `./.chutes-build/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Config to write to: user (~/.chutes-build/config.toml) or project (./.chutes-build/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `chutes-build mcp add --help` to get started.");
    } else {
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: chutes-build mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: chutes-build mcp add --transport {label} <name> <url>"
                );
            };
...
async fn run_set_enabled(name: &str, enabled: bool) -> Result<()> {
    ...
    if !mcp_server_is_known(name, &cwd) {
        ...
        if !available.is_empty() {
            ...
            eprintln!(
                "No MCP servers configured. Run `chutes-build mcp add --help` to get started."
            );
        }
        std::process::exit(1);
    }
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let (scope, path) = match select_remove_site(user_defined, find_project_site(), requested_scope)
    {
        ...
        Err(RemoveError::Ambiguous { project_path }) => {
            ...
            eprintln!(
                "Specify which one to remove, e.g.: chutes-build mcp remove {name} --scope project"
            );
            std::process::exit(1);
        }
    };
    ...
    // A scoped delete can leave the name defined in the other scope or an ancestor .chutes-build/config.toml, where it still resolves for sessions
...
mod tests {
    ...
    fn grok_com_known_only_with_toml_definition() {
        // Unique name: `grok_home()` is process-wide OnceLock, so CHUTES_BUILD_HOME
        // EnvGuard is a no-op if another test already resolved it. A leftover
        // `grok_com_*` in the real ~/.chutes-build disabled list would fail an orphan
        ...
        std::fs::create_dir_all(defined.path().join(".chutes-build")).unwrap();
        std::fs::write(
            defined
                .path()
                .join(".chutes-build")
...
"#
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        ...
        let project = PathBuf::from("/repo/.chutes-build/config.toml");
