...
///   - [`update_from_notification`]: the `chutes.ai/git_head_changed` ACP notification.
...
///
/// An explicit `CHUTES_BUILD_NERD_FONTS` override always wins: `0`/`false` means off, anything else means on.
...
pub(crate) fn home_dir() -> Option<String> {
    xai_dirs::home_dir().map(|home| home.to_string_lossy().into_owned())
}
...
mod tests {
    ...
    fn compute_cwd_git_info_nested_repo_does_not_inherit_db_record() {
        ...
        // serial(CHUTES_BUILD_HOME) orders peers; EnvVarGuard restores on drop so later `open_default()` callers do not see a deleted temp home
        let _grok_home = crate::test_util::EnvVarGuard::set("CHUTES_BUILD_HOME", home.path());
