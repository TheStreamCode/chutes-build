...
fn mixed_report() -> DiagnosticReport {
    ...
    report.facts.color = ColorFacts {
        ...
        available_themes: vec![ThemeKind::ChutesNight, ThemeKind::ChutesDay],
        total_themes: ThemeKind::ALL.len(),
    };
    ...
    report.findings = vec![
        ...
        DiagnosticFinding {
            ...
            message: "Use local SSH wrapping".to_owned(),
            remediation: Some(ManualRemediation {
                fix: "chutes-build wrap ssh <host>".to_owned(),
                config_path: None,
            }),
            automatic_remediation: Some(crate::diagnostics::ssh_wrap_automatic_remediation()),
            note: None,
...
fn human_mixed_fixture_is_exact() {
    assert_eq!(
        ...
        concat!(
            ...
            "  · ssh                          yes\n",
            "  · color                        256\n",
            "  · themes                       2/5: chutesnight, chutesday\n",
            ...
            "    → Automatic setup: `chutes-build doctor fix tmux-clipboard`\n",
            "    → Add `set -g set-clipboard on` to ~/.tmux.conf\n",
            "      Reload tmux after editing.\n",
            "  i terminal.ssh-wrap            Use local SSH wrapping\n",
            "    → Automatic setup: `chutes-build doctor fix ssh-wrap`\n",
            "    → One-off: `chutes-build wrap ssh <host>`\n",
            "\n",
            ...
            "  Some checks only run in Chutes Build. Start Chutes Build and run /doctor.\n",
            "\n",
...
fn fix_preview_contains_exact_change_and_caveats() {
    ...
    assert!(
        preview.contains("To use once without changing config: `chutes-build wrap ssh <host>`")
    );
...
fn json_contract_is_structural_stable_ordered_and_ansi_free() {
        ...
        serde_json::json!({
            ...
            "facts": {
                ...
                "color": {
                    ...
                    "availableThemes": ["chutesnight", "chutesday"],
                    "totalThemes": 5
                },
            ...
            "findings": [
                ...
                },
                {
                    ...
                    "remediation": {"fix": "chutes-build wrap ssh <host>", "configPath": null},
                    "automaticRemediation": {
                        "fixId": "terminal.ssh-wrap",
                        "command": "chutes-build doctor fix terminal.ssh-wrap"
                    },
                    "note": null
                }
            ],
    ...
    assert!(!text.contains("Chutes Build Doctor"));
}
...
fn new_named_findings_extend_json_without_schema_changes() {
    ...
    report.facts.clipboard.fix = Some("chutes-build wrap <ssh command> or /minimal".to_owned());
    report.findings.push(DiagnosticFinding {
    ...
    assert_eq!(
        ...
        "chutes-build wrap <ssh command> or /minimal"
    );
