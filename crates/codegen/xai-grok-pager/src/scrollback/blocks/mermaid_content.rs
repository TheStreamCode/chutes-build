...
///
/// `ChutesDay` is the only light theme.
/// Every other concrete theme (and the `ChutesNight` default that `Auto` resolves to before it reaches the cache) is dark.
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::ChutesDay)
}
