...
mod tests {
    ...
    fn thinking_quote_line_selection_excludes_bar_prefix() {
        ...
        // The bar is recognised by comparing its span's style against the
        // theme's, and both come from the process-global `Theme::current()`.
        // A concurrent `set_theme` test flips it mid-render and the row stops
        // being recognised as a quote — rare, and invisible in isolation.
