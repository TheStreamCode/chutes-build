...
impl<'a> EntryRenderer<'a> {
    ...
    /// Lets the layout cache size off-screen entries on a bulk load (`chutes-build -r`) without word-wrapping or markdown-rendering every entry (O(history)).
...
mod tests {
    ...
    fn gutter_keeps_code_block_background_on_no_background_block() {
        ...
        //
        // Requires color support: under `NO_COLOR` the global markdown style has no code background while this test's `chutesnight()` theme has RGB
    ...
    fn affordance_row_adds_one_row_per_diagram() {
        ...
        // That is the invariant a bulk load (`chutes-build -r`) relies on to avoid clipping
