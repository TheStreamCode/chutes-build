...
fn apply_wrap_child_env(
    ...
    if let Some(appearance) = appearance {
        ...
        cmd.env("CHUTES_BUILD_APPEARANCE", value);
        cmd.env("LC_CHUTES_BUILD_APPEARANCE", value);
    }
}

/// Run an arbitrary command inside a local PTY with OSC 52 output filtering.
///
/// This is the engine behind `chutes-build wrap`: it spawns `program` (with `args`) attached to a local pseudo-terminal.
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // Handles are intentionally detached: `chutes-build wrap` is short-lived and exits with the child
...
mod tests {
    ...
    fn apply_wrap_child_env_dark_overrides_parent_light_on_both_names() {
        ...
        cmd.env("CHUTES_BUILD_APPEARANCE", "light");
        cmd.env("LC_CHUTES_BUILD_APPEARANCE", "light");
        apply_wrap_child_env(&mut cmd, Some(SystemAppearance::Dark));
        assert_eq!(
            env_str(&cmd, "CHUTES_BUILD_APPEARANCE").as_deref(),
            Some("dark")
        );
        assert_eq!(
            env_str(&cmd, "LC_CHUTES_BUILD_APPEARANCE").as_deref(),
            Some("dark")
        );
        assert_eq!(
            env_str(&cmd, "CHUTES_BUILD_OSC52_SINK").as_deref(),
            Some("1")
        );
        assert_eq!(
            env_str(&cmd, "LC_CHUTES_BUILD_OSC52_SINK").as_deref(),
            Some("1")
        );
    }
    ...
    fn apply_wrap_child_env_light_overrides_parent_dark_on_both_names() {
        let mut cmd = portable_pty::CommandBuilder::new("true");
        cmd.env("CHUTES_BUILD_APPEARANCE", "dark");
        cmd.env("LC_CHUTES_BUILD_APPEARANCE", "dark");
    ...
    fn apply_wrap_child_env_none_does_not_stamp_from_parent_snapshot() {
        let mut cmd = portable_pty::CommandBuilder::new("true");
        cmd.env("CHUTES_BUILD_APPEARANCE", "dark");
        cmd.env_remove("LC_CHUTES_BUILD_APPEARANCE");
        apply_wrap_child_env(&mut cmd, None);
        assert_eq!(
            env_str(&cmd, "CHUTES_BUILD_APPEARANCE").as_deref(),
            Some("dark")
        );
        assert_eq!(env_str(&cmd, "LC_CHUTES_BUILD_APPEARANCE"), None);
        assert_eq!(
            env_str(&cmd, "CHUTES_BUILD_OSC52_SINK").as_deref(),
            Some("1")
        );
