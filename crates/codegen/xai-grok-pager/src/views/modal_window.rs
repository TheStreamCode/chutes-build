...
fn render_tab_bar(
    ...
    for (row_idx, row_indices) in rows.iter().enumerate() {
        ...
        for (local_idx, &tab_idx) in row_indices.iter().enumerate() {
            ...
            // That is fine for the modal frame's one-cell border line but unreadable as text glyphs on chutesday
...
mod tests {
    ...
    fn centered_tip_footer_centers_and_clips() {
        ...
        let theme = Theme::current();
        const TEXT: &str = "Tip · Ask Chutes Build about the docs";
        ...
        assert!(wide.contains("Ask Chutes Build about the docs"));
