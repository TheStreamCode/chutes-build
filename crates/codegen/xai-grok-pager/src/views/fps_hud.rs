//! Release-safe FPS readout: `/debug fps`, and `CHUTES_BUILD_FPS` on release builds.
//!
//! The frame profiler (`render::frame_metrics`, `CHUTES_BUILD_FPS`) threads per-phase timings through `draw_frame`, so it exists only in debug/dev builds.
...
//!
//! `CHUTES_BUILD_FPS` ownership: in debug/dev builds the env feeds `FrameMetrics` and this HUD stays toggle-only (no double overlay).
//! On release binaries, where that overlay does not exist, the same env enables this HUD from startup.
//! `CHUTES_BUILD_FPS=1` is therefore never a silent no-op ([`HONORS_CHUTES_BUILD_FPS_ENV`]).
//!
...
/// Whether this HUD owns the `CHUTES_BUILD_FPS` env gate: only where the dev `FrameMetrics` overlay is compiled out.
/// In debug/dev builds the env keeps feeding that overlay alone.
const HONORS_CHUTES_BUILD_FPS_ENV: bool = true;
/// Runtime state for the FPS HUD.
/// `CHUTES_BUILD_FPS` enables it at startup on release binaries ([`HONORS_CHUTES_BUILD_FPS_ENV`]); `/debug fps` toggles it live everywhere.
...
impl FpsHud {
    pub fn new() -> Self {
        Self::with_env(std::env::var("CHUTES_BUILD_FPS").ok())
    }
    /// `env` is the raw `CHUTES_BUILD_FPS` value; the truthiness rule (nonempty and not `"0"`) matches `FrameMetrics` and `CHUTES_BUILD_SCROLL_DEBUG`.
    fn with_env(env: Option<String>) -> Self {
        let env_on = HONORS_CHUTES_BUILD_FPS_ENV && env.is_some_and(|v| !v.is_empty() && v != "0");
        Self {
            enabled: env_on,
...
pub struct FpsOverlay {
    body: String,
    /// Rows left free for overlays above (the dev `CHUTES_BUILD_FPS` line).
    pub top_offset: u16,
}
impl FpsOverlay {
...
mod tests {
    ...
    /// Asserting against [`HONORS_CHUTES_BUILD_FPS_ENV`] keeps the test true under both cfgs.
    ...
    fn grok_fps_env_enables_hud_where_dev_overlay_absent() {
        for truthy in ["1", "full", " "] {
            assert_eq!(
                ...
                HONORS_CHUTES_BUILD_FPS_ENV,
                "CHUTES_BUILD_FPS={truthy:?} must track the env-gate owner"
            );
        }
