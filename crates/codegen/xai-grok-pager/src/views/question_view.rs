...
/// Pager-internal origin for a locally-opened question (one that was NOT driven by an ACP `chutes.ai/ask_user_question` request).
///
...
pub const FEEDBACK_QUESTION_LABEL: &str = "How can we improve Chutes Build?";
...
///
/// Created when an `chutes.ai/ask_user_question` ext-method request arrives;
...
pub struct QuestionViewState {
    ...
    /// `Some` when this question was opened locally (e.g. by `/fork`) instead of by an ACP `chutes.ai/ask_user_question` request.
...
impl QuestionViewState {
    ...
    ///
    /// Called by the `ExtMethod` handler when a blocking `chutes.ai/ask_user_question` request arrives from the shell coordinator.
...
impl QuestionViewState {
    ...
    pub fn begin_feedback_trace_stage(
        ...
        // "Opt in" is a persistent grant, so its description names what it turns on beyond this one upload
...
///
/// `prefix_w` is the total prefix width used by option rows (number and marker), so the freeform row aligns with the option labels.
/// `freeform_text` is the per-question freeform text; when non-empty the row shows as ticked with a preview of the answer.
...
mod tests {
    ...
    fn chrome_height_option_less_question_drops_the_label_gap() {
        ...
        let q = make_question("How can we improve Chutes Build?", &[], false);
        assert_eq!(
            chrome_height(
                &q,
                80,
