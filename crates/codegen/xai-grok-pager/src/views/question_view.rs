...
/// driven by an ACP `chutes.ai/ask_user_question` request).
///
...
pub struct QuestionViewState {
    ...
    /// instead of by an ACP `chutes.ai/ask_user_question` request. `None` for
...
impl QuestionViewState {
    ...
    ///
    /// Called by the `ExtMethod` handler when a blocking `chutes.ai/ask_user_question`
