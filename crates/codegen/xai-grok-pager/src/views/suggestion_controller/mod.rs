...
//!
//! On a text change (after debounce), the controller sends an `chutes.ai/suggest` request through the Effect pipeline.
...
/// Parsed ghost suggestion from an ACP `chutes.ai/suggest` response.
#[derive(Debug, Clone)]
...
/// A single completion item from an ACP `chutes.ai/suggest` response.
...
/// Parsed response from an ACP `chutes.ai/suggest` request.
#[derive(Debug, Clone)]
...
impl SuggestResponseParsed {
    /// Parse a raw JSON value from an ACP `chutes.ai/suggest` response.
...
/// Wire `limit` for `chutes.ai/suggest` fetches.
...
pub struct SuggestionController {
    ...
    /// Resolved at construction from the `CHUTES_BUILD_SUGGESTIONS` env var.
    ...
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `chutes.ai/suggest` request.
...
impl SuggestionController {
    ...
    /// Called when an ACP `chutes.ai/suggest` response arrives, with the text and cursor the request was built from.
