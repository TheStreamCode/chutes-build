...
pub enum QueueRowOrigin {
    ...
    Local,
    /// A server-authoritative `shared_prompt_queues` entry (plain prompt queued while running). Edits route to the agent as `chutes.ai/queue/*`.
    Server,
}
...
pub struct QueuePane {
    ...
    /// (Otherwise the focused-row highlight keeps the previous theme's `bg_highlight`, e.g. ChutesNight's dark band leaking into ChutesDay.)
...
impl QueuePane {
    ...
    pub fn render(
        ...
        // Without this it would keep the theme active at construction (default ChutesNight, dark) after the user switches
        // That paints a dark band on a light ChutesDay canvas
