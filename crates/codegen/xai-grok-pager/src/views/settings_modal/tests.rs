...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("Chutes Build Test", 4); // cursor between "Chutes Build" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
fn picking_enum_esc_returns_to_browse() {
    let mut s = make_state();
    s.transition_to_picking_enum("theme", 0, SettingValue::Enum("chutesnight"), true);
    let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
    match outcome {
        ...
        other => panic!("expected Action::PreviewTheme(\"chutesnight\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode(), SettingsModalMode::Browse));
}
...
fn string_editor_uses_canonical_edits_policy_and_live_validation() {
    ...
    let mut state = editor_render_fixture("Chutes Build Tes", "Chutes Build Tes".len());
    ...
    assert_eq!(state.editing_buffer(), Some("Chutes Build Test"));
...
// -- Blank line between the "Tip · Ask Chutes Build…" docs footer and the keybinding hints --
//
...
fn string_editor_paste_sanitizes_validates_and_consumes_rejected_text() {
    let mut state = editor_render_fixture("Chutes Build Tst", "Chutes Build T".len());
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT is "Tip · Ask Chutes Build to change a setting" (34 cells); LONG is ~73 cells. width=40 lands in the SHORT band.
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    // The default theme's original canonical is `"chutesnight"`
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"chutesnight\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // The default theme is `chutesnight` (index 1 per the registry); advance to index 0 to ensure we're navigating to a different value
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `chutesnight`
            // Entering the picker captures `original_value = current value = chutesnight`, so the revert dispatches with that canonical
            assert_eq!(
                orig, "chutesnight",
...
fn consent_chooser_drops_tip_and_reset() {
    ...
    assert!(
        !text.contains("Ask Chutes Build"),
    ...
    assert!(
        text.contains("d reset") && text.contains("Ask Chutes Build"),
...
/// "Darker" is not the contract; on the dark themes `bg_visual` is lighter than `bg_highlight`, and only ChutesDay renders the title darker.
#[test]
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::ChutesNight => crate::theme::Theme::chutesnight(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::ChutesDay => crate::theme::Theme::chutesday(),
        ...
        crate::theme::ThemeKind::Auto => crate::theme::Theme::chutesnight(),
    };
    assert_ne!(
