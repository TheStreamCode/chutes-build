...
//! Users review each discovered item, toggle which to import, and confirm. Only checked items are written to `.chutes-build/config.toml`.
...
fn build_rows(
    ...
    if !plan.global_items.is_empty() {
        ...
        let label = "Global  ~/.chutes-build/config.toml".to_string();
