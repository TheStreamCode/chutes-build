...
//!
//! The box-drawing art renders at every height tier on every console: the
//! wordmark must never silently vanish because of terminal detection, and
//! modern ConHost fonts cover box drawing fine.
...
/// Pure tier selection so tests can drive it directly. Height is the only
/// input: no terminal detection may suppress the wordmark.
...
fn shine_opacity(diag: f32, secs: f32) -> f32 {
    const BAND: f32 = 0.42; // half-width of the shine band — wider = more gradual falloff
    const CYCLE: f32 = 4.0; // seconds per sweep + rest
    const SWEEP_FRAC: f32 = 0.40; // portion of the cycle spent sweeping (~1.6s glint, rest idles)
    const SHINE: f32 = 0.60; // peak shine strength
    const PULSE: f32 = 0.12; // global breathing amount
...
mod tests {
    ...
    #[test]
    fn compact_card_uses_the_small_logo() {
        // The minimal welcome card budgets exactly the small logo's rows,
        // strictly shorter than the full art.
