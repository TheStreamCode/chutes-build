...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Chutes Build** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Chutes Build**  VERSION (left-aligned).
    HeroInline,
}
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("chutes.ai");
...
fn render_welcome_authenticating(
    ...
    match mode {
        ...
        AuthMode::ApiKeyEntry => {
            // No browser and no URL: just ask for the key. This is the path that
            // works with no configuration at all, so it stays deliberately plain.
            let h_pad: u16 = content_area.width / 6;
            let [_, logo_area, _, msg_area, _, prompt_area, _, hint_area, _] = Layout::vertical([
                Constraint::Length(top_pad),
                Constraint::Length(logo_line_count),
...
fn render_welcome_done(
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let gate_text = p
            ...
            .unwrap_or("A Chutes plan is required");
        let msg = Line::from(Span::styled(
            gate_text,
            Style::default().fg(theme.gray_bright),
        ))
        ...
        if centered.height > 2 {
            ...
            let gate_link = p
                ...
                .unwrap_or("https://chutes.ai/supergrok?referrer=chutes-build");
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("Chutes Build"), "full badge: {full:?}");
        assert!(inline.contains("Chutes Build"), "inline badge: {inline:?}");
    ...
    /// Chutes Build view when scanned foreign rows exist — even when the native list
    ...
    fn hidden_external_hint_renders_outside_chat_mode() {
        ...
        assert!(
            ...
            "default Chutes Build filter must pin the hidden-external hint:\n{build_mode}"
        );
        assert!(
    ...
    fn extract_user_code_parses_verification_url() {
        assert_eq!(
            extract_user_code("https://accounts.chutes.ai/oauth2/device?user_code=ABCD-EFGH"),
            Some("ABCD-EFGH"),
        );
    ...
    fn device_auth_arm_shows_url_and_no_paste_box() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.chutes.ai/oauth2/device?user_code=ABCD-EFGH";
    ...
    fn device_auth_arm_raw_url_mode_shows_full_url() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.chutes.ai/oauth2/device?user_code=WXYZ-1234";
    ...
    fn raw_url_mode_uses_full_width_for_long_urls() {
        ...
        let url = "https://accounts.chutes.ai/oauth2/device?user_code=WXYZ-1234&extra=0123456789";
    ...
    fn command_auth_arm_shows_url_and_waiting() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.chutes.ai/oauth2/authorize?client_id=chutes-build";
