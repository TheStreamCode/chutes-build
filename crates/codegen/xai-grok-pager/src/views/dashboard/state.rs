...
/// Persisted dashboard configuration stored under `[dashboard]` in `~/.chutes-build/config.toml`.
...
/// Without this guard, a single dashboard pin would clobber every other table in `~/.chutes-build/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
