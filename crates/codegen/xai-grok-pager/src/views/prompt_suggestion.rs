...
/// Env override for the whole feature: `CHUTES_BUILD_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "CHUTES_BUILD_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call: `CHUTES_BUILD_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "CHUTES_BUILD_PROMPT_SUGGESTIONS_MODEL";
...
pub struct PromptSuggestionController {
    ...
    shown_logged: bool,
    /// Whether the feature is enabled. Resolved from `CHUTES_BUILD_PROMPT_SUGGESTIONS`, falling back to the persisted `prompt_suggestions` setting.
    pub enabled: bool,
}
