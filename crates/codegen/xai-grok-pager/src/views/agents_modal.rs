...
/// User-level vs project-level config files (`~/.chutes-build` vs `{cwd}/.grok`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as the shell: `[agent]`, `CHUTES_BUILD_AGENT`, model `agentType`, then `grok-build`).
...
/// Union bundled personas with local `~/.chutes-build/personas` and `{cwd}/.chutes-build/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let dirs = [
        (
            ...
            cwd.join(".chutes-build").join("personas"),
        ),
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ...
        ConfigFileScope::Project => cwd.join(".chutes-build").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.chutes-build/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.chutes-build/{subdir}` or `{cwd}/.chutes-build/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let project_suffix = std::path::Path::new(".chutes-build").join(subdir);
...
mod tests {
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".chutes-build").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.chutes-build/bundled/personas/b.toml".into()),
            scope_label: None,
        };
