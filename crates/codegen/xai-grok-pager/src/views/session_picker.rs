...
/// The filter check (not `entries.is_none()`) matters: the fast foreign scan can land rows the default Chutes Build view hides before the native list
/// arrives.
...
///
/// Default is [`Self::Grok`]: native Chutes Build sessions only (local / remote / conversation).
/// That keeps Claude/Codex/Cursor foreign sessions and `chutes-build -p` one-shots out of `/resume`.
/// `f` cycles Chutes Build, Headless, External, All, Local, Remote, then back to Chutes Build.
...
pub enum SourceFilter {
    /// Native Chutes Build sessions only; excludes Claude/Codex/Cursor foreign rows.
    #[default]
    Grok,
    /// `chutes-build -p` one-shots only (`session_kind == "headless"`).
    Headless,
    Local,
    Remote,
    External,
...
impl SourceFilter {
    ...
    ///
    /// grok.com conversations carry `source == "conversation"` and live remotely, so they pass the `Remote` filter (and `Chutes Build` / `All`) but not
    /// `Local`.
...
mod tests {
    ...
    fn source_filter_matches() {
        // Default Chutes Build filter: native only (not Claude/Codex/Cursor).
        ...
        // grok.com conversations are remote: visible under Chutes Build, All, and Remote, not Local
    ...
    fn source_filter_cycles() {
        ...
        assert_eq!(SourceFilter::Grok.label(), "Chutes Build");
    ...
    fn source_filter_empty_and_unknown_source() {
        ...
        // It passes Chutes Build and All but never Local, Remote, or External
