...
//! Folds a completed search inline (or the generic client split on failure), and parses Chutes Build's `WebSearchCall` output into the wire hit array.
...
/// Chutes Build nests query/sources under `action`, with a flat `{"query",...,"sources"}` fallback.
