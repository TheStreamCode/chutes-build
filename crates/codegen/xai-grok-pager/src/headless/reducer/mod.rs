...
pub(crate) struct ToolCallEvent {
    ...
    content: Value,
    locations: Value,
    /// True for Chutes Build's backend `web_search`; it is folded inline instead of taking the split path client tools take.
...
/// True iff `tc` is Chutes Build's backend `web_search` (`_meta.backend` and `raw_input.variant == "WebSearch"`).
