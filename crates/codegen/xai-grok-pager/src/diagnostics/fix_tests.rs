...
fn canonical_and_short_ids_resolve_to_canonical_id() {
    ...
    assert_eq!(command, "chutes-build doctor fix ssh-wrap");
    assert_eq!(
        resolve_fix_id(command.strip_prefix("chutes-build doctor fix ").unwrap()).unwrap(),
        SSH_WRAP_ID
    );
...
fn tmux_fix_registry_resolves_every_short_and_canonical_id() {
    for (id, handle, _) in automatic_fix_choices() {
        ...
        assert_eq!(
            ...
            format!("chutes-build doctor fix {handle}")
        );
    }
}
...
fn tmux_managed_items_coexist_and_each_apply_is_one_transaction() {
    ...
    assert_eq!(content.matches("# >>> chutes-build doctor >>>").count(), 1);
    for id in [
        TMUX_CLIPBOARD_ID,
        DCS_PASSTHROUGH_ID,
        TMUX_EXTENDED_KEYS_ID,
...
fn shell_aliases_expand_to_exact_argv_and_bypass_is_explicit() {
    ...
    if let Some(bash) = find_on_path("bash") {
        ...
        std::fs::write(&rc, "alias ssh='chutes-build wrap ssh'\n").unwrap();
        let command = format!(
    ...
    let mut shell = std::process::Command::new(bash);
    shell
        .args(["-ic", "alias ssh='chutes-build wrap ssh'; command ssh host"])
        .env("CAPTURE", &capture)
        .env(
            "PATH",
            format!(
