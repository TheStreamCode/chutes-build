...
fn voice_missing_finding(error: String) -> DiagnosticFinding {
    DiagnosticFinding {
        ...
        // The line breaks keep the command name and its subcommand on the same
        // line. A string continuation splitting them hides the invocation from
        // any per-line rename rule, which is how this one kept naming upstream's
        // binary long after the rest of the product had been renamed.
        note: Some(
            "Connect or select a microphone in your system sound settings. On Linux, \
             ...
             `/doctor` or `chutes-build doctor` again. Doctor can't detect denied macOS \
...
pub fn color_support_warning(
    ...
    // what Chutes Build emits, which is a different question from what survives tmux.
    // A truecolor detection is not evidence that truecolor reaches the
    // terminal, and a session with no color evidence (piped `chutes-build doctor`)
...
mod tests {
    ...
    /// Doctor whenever Chutes Build's own detection reported truecolor, so a session
    ...
    /// Piped `chutes-build doctor` has no color evidence, but the tmux client is still
