...
/// Passive input-device probe for `chutes-build doctor` / `/doctor`.
///
...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `chutes-build wrap` on the local end.
...
pub(crate) fn collect_startup_warnings_from(
    ...
    if ctx.brand == TerminalName::AppleTerminal && ctx.is_ssh {
        ...
        warning.note = Some(
            "Chutes Build also saves each copy to the backup file shown in the copy message. To copy \
             directly, run `chutes-build wrap ssh <host>` on your local computer or use a terminal that \
    ...
    if ctx.is_tmux_backed() && matches!(tmux.control_mode, probes::TmuxProbeResult::Available(true))
    {
        let message = match fullscreen_active {
            ...
            Some(false) => {
                "Chutes Build is using inline mode because tmux control mode limits fullscreen"
            }
...
pub(crate) fn wezterm_kitty_keyboard_warning_from(
    ...
    if shape == WezTermShape::SshXtversion {
        ...
        warning.note = Some(
            "For this session, type `\\` and then press Enter. Chutes Build can't negotiate the Kitty \
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        config_path: None,
        note: Some(format!(
            "Chutes Build is using the user profile. Compare `.chutes-build/sandbox.toml` with {}, then rename \
...
/// Pure SSH `chutes-build wrap` recommendation: suggests launching the session through `chutes-build wrap ssh <host>` on the user's local machine.
...
///   `chutes-build wrap` advertises its OSC 52 sink through the SSH hop via an env var (see `clipboard::osc52_sink_active`).
...
pub(crate) fn collect_notification_warnings_with_method(
        ...
        && ctx.brand == TerminalName::Unknown
    {
        let mut warning = TerminalWarning::new(
            ...
            "Chutes Build is using the terminal bell because the terminal was not recognized",
            None,
            None,
        );
...
pub fn color_support_warning(
    ...
    if level == probes::RuntimeEvidence::Available(ColorLevel::None) {
        ...
        warning.note = Some("Unset `NO_COLOR`, then restart Chutes Build.".to_string());
        return Some(warning);
    }

    // Checked before the detected level is consulted at all: the level says what Chutes Build emits, which is a different question from what survives tmux
    // A truecolor detection is not evidence that truecolor reaches the terminal
    // A session with no color evidence (piped `chutes-build doctor`) still has a clamping client worth reporting
    if color_passthrough == TmuxColorPassthrough::Reduced {
        ...
        warning.note = Some(format!(
            "Run `tmux source-file {tmux_config_path}`, then detach and reattach: the server \
             ...
             If Chutes Build still reports less than truecolor afterwards, also add `set -g \
    ...
    if is_tmux_backed {
        ...
        warning.note = Some(format!(
            "In the same tmux config, also add `set -g default-terminal \"tmux-256color\"`. Add \
             ...
             `tmux source-file {tmux_config_path}`, then detach and reattach, and restart Chutes Build."
        ));
        return Some(warning);
    }
...
mod tests {
    ...
    fn sandbox_profile_conflict_warning_reports_conflicts() {
        ...
        assert!(w.note.as_deref().is_some_and(|note| {
            note.contains("rename or remove")
                && note.contains(".chutes-build/sandbox.toml")
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        // An active OSC 52 sink means the session already runs under `chutes-build wrap`; adoption silences the hint by itself
    ...
    fn voice_missing_finding_has_stable_id_and_manual_remediation() {
        ...
        assert!(finding.note.as_deref().is_some_and(|note| {
            note.contains("install a supported audio recorder")
                && note.contains("chutes-build doctor")
    ...
    /// Regression: a tmux client that reduces color used to be invisible to Doctor whenever Chutes Build's own detection reported truecolor.
    ...
    /// Piped `chutes-build doctor` has no color evidence, but the tmux client is still measurable, and `doctor fix` needs the finding to plan against.
    #[test]
