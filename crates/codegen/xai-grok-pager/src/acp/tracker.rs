...
pub struct AcpUpdateTracker {
    ...
    ///
    /// First-detection (no scrollback entry yet): defers entry creation until `chutes.ai/task_backgrounded` creates a `BgTask` block.
...
///
/// These are deferred from scrollback; the `chutes.ai/task_backgrounded` notification creates a `BgTask` block instead of an `Execute` block.
///
