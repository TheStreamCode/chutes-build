...
pub(crate) fn is_session_update_ext_method(method: &str) -> bool {
    matches!(
        method,
        "chutes.ai/session_notification" | "chutes.ai/session/update"
    )
}
...
pub struct AcpConnection {
    ...
    /// The client gates its automatic away-recap poll and the manual `/recap` on this so a disabled feature produces zero `chutes.ai/recap` traffic.
...
pub struct ConnectFlags {
    ...
    /// Whether this client will draw a status row, advertised as `chutes.ai/statusLine` so the agent can skip an unpainted payload.
...
fn client_capabilities_meta(flags: &ConnectFlags) -> serde_json::Value {
    ...
    let mut meta = serde_json::json!({
        "chutes.ai/incrementalBashOutput": true,
        "chutes.ai/hunkTracker": { "mode": hunk_mode },
        "chutes.ai/bashOutputNoColor": true,
        "chutes.ai/gitHeadChanged": true,
    });
...
/// When `chutes.api_key` was advertised, non-interactive credentials were available: do not promote to interactive auto-Login.
...
mod tests {
    ...
    fn is_session_update_ext_method_covers_both_carriers() {
        ...
        assert!(is_session_update_ext_method("chutes.ai/session/update"));
        assert!(!is_session_update_ext_method("chutes.ai/task_completed"));
    ...
    /// If a change re-orders `build_auth_methods()` to put `chutes.api_key` anywhere other than first (the shape of a past regression), this test fails.
    ...
    /// Inverse direction: when `chutes.api_key` is NOT in the list, the pager MUST show the login screen.
    /// We assert this with `chutes.api_key` present LATER in the list (the shape of a past regression) and confirm the pager still requires login.
    ...
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use xai_grok_shell::agent::auth_method::{
            CHUTES_BUILD_COM_METHOD_ID, XAI_API_KEY_METHOD_ID,
        };

        let methods = vec![
            make_auth_method(CHUTES_BUILD_COM_METHOD_ID, "Chutes Build", None),
            make_auth_method(XAI_API_KEY_METHOD_ID, "chutes.api_key", None),
        ];
    ...
    fn client_capabilities_meta_defaults_absent_or_blank_mode_to_off() {
        ...
        assert_eq!(absent["chutes.ai/hunkTracker"]["mode"], "off");
        let blank = client_capabilities_meta(&ConnectFlags {
            hunk_tracker_mode: Some("   ".into()),
            ..Default::default()
        });
        assert_eq!(blank["chutes.ai/hunkTracker"]["mode"], "off");
    }
    ...
    fn client_capabilities_meta_canonicalizes_off_and_mixed_case() {
        ...
        for raw in ["off", "OFF", "Disabled"] {
            ...
            assert_eq!(meta["chutes.ai/hunkTracker"]["mode"], "off", "raw={raw}");
        }
    }
}
