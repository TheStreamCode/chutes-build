...
mod tests {
    ...
    fn with_test_env(f: impl FnOnce()) {
        ...
        theme_cache::set(ThemeKind::ChutesNight);
        f();
        system_appearance::clear_mock();
        theme_cache::reset_for_test();
    }
    ...
    fn suggest_args_explicit_active_when_not_auto() {
        with_test_env(|| {
            ...
            let items = cmd.suggest_args(&ctx, "").expect("should return items");
            let chutesnight = items
                .iter()
                .find(|i| i.insert_text == "chutesnight")
                .expect("chutesnight should be in list");
            assert!(
                chutesnight.description.contains("(active)"),
                "explicit theme should show (active), got: {}",
                chutesnight.description
            );
        });
    }
    ...
    fn run_explicit_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            let result = cmd.run(&mut ctx, "chutesnight");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "chutesnight");
                }
                other => panic!("expected Action::SetTheme(\"chutesnight\"), got {other:?}"),
            }
        });
    }
    ...
    fn run_toggle_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    // available[0] is ChutesNight; next is available[1]
    ...
    fn run_alias_normalises_to_canonical() {
        with_test_env(|| {
            ...
            // "dark" is an alias for ChutesNight.
            let result = cmd.run(&mut ctx, "dark");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "chutesnight", "alias must normalise to canonical");
                }
                other => panic!("expected Action::SetTheme(\"chutesnight\"), got {other:?}"),
            }
        });
    ...
    fn preview_auto_applies_resolved_theme() {
        with_test_env(|| {
            ...
            // The default auto config maps Light to ChutesDay
            assert_eq!(Theme::current_kind(), ThemeKind::ChutesDay);
        });
    }
    ...
    fn cancel_preview_restores_previous_kind() {
        with_test_env(|| {
            ...
            cmd.cancel_preview("chutesnight");
            assert_eq!(
                Theme::current_kind(),
                ThemeKind::ChutesNight,
    ...
    fn cancel_preview_unknown_theme_is_no_op() {
        with_test_env(|| {
            theme_cache::set(ThemeKind::ChutesDay);
            let cmd = ThemeCommand;
