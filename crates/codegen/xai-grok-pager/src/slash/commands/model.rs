...
impl SlashCommand for ModelCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        // If we split on the last token first, a shorter catalog entry ("Chutes Build") would steal the prefix and treat "4.5" as an effort level
...
mod tests {
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // The catalog has both "Chutes Build" (reasoning) and "Grok 4.5"
        // `/model Grok 4.5` must select the full name, not treat "4.5" as an effort on "Chutes Build"
        let mut state = ModelState::default();
