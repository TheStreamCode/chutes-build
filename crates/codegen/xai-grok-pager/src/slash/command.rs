...
pub struct AppCtx<'a> {
    ...
    /// False for external-auth deployments with no chutes.ai billing session.
...
pub trait SlashCommand: Send + Sync {
    ...
    ///
    /// Minimal mode (`chutes-build --minimal`) deletes the interactive fullscreen scrollback pane, the in-app mouse selection path, and the agent dashboard.
