...
//! Any process that can write to the PTY (not only the inner `chutes-build`) can solicit the host pasteboard.
//! That is intentional and acceptable for `chutes-build wrap`: the user opted into wrap on their own host, and the answer stays inside their session.
//! The remote also only requests when `osc52_sink_active()` (wrap already set `CHUTES_BUILD_OSC52_SINK` / `LC_CHUTES_BUILD_OSC52_SINK`).
