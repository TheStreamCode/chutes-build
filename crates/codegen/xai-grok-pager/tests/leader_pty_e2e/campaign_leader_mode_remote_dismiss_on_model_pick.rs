...
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // Pin the leader socket under the shared CHUTES_BUILD_HOME so every spawn elects or attaches to the same leader (mirrors `LeaderCluster`)
    ...
    // Use session (OAuth) auth instead of the harness's default CHUTES_API_KEY
