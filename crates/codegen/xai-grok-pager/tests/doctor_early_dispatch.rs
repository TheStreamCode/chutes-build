...
fn doctor_json_bypasses_unrelated_startup_state() {
    ...
    assert!(!String::from_utf8_lossy(&output.stdout).contains("Chutes Build Doctor"));
...
fn doctor_fix_without_id_lists_only_applicable_automatic_fixes() {
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(
        stdout.contains("On your local computer, run: chutes-build doctor fix ssh-wrap"),
        "{stdout}"
    );
    assert!(!home.join(".bashrc").exists());
...
fn doctor_fix_safety_boundaries_are_process_isolated() {
    ...
    assert!(
        stderr.contains("Chutes Build found an existing SSH alias or function")
...
fn base_pager_command(
    ...
    command
        .env_clear()
        .env("HOME", home)
        .env("CHUTES_BUILD_HOME", grok_home)
        .env("SHELL", shell)
