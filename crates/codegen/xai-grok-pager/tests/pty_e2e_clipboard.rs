...
async fn unknown_ssh_clipboard_delivery_is_unverified() {
    ...
    let mut harness = PtyHarness::spawn_with_content_env_ops_in_dir(
        ...
        &[
            ...
            // launched under `chutes-build wrap`.
            EnvOp::remove("CHUTES_BUILD_OSC52_SINK"),
            EnvOp::remove("LC_CHUTES_BUILD_OSC52_SINK"),
        ],
        Some(content.home()),
    )
    .expect("spawn pager");
    ...
    harness
        .wait_for_text("chutes-build wrap ssh <host>", Duration::from_secs(10))
