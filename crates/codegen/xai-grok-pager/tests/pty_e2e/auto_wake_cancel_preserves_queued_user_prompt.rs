...
//!
//! Set `CHUTES_BUILD_PTY_CAST_DIR` to also dump asciinema casts of both pager runs (written before the final asserts so a failing run still produces them).
//!
...
fn unified_log_diagnostics(content: &ContentController) -> String {
    let path = content.home().join(".chutes-build/logs/unified.jsonl");
