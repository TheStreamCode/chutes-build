...
async fn embedded_mode_boots_without_hanging_on_blocked_backend() {
    ...
    let env = [
        ...
        ("CHUTES_BUILD_HOME", grok_home.to_str().unwrap()),
        ("CHUTES_API_KEY", "test-key-for-ci"),
        ("CHUTES_BUILD_CLI_CHAT_PROXY_BASE_URL", base.as_str()),
        ("CHUTES_BUILD_XAI_API_BASE_URL", base.as_str()),
