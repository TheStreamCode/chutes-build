...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // The harness's CHUTES_API_KEY is ApiKey mode and never uploads, so seed a fake OAuth entry instead
    ...
    let overrides = [
        ...
        EnvOp::set("CHUTES_BUILD_TRACE_UPLOAD", "true"),
        EnvOp::set("CHUTES_BUILD_TELEMETRY_TRACE_UPLOAD", "true"),
        EnvOp::set("CHUTES_BUILD_UPLOAD_QUEUE_AUTH_PROBE_SECS", "2"),
    ];
    ...
    //   `has_usable_credential()` stays true for the seeded entry, so the 2s probe (`CHUTES_BUILD_UPLOAD_QUEUE_AUTH_PROBE_SECS`) can wake workers early
    //
