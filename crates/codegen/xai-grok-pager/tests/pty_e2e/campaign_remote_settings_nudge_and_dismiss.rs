...
/// The sibling test injects the campaign through `CHUTES_BUILD_CAMPAIGNS_OVERRIDE` instead.
///
...
async fn campaign_remote_settings_nudge_and_dismiss() {
    ...
    // Use session (OAuth) auth, not the harness's default CHUTES_API_KEY: the settings fetch requires `auth_manager.auth()`
