...
/// Direct-route e2e: `chutes-build wrap` with a resolvable explicit path runs the command inside the wrap PTY with output passing through.
...
fn wrap_echo_passthrough_and_exit_code() {
    ...
    assert_eq!(
        code,
        Some(7),
        "a nonzero child exit must propagate as chutes-build wrap's own exit\nraw:\n{raw}"
    );
}
