...
async fn run_reasoning_turn(collapse_thinking: bool) -> Turn {
    ...
    std::fs::create_dir_all(content.home().join(".chutes-build")).expect("mk .chutes-build");
    std::fs::write(
        content.home().join(".chutes-build/config.toml"),
        "[ui]\nshow_thinking_blocks = true\n",
    )
    .expect("write config");
    if collapse_thinking {
        let grok_home = content.sandbox().grok_home().to_path_buf();
        std::fs::create_dir_all(&grok_home).expect("mk Chutes Build home");
        std::fs::write(
