...
/// `CHUTES_BUILD_SCREEN_MODE_SWITCH=exec` forces the legacy switch that quits, execs, and resumes, keeping that fallback covered end to end.
...
async fn minimal_switch_exec_escape_hatch() {
    ...
    let mut harness = PtyHarness::spawn_with_content_env_in_dir(
        ...
        &content,
        &["--no-leader"],
        &[("CHUTES_BUILD_SCREEN_MODE_SWITCH", "exec")],
        Some(project.path()),
    )
