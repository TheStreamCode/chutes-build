...
//
// The harness runs with `TERM_PROGRAM=zed` (Zed profile: ept=1, wheel_lines=3) plus `CHUTES_BUILD_SCROLL_MODE=wheel` and `CHUTES_BUILD_SCROLL_LINES=1`
...
// - Without CHUTES_BUILD_SCROLL_LINES=1, Zed's wheel profile prices the burst at 3 lines per event, 9 rows total
// - Without CHUTES_BUILD_SCROLL_MODE=wheel, Auto finalizes a 3-event ZERO-interval burst as Trackpad on ept=1 (event_count > 2)
...
// - The harness's hermetic CHUTES_BUILD_HOME pins scroll_speed at its default (50, a 1.0x multiplier)
...
/// The `CHUTES_BUILD_SCROLL_MODE` and `CHUTES_BUILD_SCROLL_LINES` overrides must reach the live config: a 3-event burst scrolls the viewport up by exactly 3 rows.
...
async fn forced_wheel_mode_env_scrolls_exact_rows() {
    let (mut harness, _content, top_before) = spawn_bottom_pinned_marker_scrollback_with_env(
        ...
        &[
            ("TERM_PROGRAM", "zed"),
            ("CHUTES_BUILD_SCROLL_MODE", "wheel"),
            ("CHUTES_BUILD_SCROLL_LINES", "1"),
        ],
    )
    .await;
