...
/// Seed a user-invocable skill under `dir/.chutes-build/skills`. The workspace must
/// be a git repo: the live session advertises workspace-local skills only for git workspaces (offline `chutes-build inspect` scans plain dirs too).
/// The mid_text_skill_token_echo.yaml scenario sets `git_init: true` for the same reason.
fn seed_test_skill(dir: &Path) {
    let skill_dir = dir.join(".chutes-build").join("skills").join("test-skill");
