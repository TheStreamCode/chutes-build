#
# Chutes Build CLI installer for PowerShell — https://x.ai/cli/install.ps1
#
# Auth: CHUTES_BUILD_DEPLOYMENT_KEY env var (takes precedence) or ~/.chutes-build/auth.json from `chutes-build login`.
...
#   $env:CHUTES_BUILD_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex           # specific version (alt)
#   $env:CHUTES_BUILD_DEPLOYMENT_KEY="<key>"; irm https://x.ai/cli/install.ps1 | iex
#
...
if (-not $Version -and $env:CHUTES_BUILD_VERSION) {
    $Version = $env:CHUTES_BUILD_VERSION
}
...
$GrokDir = Join-Path $env:USERPROFILE '.chutes-build'
...
if ($env:CHUTES_BUILD_DEPLOYMENT_KEY) {
    ...
    if ($oidcToken) {
        ...
        Write-Host 'Auth: using OIDC token from ~/.chutes-build/auth.json.' -ForegroundColor DarkGray
    } elseif ($legacyToken) {
        $AuthSource = 'auth.json (legacy)'
        Write-Host 'Auth: using legacy token from ~/.chutes-build/auth.json.' -ForegroundColor DarkGray
    }
}
...
$BinDir = if ($env:CHUTES_BUILD_BIN_DIR) { $env:CHUTES_BUILD_BIN_DIR } else { Join-Path $GrokDir 'bin' }

New-Item -ItemType Directory -Path $DownloadDir -Force | Out-Null
New-Item -ItemType Directory -Path $BinDir -Force | Out-Null

$Channel = if ($env:CHUTES_BUILD_CHANNEL) { $env:CHUTES_BUILD_CHANNEL } else { 'stable' }
...
if ($AuthSource) {
    Write-Host "Installing Chutes Build $resolvedVersion ($platform, $AuthSource)..." -ForegroundColor Cyan
} else {
    Write-Host "Installing Chutes Build $resolvedVersion ($platform)..." -ForegroundColor Cyan
}
...
if ($env:CHUTES_BUILD_DEPLOYMENT_KEY) {
    $ProxyUrl = if ($env:CHUTES_BUILD_PROXY_URL) { $env:CHUTES_BUILD_PROXY_URL } else { 'https://cli-chat-proxy.grok.com/v1' }
    # Refuse cleartext / userinfo / empty-host proxies before attaching the key.
    try {
        $proxyUri = [Uri]$ProxyUrl
    } catch {
        Write-Error "CHUTES_BUILD_PROXY_URL must be an https:// URL."
        exit 1
    }
    ...
    try {
        $headers = @{ 'Authorization' = "Bearer $($env:CHUTES_BUILD_DEPLOYMENT_KEY)" }
...
Write-Host "Chutes Build $resolvedVersion installed to $BinDir\grok.exe" -ForegroundColor Green
