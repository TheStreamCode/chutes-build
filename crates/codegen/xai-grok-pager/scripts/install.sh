#!/bin/bash
#
# Chutes Build CLI installer — https://x.ai/cli/install.sh
#
# Auth: CHUTES_BUILD_DEPLOYMENT_KEY (takes precedence) or ~/.chutes-build/auth.json from `chutes-build login`.
# Env: CHUTES_BUILD_CHANNEL (stable|alpha|enterprise, default: stable), CHUTES_BUILD_BIN_DIR, CHUTES_BUILD_PROXY_URL
#
# Usage:
#   curl -fsSL https://x.ai/cli/install.sh | bash            # latest stable
#   curl -fsSL https://x.ai/cli/install.sh | bash -s 0.1.42  # specific version
#   CHUTES_BUILD_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/install.sh)
#
...
# Read a token from ~/.chutes-build/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_grok_token() {
    local auth_file="$HOME/.chutes-build/auth.json"
    local scope="$1"
...
if [ -n "$CHUTES_BUILD_DEPLOYMENT_KEY" ]; then
    ...
    if [ -n "$OIDC_TOKEN" ]; then
        ...
        echo "Auth: using OIDC token from ~/.chutes-build/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.chutes-build/auth.json." >&2
    fi
fi
...
DOWNLOAD_DIR="$HOME/.chutes-build/downloads"
BIN_DIR="${CHUTES_BUILD_BIN_DIR:-$HOME/.chutes-build/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${CHUTES_BUILD_CHANNEL:-stable}"
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing Chutes Build $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing Chutes Build $version ($platform)..." >&2
fi
    ...
    if ! download_file_parallel "${artifact_base}.exe" "$binary_tmp"; then
        if ! download_file_parallel "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: Chutes Build is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    # (default layout: ~/.chutes-build/bin and ~/.chutes-build/downloads are siblings).
...
mkdir -p "$HOME/.chutes-build/completions/bash" "$HOME/.chutes-build/completions/zsh"
"$BIN_DIR/grok" completions bash > "$HOME/.chutes-build/completions/bash/grok.bash" 2>/dev/null || true
"$BIN_DIR/grok" completions zsh  > "$HOME/.chutes-build/completions/zsh/_grok"     2>/dev/null || true
...
CONFIG_FILE="$HOME/.chutes-build/config.toml"
...
if [ -n "$CHUTES_BUILD_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${CHUTES_BUILD_PROXY_URL:-https://cli-chat-proxy.grok.com/v1}"
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$CHUTES_BUILD_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.chutes-build/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.chutes-build/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.chutes-build/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.chutes-build/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "Chutes Build $version installed to $BIN_DIR/grok.exe" >&2
else
    echo "Chutes Build $version installed to $BIN_DIR/grok" >&2
fi
...
# Also update shell config so ~/.chutes-build/bin is on PATH for future sessions
...
fish_add_path $HOME/.chutes-build/bin
# <<< grok installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> grok installer >>>
export PATH="$HOME/.chutes-build/bin:$PATH"
fpath=(~/.chutes-build/completions/zsh $fpath)
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    ...
    echo '  export PATH="$HOME/.chutes-build/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use grok from cmd.exe or PowerShell, add %USERPROFILE%\\.chutes-build\\bin to your PATH." >&2
fi
