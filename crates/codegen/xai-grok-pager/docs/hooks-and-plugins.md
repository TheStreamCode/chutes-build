...
Chutes Build supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.chutes-build/hooks/`
- **Project hooks** — from `.chutes-build/hooks/` in your repo
...
Plugin hooks automatically receive `CHUTES_BUILD_PLUGIN_ROOT` and `CHUTES_BUILD_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.chutes-build/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `chutes-build plugin marketplace add <source>`)
...
Sources land in `~/.chutes-build/config.toml`:
...
Or in `~/.chutes-build/settings.json` / `~/.claude/settings.json`:
