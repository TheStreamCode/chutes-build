...
MCP (Model Context Protocol) servers extend Chutes Build with external tool integrations. They let Chutes Build interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to Chutes Build over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside Chutes Build's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.chutes-build/config.toml` under `[mcp_servers.<name>]` sections.
...
Chutes Build spawns a local process and communicates over stdin/stdout:
...
> `CHUTES_BUILD_MCP_STARTUP_TIMEOUT_SECS` (seconds). A per-server `startup_timeout_sec`
...
>
> - env `CHUTES_BUILD_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Chutes Build-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.chutes-build/config.toml`) **or repo-level**
>   (`.chutes-build/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.chutes-build/config.toml` >
...
chutes-build mcp list --json          # Machine-readable output
...
chutes-build mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
chutes-build mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
chutes-build mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
chutes-build mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
chutes-build mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
chutes-build mcp remove github

# Enable or disable a local/TOML (or compat-sourced) server
chutes-build mcp enable github
chutes-build mcp disable github

# Diagnose a server's configuration and connectivity
chutes-build mcp doctor               # Check every configured server
chutes-build mcp doctor github        # Check one server
chutes-build mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `chutes-build mcp add` writes to `~/.chutes-build/config.toml` (`--scope user`). Use `--scope project` to write to `.chutes-build/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `chutes-build mcp list` shows servers from both scopes, marking project-scoped ones with `(project)` and disabled ones with `(disabled)`.

`chutes-build mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.

`chutes-build mcp enable` / `disable` persist the personal on/off state to user `~/.chutes-build/config.toml` (`disabled_mcp_servers`, and `[mcp_servers.<name>].enabled` when that entry exists). Scope:

- **Known names:** user/project Chutes Build TOML, names already on the disabled list, compat sources (`.mcp.json`, Claude, Cursor), **plugin** MCP servers (same discovery as doctor/`/mcps`), and legacy managed `grok_com_*` (no local entry required).
...
MCP servers can be configured per-project by placing a `.chutes-build/config.toml` in your repository:
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. Chutes Build handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers Chutes Build's own OAuth client with the provider.

Chutes Build walks from the current directory up to the git repo root, loading `.chutes-build/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.chutes-build/config.toml` | All projects | Lowest |
| `<repo-root>/.chutes-build/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Chutes Build reads most other config sections only from `~/.chutes-build/config.toml`.
...
You can enable or disable MCP servers without restarting Chutes Build (TUI `/mcps` or CLI — see [CLI Management](#cli-management)).
...
Chutes Build loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native Chutes Build config | `~/.chutes-build/config.toml`, `.chutes-build/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.chutes-build/config.toml` or the corresponding environment variable (`CHUTES_BUILD_CURSOR_MCPS_ENABLED`, `CHUTES_BUILD_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `chutes-build inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, Chutes Build handles the credential flow automatically. When an MCP server requests OAuth credentials, Chutes Build opens a browser-based authorization flow and stores the resulting tokens for future use.
...
You must authenticate OAuth-based MCP servers before you can use them. Chutes Build stores the resulting tokens under `~/.chutes-build/mcp_credentials.json` as local plaintext with owner-only file permissions (`0600` on Unix). Prefer full-disk encryption on the host. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). Chutes Build expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
1. The parent session actually connected the server (see Extensions / `chutes-build inspect`)
...
For stdio servers, Chutes Build captures the process's standard error to `~/.chutes-build/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.chutes-build/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `chutes-build inspect` to see all loaded MCP servers and their sources:

```bash
chutes-build inspect          # Human-readable
chutes-build inspect --json   # Machine-readable
```
