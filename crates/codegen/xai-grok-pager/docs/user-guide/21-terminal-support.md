...
Chutes Build runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in Chutes Build to check the current session and see available fixes. If
Chutes Build cannot start, run `chutes-build doctor` in your shell. Use `chutes-build doctor --json`
...
`chutes-build doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `chutes-build doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. Chutes Build preserves the file's
...
Chutes Build deliberately does **not** run `tmux source-file` or change the live tmux
...
Chutes Build detects these terminal emulators from environment variables:
...
- **Chutes Build Desktop**
- **VTE**-based terminals such as GNOME Terminal, GNOME Console, and Tilix
- **Windows Terminal**

Detection has these limitations:

- Inside tmux, variables that identify the outer terminal may not reach Chutes Build.
...
Inside tmux there are two separate questions: what color Chutes Build emits, and what
...
Chutes Build writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when Chutes Build runs inside tmux.
...
terminal focused. Older compositors may require Chutes Build to remain focused until
the copy message appears. Chutes Build shows a startup warning when this applies; run
`/doctor` for the detected status and steps.

`CHUTES_BUILD_CLIPBOARD_NO_DATA_CONTROL=1` is an advanced fallback that disables the
data-control route. Copies then use command-line clipboard tools.

#### OSC 52 kill switch

Chutes Build emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `CHUTES_BUILD_CLIPBOARD_NO_OSC52=1` before starting Chutes Build to
...
A remote Chutes Build process normally cannot read the local terminal's selection. Use
...
When Chutes Build cannot identify the outer terminal over SSH, it predicts that OSC 52
...
clipboard. Each copy is still saved to a backup file (`~/.chutes-build/last-copy.txt` by
default; override with `CHUTES_BUILD_COPY_FILE`); the toast names that path when delivery
...
through `chutes-build wrap`, for example `chutes-build wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `chutes-build wrap`, Chutes Build shows the one-time tip
...
`[ui.contextual_hints]` in `$CHUTES_BUILD_HOME/config.toml`. This setting does not hide
the Doctor recommendation.

For repeated SSH use, Doctor offers `chutes-build doctor fix ssh-wrap`. It also shows
...
> **Warning**: `chutes-build wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. Chutes Build normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.chutes-build/pager.toml`, or run
`chutes-build --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with Chutes Build

Zellij can intercept Ctrl/Alt keys before they reach Chutes Build. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach Chutes Build, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in Chutes Build.
...
keys. Chutes Build therefore does not negotiate the protocol there, and Shift+Enter can
...
If Chutes Build stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, Chutes Build stops capture and shows
...
to the terminal hosting Chutes Build. Open **System Settings → Privacy & Security →
...
Run `chutes-build doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone Chutes Build would use. If no input device is available, Doctor
...
problem, set `CHUTES_BUILD_VOICE_CAPTURE=inprocess` to use the in-process fallback for
comparison.
