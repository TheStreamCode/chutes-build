...
Chutes Build CLI can export usage **metrics** and **events** to your organization's
...
any vendor-side telemetry. There is none in this build: product analytics are compiled out (`product::TELEMETRY`), so this OTEL export is the only usage stream Chutes Build produces, and it goes only where you point it.
...
|---------|---------------|
| Telemetry master switch | `[features] telemetry` / `CHUTES_BUILD_TELEMETRY_ENABLED` |
| Coding data, retention, and training | Settings — `/privacy` opens the row |
| Trace upload | `[telemetry] trace_upload` / `CHUTES_BUILD_TELEMETRY_TRACE_UPLOAD` |
| External OpenTelemetry | `CHUTES_BUILD_EXTERNAL_OTEL` / `[telemetry] otel_*` (this guide) |
...
- **Carries only what you configure**: its exporters send the headers you set and
  no credential of ours.
- **Independent of any data-retention setting**: it works even when
  `telemetry` is disabled and for ZDR (zero-data-retention) teams. Those
  those settings are off; the external stream is governed solely
  by your own OTEL configuration.

## Quick start

```bash
export CHUTES_BUILD_EXTERNAL_OTEL=1                  # master switch
...
`CHUTES_BUILD_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `CHUTES_BUILD_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `CHUTES_BUILD_TELEMETRY_ENABLED`, which controlled upstream's product analytics and has no effect here — that path is compiled out. |
...
> `CHUTES_BUILD_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`CHUTES_BUILD_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `chutes-build setup` managed config /
...
(`chutes-build --debug`) for `external otel:` lines — they record whether the stream
