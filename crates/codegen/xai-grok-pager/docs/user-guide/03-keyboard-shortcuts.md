...
Reference for key bindings in the Chutes Build TUI. Bindings are built in and cannot currently be remapped.

---

## Input Modes

Chutes Build has two input modes that control how you navigate the scrollback:
...
Simple mode is active by default. To switch to Vim mode, set `vim_mode = true` under `[ui]` in `~/.chutes-build/config.toml`, or toggle it at runtime with `/vim-mode`. See [Configuration](05-configuration.md) for details.
...
> in `~/.chutes-build/config.toml` (or `/vim-mode` to toggle). Arrow keys, `Tab`,
...
- An unmodified middle click in Chutes Build reads **PRIMARY**, the current mouse selection, only when `DISPLAY` is non-empty. Pure X11 can use its native reader fallback; XWayland requires `xclip` or `xsel` on `PATH` so Chutes Build reads the X11 selection rather than Wayland PRIMARY. The press is handled once; the release does not paste again.
...
Over SSH, the remote Chutes Build process usually cannot access the terminal's local X11 selection. Use terminal-native `Shift+Insert` or `Shift+middle click` so the local terminal sends the selected text through the PTY.
...
**VS Code family terminal** (VS Code, Cursor, Windsurf, Zed integrated terminals): `Ctrl+Q` is captured by the host, so Chutes Build makes **`Ctrl+D` the sole quit key** (`Ctrl+Q` is not bound). Half-page-down is rebound to bare **`Shift+D`**. Mid-turn interject uses **`Ctrl+L`** (no alternates) because `Ctrl+Enter` / `Ctrl+I` do not reliably reach the PTY; extensions are opened via `/plugins` instead of `Ctrl+L`.
...
> **Cmd+A is gated to Ghostty.** Chutes Build's in-app `Cmd+A` handler is only
