...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.chutes-build/agents/` or `~/.chutes-build/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.chutes-build/personas/`; applied during subagent resolution |
...
| **Examples** | `chutes-build`, `explore`, `plan` | `researcher`, `concise` |
...
Chutes Build discovers file-based personas from these locations, in priority order:

- `.chutes-build/personas/*.toml` (project)
- `~/.chutes-build/personas/*.toml` (user)
...
> **Note:** Chutes Build applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Chutes Build resolves the effective model and reasoning effort in this order, highest priority first:
...
Chutes Build manages worktrees through the `chutes.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
# instructions_file = ".chutes-build/personas/concise.md"  # or load from a file
```

Chutes Build also discovers roles from `.chutes-build/roles/*.toml` and personas from `.chutes-build/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Chutes Build shows running and finished work in side panes on the agent screen:
