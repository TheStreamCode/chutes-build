...
Control what Chutes Build can access and do: permission modes, allow/ask/deny rules, hooks, and the optional OS-level sandbox.

- **Modes** set how often Chutes Build asks for approval (always-approve, auto, ask, and related).
...
When Chutes Build edits a file, runs a command, or calls an external tool, it may pause for approval. Permission modes control how often that happens.
...
```bash
chutes-build -p "Run the tests" --always-approve
chutes-build agent --always-approve stdio
chutes-build agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
chutes-build --always-approve -p "Run the test suite"
chutes-build --permission-mode auto
chutes-build agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```toml
# project .chutes-build/config.toml
[ui]
...
```bash
chutes-build -p "Deploy the service" --always-approve --deny 'Bash(rm -rf *)'
```
...
Organizations can prevent always-approve from being enabled via CLI, TUI, or `/always-approve`. Set this in `requirements.toml` (user-level under `~/.chutes-build/`, or system-wide under `/etc/grok/` for enforcement users cannot remove):
...
Chutes Build can still load Claude-style permission **rules** from managed settings; always-approve is locked with `requirements.toml` as shown above.
...
Chutes Build reads permission rules from three compatible sources. Rules from all sources are merged into one set; a rule's effect depends on its action (`deny` > `ask` > `allow`), not on which file it came from.
...
|-------|------|-----------------------|
| Global (all projects) | `~/.chutes-build/config.toml` | No |
| Project (committed) | `<project>/.chutes-build/config.toml` | Yes (commit it) |
| Project (personal) | `<project>/.claude/settings.local.json` | No (gitignore it) |
| Interactive grants | Stored internally by Chutes Build, per project | No |

Notes on scoping:

- Chutes Build discovers a `.chutes-build/config.toml` at every directory level from the repository root down to your working directory, so a subdirectory can add rules on top of the repo root's.
...
- Chutes Build has no native `config.local.toml`. For personal, uncommitted rules in a project, use `.claude/settings.local.json`; Chutes Build reads it directly (see [Claude Code Compatibility](#3-claude-code-compatibility-claudesettingsjson)).
...
To stop prompts for a specific command in one project, add a narrow allow rule to that project's `.chutes-build/config.toml` (or `.claude/settings.json`):
...
```bash
chutes-build -p "Review the API changes" \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)' \
  --allow 'Read' \
  --allow 'Grep' \
...
### 2. Native Configuration (`~/.chutes-build/config.toml` and `.chutes-build/config.toml`)
...
Rules from the global `~/.chutes-build/config.toml` and every project `.chutes-build/config.toml` (from the repo root down to your working directory) are merged into one rule set, alongside any `.claude/settings.json` rules.

Managed configuration deployed by your organization also contributes `[permission]` rules: the system `/etc/grok/managed_config.toml`, and a user-level copy that Chutes Build maintains automatically at `~/.chutes-build/managed_config.toml`. Managed rules merge like rules from any other source, with two properties specific to managed `allow` rules: your own `deny` and `ask` rules win over a managed `allow` (severity ordering), and a catch-all managed `allow` is ignored when always-approve is locked off. For rules that users cannot edit away, use the root-owned system `/etc/grok/requirements.toml`.
...
Chutes Build reads `~/.claude/settings.json` and `~/.claude/settings.local.json`, plus the project-level `<project>/.claude/settings.json` and `settings.local.json` (walking up to the repo root). The native `.grok` source for permission rules is `config.toml`, described in the section above.
...
Supported `defaultMode` values include `default`, `auto`, `acceptEdits`, `bypassPermissions`, `dontAsk`, and `plan`. Chutes Build reads `defaultMode` from its canonical location under `permissions`; a top-level `defaultMode` is also accepted when the nested key is absent.
...
**Chained commands.** Chutes Build parses each command like a shell and splits it on `&&`, `||`, `;`, `|`, and newlines. The rule actions treat segments differently:
...
`MCPTool(...)` patterns match the full Chutes Build tool name in `server__tool` form, with glob support: `MCPTool(linear__*)` matches every tool from the `linear` server. Chutes Build tool names carry no `mcp__` prefix, so a rule written as `mcp__server__tool` never matches an MCP call; write `MCPTool(server__tool)` instead.
...
Interactive grants are stored in Chutes Build's own state directory under your home directory, scoped to the directory you launched Chutes Build from. A grant made in one project never applies in another, grants are not written into the repository, and they are not meant to be hand-edited.

Interactive grants are personal, per-machine state. For an allowlist you can review in code review and share with teammates, use declarative rules in the project's `.chutes-build/config.toml` instead.
...
**`~/.chutes-build/hooks/git-gh-only.json`**
...
**`~/.chutes-build/hooks/git-gh-only.sh`**
...
```bash
chmod +x ~/.chutes-build/hooks/git-gh-only.sh
```
...
```bash
chutes-build -p "Implement the feature using only git and GitHub CLI" \
  --allow 'Read' \
  --allow 'Grep' \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)'
...
3. **Review project configuration from unfamiliar sources.** Project permission rules in `.chutes-build/config.toml` and `.claude/settings.json`, including `allow` rules, apply without a separate trust prompt. Review them, and any project hooks, before working in an unfamiliar checkout (see the security notes in [10-hooks.md](10-hooks.md)).
