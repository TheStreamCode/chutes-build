...
Memory lets Chutes Build recall facts, decisions, and patterns from earlier sessions. Chutes Build indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each Chutes Build session starts fresh: the model knows nothing about previous sessions. When you enable memory, Chutes Build can:
...
3. `CHUTES_BUILD_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
|----------|-------|-------------|
| `~/.chutes-build/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.chutes-build/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.chutes-build/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |

Every one of those files is created owner-only — mode 0600 where the platform has
POSIX permissions — including the templates written on first run. Memory holds
whatever you asked the agent to remember, so it is not left readable by other
accounts on a shared machine. Known secret formats are also filtered out before
each write, but that is a second line of defence and not a reason to paste a
credential into a prompt.

Chutes Build suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
When a session ends, Chutes Build saves a structured metadata summary to that session's daily log. The summary contains:
...
Chutes Build builds the summary from conversation metadata without an LLM call, without added latency. Chutes Build skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask Chutes Build to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.chutes-build/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

Chutes Build records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, Chutes Build opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, Chutes Build shows `Memory saved to ~/.chutes-build/memory/MEMORY.md`.

### Forget

Ask Chutes Build to forget something, and it finds and removes the matching entry:

```
> forget the snake_case convention
```

Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.chutes-build/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.

### Recall

Ask what Chutes Build remembers:

```
> what do you remember?
```

Chutes Build searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.

### Direct Editing

You can edit memory files directly under `~/.chutes-build/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
When you save a note with `/remember`, Chutes Build confirms in the scrollback:

```
Memory saved to ~/.chutes-build/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what Chutes Build has stored.
...
Dream also runs automatically. By default, Chutes Build checks the consolidation gates when a session ends and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, Chutes Build automatically searches memory for content relevant to the current project and injects it as context. This means Chutes Build starts with knowledge from previous sessions without a reminder.
...
Chutes Build searches memory automatically, but you can also trigger searches manually in the chat:
...
The `chutes-build memory` command manages memory from the shell. It has one subcommand, `clear`:

```bash
# Clear workspace memory (MEMORY.md, sessions/, and index.sqlite). This is the default scope.
chutes-build memory clear

# The same scope, stated explicitly
chutes-build memory clear --workspace

# Clear the global MEMORY.md
chutes-build memory clear --global

# Clear both workspace and global memory
chutes-build memory clear --all

# Skip the confirmation prompt (-y is the short form)
chutes-build memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.chutes-build/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.chutes-build/memory/` for external edits and reindex |
...
| `min_score` | unset | Score threshold for first-turn results. When unset, Chutes Build applies no threshold, which is equivalent to `0.0`. |
...
| `flush_model` | unset | Model for the flush turn. When unset, Chutes Build uses the session's primary model. |
...
When a session memory is old, Chutes Build attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, Chutes Build watches `~/.chutes-build/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `chutes-build inspect` output.
2. Check the flag: `chutes-build --experimental-memory` or `CHUTES_BUILD_MEMORY=1`.
3. Check for `--no-memory` or `CHUTES_BUILD_MEMORY=0` overriding your config.
...
```bash
ls ~/.chutes-build/memory/
cat ~/.chutes-build/memory/MEMORY.md
$EDITOR ~/.chutes-build/memory/MEMORY.md
```

### Debug Logging

```bash
RUST_LOG=debug CHUTES_BUILD_LOG_FILE=/tmp/grok.log chutes-build
