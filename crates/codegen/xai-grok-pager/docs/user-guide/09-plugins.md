...
A plugin bundles skills, slash commands, agents, hooks, and MCP servers into one installable unit. You get plugins from a marketplace, install the ones you want, and Chutes Build loads what they add. To build and share your own, see [Create your own marketplace](#create-your-own-marketplace).
...
1. **Add the marketplace** so Chutes Build can show what it offers. Nothing installs yet.
...
```bash
chutes-build plugin marketplace add my-org/team-plugins                  # GitHub shorthand (owner/repo)
chutes-build plugin marketplace add https://gitlab.com/acme/plugins.git  # any git host, include https:// and .git
chutes-build plugin marketplace add ./my-marketplace                     # a local folder
```

List, refresh, and remove sources with `chutes-build plugin marketplace list`, `chutes-build plugin marketplace update [<name>]`, and `chutes-build plugin marketplace remove <url>`.
...
Place this file at `~/.chutes-build/settings.json` or `~/.claude/settings.json`.
...
```bash
chutes-build plugin install deploy-tools --trust
```
...
Run `chutes-build plugin install <source>` without `--trust` and Chutes Build shows the source, warns that installing activates the plugin's hooks, MCP servers, and skills, then stops. Add `--trust` to go ahead. Only install plugins from sources you trust (see [Trust and security](#trust-and-security)).

A plugin's skills appear in the slash menu. When a skill name is ambiguous, Chutes Build shows the qualified form prefixed by the plugin name, for example `/deploy-tools:release`. To pick up a newly installed plugin, press `r` in the Plugins tab or start a new session.
...
```bash
chutes-build plugin list [--json] [--available]   # installed plugins (--available requires --json)
chutes-build plugin uninstall <name> [--confirm] [--keep-data]   # aliases: rm, remove
chutes-build plugin update [<name>]               # omit the name to update every plugin
chutes-build plugin enable <name>
chutes-build plugin disable <name>
chutes-build plugin details <name>                # show the plugin's component inventory
```
...
Set these in `~/.chutes-build/config.toml`:
...
Plugins are off by default, so list one in `enabled` to turn it on, or in `disabled` to discover it but skip loading it. Each entry is a plain plugin name (from `chutes-build plugin list`) or a full ID (`<scope>/<hash>/<name>`).

To hide the plugins and hooks interface entirely, set `disable_plugins = true` in `~/.chutes-build/pager.toml`.
...
Enabling a plugin loads its skills, commands, and agents. Trust is separate and controls whether a plugin's code runs: even when enabled, its hooks, MCP servers, and LSP servers stay inactive until you trust it. Chutes Build trusts plugins in `~/.chutes-build/plugins/` automatically; project plugins in `.chutes-build/plugins/` require trust. Install with `--trust` to grant it:

```bash
chutes-build plugin install <source> --trust
```

Trusted plugin `.mcp.json` servers attach to the session like other MCP config, and child agents inherit them. Plugin agents (`plugin-name:agent-name`) use the parent session's MCP servers by default, the same as user agents under `~/.chutes-build/agents/`; restrict that with the `mcpInheritance` frontmatter (see [Subagents](16-subagents.md#mcp-inheritance)). For safety, plugin agent frontmatter cannot declare `mcpServers` or hooks, or set `permissionMode: bypassPermissions`.
...
3. **List the plugins in `.chutes-build-plugin/marketplace.json`.** This is the index Chutes Build reads.
...
my-org-plugins/
  .chutes-build-plugin/
    marketplace.json      # the index Chutes Build reads (required)
...
Chutes Build reads the index from `.chutes-build-plugin/marketplace.json`. It also accepts `.chutes-build-plugin/plugin.json` and the `.claude-plugin/` equivalents.
...
Validate a plugin before publishing with `chutes-build plugin validate [<path>]`, and tag a release from the manifest version with `chutes-build plugin tag [<path>] [--push]`. Then point people at the repository. They add it once and install the plugins they want:

```bash
chutes-build plugin marketplace add my-org/my-org-plugins   # GitHub shorthand, a git URL, or a local path
chutes-build plugin install gdrive --trust
```
...
# Plugins stay off until enabled. List plugin names (from `chutes-build plugin list`)
# or full IDs (`<scope>/<hash>/<name>`).
[plugins]
enabled = ["gdrive"]
```

For a hands-off install with no per-person step, also place the plugin's files where Chutes Build discovers and trusts them automatically: `~/.chutes-build/plugins/`, or a directory your device-management tool manages that you point to with `[plugins].paths`. Then enable them with `[plugins].enabled`.
...
You can also set `CHUTES_BUILD_MARKETPLACE_REQUIRE_SHA=1`. Both only tighten the policy; neither turns it back off. Publish `sha` values in your marketplace's `plugin-index.json` so installs from it satisfy the rule. Plugins vendored directly inside a marketplace repository are copied from that repository's checkout, so pin them the same way, with `sha` values in `plugin-index.json`.
...
Marketplaces distribute Chutes Build content: skills, commands, agents, hooks, and MCP server configurations. They do not install a program onto a machine. A skill or MCP server that runs a helper binary (for example a custom sign-in tool) still needs that binary delivered separately, bundled with your deployment or pushed through your device-management tool.

---

## Troubleshooting

**A plugin you installed isn't showing up.** Plugins are off until enabled. Check `chutes-build plugin list`, then add the plugin's name or ID to `[plugins].enabled`, or press `Space` on it in the Plugins tab. Reload with `r` in the Plugins tab or start a new session.

**A plugin's hooks or MCP servers don't run.** They stay inactive until the plugin is trusted. Reinstall with `--trust`, or place the plugin under `~/.chutes-build/plugins/` (auto-trusted). See [Trust and security](#trust-and-security).

**A skill or MCP server from a marketplace is missing.** Refresh the source with `chutes-build plugin marketplace update`, confirm the plugin is installed and enabled, and, if your organization restricts sources, check that the marketplace is still allowed (see [Distribute across an organization](#distribute-across-an-organization)). Some MCP servers require a sign-in and will not appear until you authenticate.
...
**See exactly what loaded.** Run `chutes-build inspect` (add `--json` for machine-readable output) to list every discovered plugin and the skills, agents, hooks, and MCP servers it provides, each labeled with its `plugin: <name>` source.
...
An optional `plugin.json` manifest can override paths or add metadata; without one, Chutes Build discovers components from these standard directories. For example, a `team-tools` plugin might bundle a deploy skill, a code-review agent, pre-commit hooks, and a Linear MCP server, installed together in one step.
...
### Where Chutes Build looks for plugins

Chutes Build discovers plugins from these locations, in priority order. The `.claude/plugins/` equivalents also work, and when two plugins share a name the higher-priority one wins:

| Location | Scope | Trust |
|----------|-------|-------|
| `_meta.pluginDirs` (`session/new` / `session/load`) | Session, that session only | Trusted automatically |
| `--plugin-dir` (the `chutes-build agent … stdio` flag) | Process, that agent process only | Trusted automatically |
| `.chutes-build/plugins/` | Project, shared through version control | Requires trust |
| `~/.chutes-build/plugins/` | User, every project | Trusted automatically |
| `[plugins].paths` (config) | Custom directories you add | Depends on location |

The `_meta.pluginDirs` field on the `session/new` and `session/load` requests loads plugins for a single session; because the caller supplies the directory, those plugins are trusted automatically and do not persist after the session. `--plugin-dir` is the process-wide equivalent for a dedicated `chutes-build agent … stdio` process, repeatable (`chutes-build agent --no-leader --plugin-dir A --plugin-dir B stdio`), and ignored in leader mode, where the shared leader discovers its own plugins.
...
|----------|-------------|
| `CHUTES_BUILD_PLUGIN_ROOT` | Absolute path to the plugin's installed directory. |
| `CHUTES_BUILD_PLUGIN_DATA` | Absolute path to the plugin's writable data directory, for state, caches, and logs. |

Chutes Build sets these and overrides any same-named value in the hook's `env` map (the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases are set too). See the [Hooks guide](10-hooks.md) for every variable passed to hooks.
