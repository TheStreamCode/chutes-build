...
Hooks let you run a script or send an HTTP request at key moments in a Chutes Build session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that Chutes Build calls when a specific lifecycle event occurs. Hooks can:
...
1. Create the hooks directory:
   ...
   ```sh
   mkdir -p ~/.chutes-build/hooks
   ```

2. Create a hook file, e.g. `~/.chutes-build/hooks/session-start.json`:
   ...
   ```json
   {
     "hooks": {
       "SessionStart": [
         {
           "hooks": [
             { "type": "command", "command": "echo 'Chutes Build session started in '$(pwd)" }
           ]
         }
       ]
     }
...
3. Start (or restart) a Chutes Build session. The hook runs automatically on `SessionStart`.
...
|-------|------|----------|-------|
| Global | `~/.chutes-build/hooks/*.json` | Always | Personal hooks |
...
| Project | `<project>/.chutes-build/hooks/*.json` | Requires trust | Per-repo automation |
...
| Config | `~/.chutes-build/config.toml` | Always | Your hooks alongside the rest of your config |
| Config | `managed_config.toml` (`$CHUTES_BUILD_HOME` and `/etc/grok`) | Always | Organization-distributed hooks (server-synced and on-device) |
...
Config-file hooks live in the same TOML your organization already controls; see [Hooks in Config Files](#hooks-in-config-files) for the format. The compatible vendor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.chutes-build/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run -- until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.chutes-build/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.chutes-build/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.

Because hooks are unified under folder-trust, a `--trust` / `/hooks-trust` grant trusts the whole folder for **MCP, LSP, and hooks** together, and cascades to subdirectories. Conversely, disabling folder-trust (`CHUTES_BUILD_FOLDER_TRUST=0` or `[folder_trust] enabled = false`) ungates project hooks along with MCP/LSP.
...
Chutes Build accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). Chutes Build skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, Chutes Build maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
Hooks can also live directly in your Chutes Build config, so a team can distribute them with the rest of their configuration instead of shipping separate JSON files. The same `hooks` object is read from three TOML files:

| File | Tier | Who sets it |
|------|------|-------------|
| `~/.chutes-build/config.toml` | User | You |
| `managed_config.toml` (`$CHUTES_BUILD_HOME`, `/etc/grok`) | Managed / system | Your organization |
...
Chutes Build sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
|-----------------------|-------------|
| `CHUTES_BUILD_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `CHUTES_BUILD_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `CHUTES_BUILD_SESSION_ID`     | The unique identifier of the current Chutes Build session. |
| `CHUTES_BUILD_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `CHUTES_BUILD_WORKSPACE_ROOT`, set for every hook. |
...
When a hook originates from a plugin, Chutes Build additionally injects the following variables:

| Variable             | Description |
|----------------------|-------------|
| `CHUTES_BUILD_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `CHUTES_BUILD_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`CHUTES_BUILD_PLUGIN_ROOT`, `CHUTES_BUILD_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
...
Press `r` in the Hooks tab to reload all hooks from disk. Chutes Build re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.chutes-build/hooks/`) run with your user permissions -- treat them like shell scripts.
...
5. **Version control project hooks** -- commit `.chutes-build/hooks/` (but never secrets).
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug CHUTES_BUILD_LOG_FILE=/tmp/grok.log grok`, then check `/tmp/grok.log`.
