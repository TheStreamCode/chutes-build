...
Chutes Build connects to custom model endpoints for alternative providers, self-hosted models, and overriding built-in settings. This guide explains how to select models, configure endpoints, and integrate third-party providers.
...
The catalog is live, so it changes without a release. `chutes-build models` lists
what your account can reach right now; see [Chutes Models and
Ecosystem](25-chutes-ecosystem.md) for how routing, reasoning modes, and fallback
interact.
...
```bash
chutes-build -p "Hello" -m grok-build
```
...
Set a persistent default in `~/.chutes-build/config.toml`:
...
Chutes Build supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
When you omit `api_backend`, Chutes Build uses `chat_completions`.

To send provider-specific authentication or version headers -- for example, Anthropic's `x-api-key` -- use the `extra_headers` field described below. Chutes Build sends those headers verbatim with every request to the endpoint.

---

## Configuring Custom Models

Add custom model endpoints in `~/.chutes-build/config.toml` under `[model.<name>]` sections:
...
env_key = "CHUTES_API_KEY"                   # Env var holding the API key (optional; string or array)
...
Chutes Build resolves the API key in this order:
...
3. Your signed-in session token (from `chutes-build login`), for a model with no `api_key`/`env_key` of its own
4. The `CHUTES_API_KEY` environment variable (global fallback; Chutes Build also accepts `CHUTES_BUILD_API_KEY` for backward compatibility)

### Context Window

The `context_window` value tells Chutes Build when to trigger auto-compaction. When you override a known model, Chutes Build inherits that model's context window. When you define a new model and omit `context_window`, Chutes Build defaults to 200,000 tokens, so set it explicitly to match your provider.
...
Some gateways route or version on the query string. `query_params` appends percent-encoded query parameters to every request Chutes Build makes for a model. For example, a gateway that selects an API version this way:
...
When you override a built-in model, Chutes Build starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which Chutes Build sends verbatim.
...
Point Chutes Build at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `CHUTES_BUILD_MODELS_BASE_URL` | Yes | Base URL for inference. Chutes Build fetches the model list from `{base_url}/models`. |
| `CHUTES_API_KEY` | Yes | API key sent as `Authorization: Bearer`. Chutes Build also accepts `CHUTES_BUILD_API_KEY`. |
| `CHUTES_BUILD_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you use `[endpoints]` with partial model overrides, Chutes Build inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, Chutes Build uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `chutes-build login` -- the API key is enough.
...
```toml
[models]
web_search = "model-router"
```

Or via environment variable:

```bash
export CHUTES_BUILD_WEB_SEARCH_MODEL="model-router"
```

If you point web search at a custom model, you also need a `[model.*]` entry so Chutes Build can reach it. Server-side ("backend") web search runs only when the model sets `supports_backend_search = true` (and the build enables backend search); it does not depend on `api_backend`:
...
```bash
RUST_LOG=debug CHUTES_BUILD_LOG_FILE=/tmp/grok.log grok
tail -f /tmp/grok.log
```
