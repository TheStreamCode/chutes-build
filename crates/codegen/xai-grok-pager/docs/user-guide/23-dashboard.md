...
- **`chutes-build dashboard`** — launch the TUI into the dashboard.
- **`/dashboard`** (aliases **`/agents-dashboard`**, **`/sessions`**) — open
  from inside a session.
- **`Ctrl+\`** — same view as the slash command.

Hidden in minimal mode. Set `CHUTES_BUILD_AGENT_DASHBOARD=0` or
...
```
 Chutes Build · Dashboard — 4 agents · 2 awaiting
...
State icons match other session lists in Chutes Build:
...
Per-user preferences under `[dashboard]` in `~/.chutes-build/config.toml`:
