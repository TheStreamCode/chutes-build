...
Headless mode runs Chutes Build non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
Chutes Build processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
On failure, Chutes Build emits an error object (process exit non-zero). Prompt-level
...
Chutes Build may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.
...
The `system`/`init` and terminal `result` lines carry metadata. Chutes Build emits the fields it has real data for and omits pure-placeholder fields it cannot fill, rather than zero-filling them. As a result, those two lines may not pass strict `init`/`result` schema validation. The individual fields are listed below. Read the fidelity notes before treating any one field as authoritative. For a clean xAI-native stream with no placeholder shape, use `streaming-json`.
...
- `apiKeySource` is `user` for API-key auth and `oauth` otherwise. Chutes Build does not distinguish the schema's `project`, `org`, and `temporary` sources.
- `permissionMode` is the effective headless mode mapped to the Messages enum: the `--permission-mode` value, or `bypassPermissions` under `--yolo`, else `default`. Chutes Build-only modes such as `auto` collapse to `default`.
...
Chutes Build omits the schema's pure-placeholder `init` fields it has no data for, rather than emitting dummy values: `claude_code_version`, `output_style`, and `plugins`.

`result` includes `duration_ms`, `duration_api_ms`, `num_turns`, `stop_reason`, `total_cost_usd`, `usage` (Messages API `message.usage` shape), and `modelUsage`. It also includes `errors[]` on the error subtypes. Chutes Build omits the schema's always-empty `permission_denials`, because it does not collect permission denials. `structured_output` (with `--json-schema`) is snake_case, matching the schema.
...
`result.usage` reports the Messages `message.usage` shape with the three token buckets disjoint: `input_tokens` (uncached), `cache_read_input_tokens`, and `cache_creation_input_tokens`. Chutes Build derives these from the turn's aggregate ledger, reshaped into those buckets. Subagent cache creation is included in `cache_creation_input_tokens`. The aggregate ledger tracks it as its own bucket, so it is no longer folded into `input_tokens`.
...
Like `streaming-json`, this stream is read only. Tool approvals and other bidirectional flows use the ACP interface (`chutes-build agent`).

---

## Session Management in Headless Mode

By default, each `chutes-build -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
chutes-build -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
chutes-build -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
chutes-build -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
chutes-build -p "What's the secret number?" --resume abc123
```
...
chutes-build -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
chutes-build --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  chutes-build -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
Chutes Build's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(chutes-build -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
```bash
chutes-build -p "Format all files" --always-approve
chutes-build -p "Run the tests and fix any failures" --cwd ~/projects/my-app --always-approve
```
...
| ------------------------------- | ------------------------------------------------------------- |
| `CHUTES_API_KEY`        | API key for authentication (required when no browser login)   |
| `CHUTES_BUILD_HOME`                    | Override config directory (default: `~/.chutes-build`)                |
| `CHUTES_BUILD_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
| `RUST_LOG`                     | Log level filter (e.g. `debug`). Headless logs to stderr.     |

For CI environments without browser access, set `CHUTES_API_KEY` with an API key from [console.x.ai](https://console.x.ai):

```bash
export CHUTES_API_KEY="cpk_..."
chutes-build -p "Run the test suite" --yolo
```
...
- **`CHUTES_API_KEY`**: simplest for CI. See [Environment Variables](#environment-variables-for-headless) above.
- **`chutes-build login --device-auth`** (or `--device-code`): no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`chutes-build login`**: browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `chutes-build -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug chutes-build -p "..." 2> debug.log`.

---

## Project Root Discovery

When Chutes Build starts, it discovers the project root by walking upward from `--cwd`
...
Chutes Build discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
Chutes Build stores data in `~/.chutes-build` (override with `CHUTES_BUILD_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
### Read-Only `~/.chutes-build`

For containers or CI, mount `~/.chutes-build` read-only:

- Pre-populate `auth.json` or use `CHUTES_API_KEY`
- Session persistence fails silently (ephemeral)
- Update checks log a warning and skip

```bash
export CHUTES_API_KEY="cpk_..."
export CHUTES_BUILD_DISABLE_AUTOUPDATER=1
chutes-build -p "..." --no-auto-update
```
...
| `CHUTES_BUILD_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`CHUTES_BUILD_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `CHUTES_BUILD_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$CHUTES_BUILD_HOME/bin/grok`).
...
- Resume: `chutes-build -p "continue" --resume "<id>"` or `chutes-build -p "continue" --continue`
