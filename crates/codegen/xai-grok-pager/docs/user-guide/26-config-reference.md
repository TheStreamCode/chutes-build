...
This file ships with the CLI and is extracted to `~/.chutes-build/docs/user-guide/26-config-reference.md` on launch. It is the complete field list for `config.toml`, `managed_config.toml`, and `requirements.toml`. For conceptual guidance see [05-configuration.md](05-configuration.md).

## How to configure

Three files configure Chutes Build, and they are written by different people.

| File | Who writes it | Where it lives | Use it to |
| --- | --- | --- | --- |
| `config.toml` | The developer | `~/.chutes-build/config.toml`, and `.chutes-build/config.toml` in a project | Set personal defaults. Anything here can be changed by the person using the machine. |
...
Chutes Build also reads these layers, later rows winning except where a requirements pin or the Managed column says otherwise.

1. Compiled defaults.
2. `/etc/grok/managed_config.toml`, then `$CHUTES_BUILD_HOME/managed_config.toml` (fleet defaults; console-synced).
3. `$CHUTES_BUILD_HOME/config.toml` (your settings; `/settings` writes here). Default `$CHUTES_BUILD_HOME` is `~/.chutes-build`.
4. Project `.chutes-build/config.toml`: only `[mcp_servers]`, `[plugins]`, `[permission]`, and `[mcp] max_output_bytes`.
5. `CHUTES_BUILD_CONFIG` (inline JSON) or `CHUTES_BUILD_CONFIG_PATH` (JSON or TOML file). Allowlisted keys only.
6. `$CHUTES_BUILD_HOME/requirements.toml`, then `/etc/grok/requirements.toml`, then macOS MDM `ai.x.chutes-build`. Admin layer. Keys marked `pin` in the table cannot be overridden; keys marked `yes` are also valid in this file.
7. `GROK_*` environment variables.
8. CLI flags such as `--model`, `--sandbox`, `--yolo`.

Run `chutes-build inspect` or `chutes-build inspect --json` to see which files and values won.
...
| `agent.name` | `string` | `yes` | `user` | Built-in or discovered agent definition name. Also CHUTES_BUILD_AGENT and `--agent-profile`. |
...
| `auth.auth_provider_command` | `string` | `yes` | `user` | External auth binary; stdout is the token. Also CHUTES_BUILD_AUTH_PROVIDER_COMMAND; also valid as `grok_com_config.auth_provider_command`. |
| `auth.auth_provider_label` | `string` | `yes` | `user` | Login button label for an external auth provider. Also CHUTES_BUILD_AUTH_PROVIDER_LABEL; also valid as `grok_com_config.auth_provider_label`. |
| `auth.auth_token_ttl` | `number` | `yes` | `user` | Token TTL in seconds for providers that return a bare token. Also CHUTES_BUILD_AUTH_TOKEN_TTL; also valid as `grok_com_config.auth_token_ttl`. |
| `auth.disable_api_key_auth` | `boolean` | `pin` | `user` | Refuse API-key auth so only the deployment IdP can log in. Also CHUTES_BUILD_DISABLE_API_KEY_AUTH; also valid as `grok_com_config.disable_api_key_auth`. |
| `auth.force_login_team_uuid` | `string / string[]` | `pin` | `user` | Require login to this team UUID, or any of an array; empty array fails closed. Also CHUTES_BUILD_FORCE_LOGIN_TEAM_ID; also valid as `grok_com_config.force_login_team_uuid`. |
| `auth.grok_ws_origin` | `string` | `yes` | `user` | Websocket origin for grok.com. Also CHUTES_BUILD_WS_ORIGIN; also valid as `grok_com_config.grok_ws_origin`. |
| `auth.grok_ws_url` | `string` | `yes` | `user` | Relay websocket URL. Also CHUTES_BUILD_WS_URL; also valid as `grok_com_config.grok_ws_url`. |
| `auth.oauth2` | `table` | `yes` | `user` | OAuth2 provider used when enterprise OIDC is unset; also valid as `grok_com_config.oauth2`. |
| `auth.oauth2.client_id` | `string` | `yes` | `user` | OAuth2 client id. Also CHUTES_BUILD_OAUTH2_CLIENT_ID; also valid as `grok_com_config.oauth2.client_id`. |
| `auth.oauth2.issuer` | `string` | `yes` | `user` | OAuth2 issuer URL. Also CHUTES_BUILD_OAUTH2_ISSUER; also valid as `grok_com_config.oauth2.issuer`. |
| `auth.oauth2.principal_id` | `string` | `yes` | `user` | Required principal id when `principal_type` is set. Also CHUTES_BUILD_OAUTH2_PRINCIPAL_ID; also valid as `grok_com_config.oauth2.principal_id`. |
| `auth.oauth2.principal_type` | `string` | `yes` | `user` | Token principal type, such as Team. Also CHUTES_BUILD_OAUTH2_PRINCIPAL_TYPE; also valid as `grok_com_config.oauth2.principal_type`. |
| `auth.oauth2.referrer` | `string` | `yes` | `user` | Referrer for OAuth usage attribution. Also CHUTES_BUILD_OAUTH2_REFERRER; also valid as `grok_com_config.oauth2.referrer`. |
| `auth.oauth2.scopes` | `string[]` | `yes` | `user` | OAuth2 scopes. Also CHUTES_BUILD_OAUTH2_SCOPES; also valid as `grok_com_config.oauth2.scopes`. |
| `auth.oidc` | `table` | `yes` | `user` | Customer OIDC identity-provider settings; also valid as `grok_com_config.oidc`. |
| `auth.oidc.audience` | `string` | `yes` | `user` | Optional OIDC audience. Also CHUTES_BUILD_OIDC_AUDIENCE; also valid as `grok_com_config.oidc.audience`. |
| `auth.oidc.client_id` | `string` | `yes` | `user` | OIDC client id. Also CHUTES_BUILD_OIDC_CLIENT_ID; also valid as `grok_com_config.oidc.client_id`. |
| `auth.oidc.issuer` | `string` | `yes` | `user` | OIDC issuer URL. Also CHUTES_BUILD_OIDC_ISSUER; also valid as `grok_com_config.oidc.issuer`. |
| `auth.oidc.scopes` | `string[]` | `yes` | `user` | OIDC scopes. Also CHUTES_BUILD_OIDC_SCOPES; also valid as `grok_com_config.oidc.scopes`. |
...
| `cli.required_maximum_version` | `string` | `pin` | `user` | Hard maximum CLI version. Also CHUTES_BUILD_REQUIRED_MAXIMUM_VERSION. |
| `cli.required_minimum_version` | `string` | `pin` | `user` | Hard minimum CLI version. Also CHUTES_BUILD_REQUIRED_MINIMUM_VERSION. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `compat.claude.agents` | `boolean` | `yes` | `user` | Scan CLAUDE.md. Also CHUTES_BUILD_CLAUDE_AGENTS_ENABLED. |
| `compat.claude.hooks` | `boolean` | `yes` | `user` | Scan Claude hooks. Also CHUTES_BUILD_CLAUDE_HOOKS_ENABLED. |
| `compat.claude.mcps` | `boolean` | `yes` | `user` | Scan Claude MCP config. Also CHUTES_BUILD_CLAUDE_MCPS_ENABLED. |
| `compat.claude.rules` | `boolean` | `yes` | `user` | Scan Claude rules. Also CHUTES_BUILD_CLAUDE_RULES_ENABLED. |
| `compat.claude.skills` | `boolean` | `yes` | `user` | Scan Claude skills. Also CHUTES_BUILD_CLAUDE_SKILLS_ENABLED. |
...
| `compat.cursor.agents` | `boolean` | `yes` | `user` | Scan agent definitions from Cursor compat sources. Also CHUTES_BUILD_CURSOR_AGENTS_ENABLED. |
| `compat.cursor.hooks` | `boolean` | `yes` | `user` | Scan Cursor hooks. Also CHUTES_BUILD_CURSOR_HOOKS_ENABLED. |
| `compat.cursor.mcps` | `boolean` | `yes` | `user` | Scan Cursor mcp.json. Also CHUTES_BUILD_CURSOR_MCPS_ENABLED. |
| `compat.cursor.rules` | `boolean` | `yes` | `user` | Scan `.cursor/rules/`. Also CHUTES_BUILD_CURSOR_RULES_ENABLED. |
| `compat.cursor.skills` | `boolean` | `yes` | `user` | Scan Cursor skills directories. Also CHUTES_BUILD_CURSOR_SKILLS_ENABLED. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `diagnostics.crash_handler` | `boolean` | `yes` | `user` | Write a panic report under `$CHUTES_BUILD_HOME/crash/`. Also CHUTES_BUILD_CRASH_HANDLER. |
...
| `endpoints.deployment_key` | `string` | `pin` | `user` | Management key for enterprise deployments. Also CHUTES_BUILD_DEPLOYMENT_KEY. |
| `endpoints.feedback_base_url` | `string` | `yes` | `user` | Where feedback submissions go. Also CHUTES_BUILD_FEEDBACK_BASE_URL. |
| `endpoints.managed_config_url` | `string` | `yes` | `user` | Override managed config endpoint. Also CHUTES_BUILD_MANAGED_CONFIG_URL. |
| `endpoints.models_base_url` | `string` | `pin` | `user` | Custom inference base URL. Also CHUTES_BUILD_MODELS_BASE_URL. |
| `endpoints.models_list_url` | `string` | `pin` | `user` | Override model-list URL. Also CHUTES_BUILD_MODELS_LIST_URL. Alias `models_endpoint`. |
| `endpoints.trace_upload_bucket` | `string` | `yes` | `user` | Direct gs:// or s3:// bucket for traces; bypasses the proxy. Also CHUTES_BUILD_TRACE_UPLOAD_BUCKET. |
...
| `endpoints.trace_upload_credentials_file` | `string (path)` | `yes` | `user` | Path to a GCS service-account JSON or AWS credentials file for that bucket. Also CHUTES_BUILD_TRACE_UPLOAD_CREDENTIALS_FILE. |
| `endpoints.trace_upload_endpoint_url` | `string` | `yes` | `user` | Custom S3-compatible endpoint for s3:// bucket uploads. Also CHUTES_BUILD_TRACE_UPLOAD_ENDPOINT_URL. |
| `endpoints.trace_upload_region` | `string` | `yes` | `user` | AWS region for s3:// bucket uploads; default us-east-1. Also CHUTES_BUILD_TRACE_UPLOAD_REGION. |
| `endpoints.trace_upload_url` | `string` | `pin` | `user` | Proxy destination for traces when no direct bucket is set. Also CHUTES_BUILD_TRACE_UPLOAD_URL. |
| `endpoints.xai_api_base_url` | `string` | `pin` | `user` | Public xAI API base. Also CHUTES_BUILD_XAI_API_BASE_URL. |

### `features`

| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `features.active_agent_messages` | `boolean` | `pin` | `user` | Enable or disable `active_agent_messages`. Default false. Also `CHUTES_BUILD_ACTIVE_AGENT_MESSAGES`. |
| `features.ask_user_question` | `boolean` | `pin` | `user` | Enable or disable `ask_user_question`. Default true. Also `CHUTES_BUILD_ASK_USER_QUESTION`. |
| `features.auto_wake` | `boolean` | `pin` | `user` | Enable or disable `auto_wake`. Default true. Also `CHUTES_BUILD_AUTO_WAKE`. |
| `features.backend_tools` | `boolean` | `pin` | `user` | Enable or disable `backend_tools`. Default true. Also `CHUTES_BUILD_BACKEND_SEARCH`. |
| `features.campaigns` | `boolean` | `yes` | `user` | Enable remote campaign patches. `CHUTES_BUILD_CAMPAIGNS=0` still disables even when requirements set this true. |
| `features.cancel_rewind` | `boolean` | `pin` | `user` | Enable or disable `cancel_rewind`. Default true. Also `CHUTES_BUILD_CANCEL_REWIND`. |
...
| `features.compaction_detail` | `none / minimal / balanced / verbose` | `yes` | `user` | Verbatim detail level for `segments` compaction. Also CHUTES_BUILD_COMPACTION_DETAIL. |
| `features.compaction_mode` | `summary / transcript / segments` | `yes` | `user` | Compaction strategy. Also CHUTES_BUILD_COMPACTION_MODE. |
| `features.compaction_tool_choice` | `string` | `yes` | `user` | Tool-choice hint used during compaction. |
| `features.compaction_verbatim_input` | `boolean` | `pin` | `user` | Enable or disable `compaction_verbatim_input`. Default true. Also `CHUTES_BUILD_COMPACTION_VERBATIM_INPUT`. |
| `features.feedback` | `boolean` | `pin` | `user` | Enable or disable `feedback`. Default true. Also `CHUTES_BUILD_FEEDBACK_ENABLED`. |
| `features.feedback_trace_card` | `boolean` | `pin` | `user` | Show a trace-upload consent question after `/feedback`. Default false. Also `CHUTES_BUILD_FEEDBACK_TRACE_CARD`. |
...
| `features.lsp_tools` | `boolean` | `pin` | `user` | Enable or disable `lsp_tools`. Default false. Also `CHUTES_BUILD_LSP_TOOLS`. |
| `features.managed_config` | `boolean` | `yes` | `user` | Fetch managed_config.toml and requirements.toml from the deployment. |
| `features.mcp_auto_restart` | `boolean` | `yes` | `user` | Auto-restart stdio MCP servers after transport failure. Also CHUTES_BUILD_MCP_AUTO_RESTART. |
...
| `features.subagent_worktree_snapshot` | `boolean` | `pin` | `user` | Enable or disable `subagent_worktree_snapshot`. Default false. Also `CHUTES_BUILD_SUBAGENT_WORKTREE_SNAPSHOT`. |
...
| `features.title_refresh` | `boolean` | `pin` | `user` | Early-session auto-title refresh. Pin this in requirements to beat CHUTES_BUILD_TITLE_REFRESH. |
| `features.turn_summary` | `boolean` | `pin` | `user` | Enable or disable `turn_summary`. Default true. Also `CHUTES_BUILD_TURN_SUMMARY`. |
| `features.two_pass_compaction` | `boolean` | `pin` | `user` | Enable or disable `two_pass_compaction`. Default true. Also `CHUTES_BUILD_TWO_PASS_COMPACTION`. |
| `features.video_gen` | `boolean` | `pin` | `user` | Enable video tools / `/imagine-video`. |
| `features.voice_mode` | `boolean` | `pin` | `user` | Enable or disable `voice_mode`. Default true. Also `CHUTES_BUILD_VOICE_MODE`. |
| `features.web_fetch` | `boolean` | `pin` | `user` | Enable or disable `web_fetch`. Default false. Also `CHUTES_BUILD_WEB_FETCH`. |
| `features.write_file` | `boolean` | `pin` | `user` | Enable or disable `write_file`. Default true. Also `CHUTES_BUILD_WRITE_FILE`. |
| `features.zdr_access_enabled` | `boolean` | `pin` | `user` | Advertise ZDR-incompatible tools when the team is on Zero Data Retention. Also `CHUTES_BUILD_ZDR_ACCESS_ENABLED`. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `grok_com_config` | `table` | `yes` | `user` | Chutes Build.com websocket and OAuth/OIDC settings. `[auth]` is an alias. |
| `grok_com_config.auth_provider_command` | `string` | `yes` | `user` | External auth binary; stdout is the token. Also CHUTES_BUILD_AUTH_PROVIDER_COMMAND. |
| `grok_com_config.auth_provider_label` | `string` | `yes` | `user` | Login button label for an external auth provider. Also CHUTES_BUILD_AUTH_PROVIDER_LABEL. |
| `grok_com_config.auth_token_ttl` | `number` | `yes` | `user` | Token TTL in seconds for providers that return a bare token. Also CHUTES_BUILD_AUTH_TOKEN_TTL. |
| `grok_com_config.disable_api_key_auth` | `boolean` | `pin` | `user` | Refuse API-key auth so only the deployment IdP can log in. Also CHUTES_BUILD_DISABLE_API_KEY_AUTH. |
| `grok_com_config.force_login_team_uuid` | `string / string[]` | `pin` | `user` | Require login to this team UUID, or any of an array; empty array fails closed. Also CHUTES_BUILD_FORCE_LOGIN_TEAM_ID. |
| `grok_com_config.grok_ws_origin` | `string` | `yes` | `user` | Websocket origin for grok.com. Also CHUTES_BUILD_WS_ORIGIN. |
| `grok_com_config.grok_ws_url` | `string` | `yes` | `user` | Relay websocket URL. Also CHUTES_BUILD_WS_URL. |
| `grok_com_config.oauth2` | `table` | `yes` | `user` | OAuth2 provider used when enterprise OIDC is unset. |
| `grok_com_config.oauth2.client_id` | `string` | `yes` | `user` | OAuth2 client id. Also CHUTES_BUILD_OAUTH2_CLIENT_ID. |
| `grok_com_config.oauth2.issuer` | `string` | `yes` | `user` | OAuth2 issuer URL. Also CHUTES_BUILD_OAUTH2_ISSUER. |
| `grok_com_config.oauth2.principal_id` | `string` | `yes` | `user` | Required principal id when `principal_type` is set. Also CHUTES_BUILD_OAUTH2_PRINCIPAL_ID. |
| `grok_com_config.oauth2.principal_type` | `string` | `yes` | `user` | Token principal type, such as Team. Also CHUTES_BUILD_OAUTH2_PRINCIPAL_TYPE. |
| `grok_com_config.oauth2.referrer` | `string` | `yes` | `user` | Referrer for OAuth usage attribution. Also CHUTES_BUILD_OAUTH2_REFERRER. |
| `grok_com_config.oauth2.scopes` | `string[]` | `yes` | `user` | OAuth2 scopes. Also CHUTES_BUILD_OAUTH2_SCOPES. |
| `grok_com_config.oidc` | `table` | `yes` | `user` | Customer OIDC identity-provider settings. |
| `grok_com_config.oidc.audience` | `string` | `yes` | `user` | Optional OIDC audience. Also CHUTES_BUILD_OIDC_AUDIENCE. |
| `grok_com_config.oidc.client_id` | `string` | `yes` | `user` | OIDC client id. Also CHUTES_BUILD_OIDC_CLIENT_ID. |
| `grok_com_config.oidc.issuer` | `string` | `yes` | `user` | OIDC issuer URL. Also CHUTES_BUILD_OIDC_ISSUER. |
| `grok_com_config.oidc.scopes` | `string[]` | `yes` | `user` | OIDC scopes. Also CHUTES_BUILD_OIDC_SCOPES. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `managed_mcps.enabled` | `boolean` | `pin` | `user` | Fetch managed MCP configs at startup. Also CHUTES_BUILD_MANAGED_MCPS_ENABLED. |
| `managed_mcps.gateway_tools_enabled` | `boolean` | `yes` | `user` | Expose managed MCP gateway tools. Also CHUTES_BUILD_MANAGED_MCP_GATEWAY_TOOLS_ENABLED. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `memory.enabled` | `boolean` | `pin` | `user` | Cross-session memory master switch. Also CHUTES_BUILD_MEMORY. |
...
| `model.<id>.api_base_url` | `string` | `yes` | `user` | Alternate API base used with CHUTES_API_KEY resolution. |
...
| `model.<id>.supports_backend_search` | `boolean` | `yes` | `user` | Whether the endpoint supports Chutes Build-hosted server-side search tools. |
...
| `models.default` | `string` | `pin` | `user` | Model used for new sessions. Also `CHUTES_BUILD_DEFAULT_MODEL`, `--model`, `-m`. |
...
| `models.web_search` | `string` | `pin` | `user` | Model used by the client `web_search` tool. Also `CHUTES_BUILD_WEB_SEARCH_MODEL`. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `sandbox.auto_allow_bash` | `boolean` | `pin` | `user` | Skip bash permission prompts when a sandbox profile is active. Also CHUTES_BUILD_SANDBOX_AUTO_ALLOW_BASH. |
| `sandbox.profile` | `off / workspace / read-only / strict / string` | `pin` | `user` | Filesystem sandbox profile. Also `--sandbox` and CHUTES_BUILD_SANDBOX. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `subagents.enabled` | `boolean` | `pin` | `user` | Subagent / task tool master switch. Also CHUTES_BUILD_SUBAGENTS. |
...
| `subagents.max_depth` | `integer` | `yes` | `user` | Max nested subagent depth (clamped 1). |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `telemetry.otel_enabled` | `boolean` | `yes` | `user` | External OTEL master switch. Also CHUTES_BUILD_EXTERNAL_OTEL. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `tools.disable_zdr_incompatible_tools` | `boolean` | `yes` | `user` | Restrict tools that need xAI-hosted output under ZDR. Also CHUTES_BUILD_DISABLE_ZDR_INCOMPATIBLE_TOOLS. |
| `tools.media_gen.max_parallel_image_gen_calls` | `integer` | `yes` | `user` | Cap parallel image_gen/image_edit calls in one model step. Also CHUTES_BUILD_MAX_PARALLEL_IMAGE_GEN_CALLS. |
| `tools.media_gen.max_parallel_video_gen_calls` | `integer` | `yes` | `user` | Cap parallel video_gen calls in one model step. Also CHUTES_BUILD_MAX_PARALLEL_VIDEO_GEN_CALLS. |
| `tools.respect_gitignore` | `boolean` | `pin` | `user` | When true, search and read tools skip gitignored files. Also CHUTES_BUILD_RESPECT_GITIGNORE. |
...
| `toolset.web_fetch.proxy_endpoint` | `string` | `yes` | `user` | Egress proxy URL for web_fetch. Also CHUTES_BUILD_WEB_FETCH_PROXY. |
...
| `ui.collapsed_edit_blocks` | `boolean` | `yes` | `user` | Show edits as one-line +N/-M summaries. Also CHUTES_BUILD_COLLAPSED_EDIT_BLOCKS. |
...
| `ui.contextual_hints.ssh_wrap` | `boolean` | `yes` | `user` | Recommend `chutes-build wrap` when SSH lacks a clipboard sink. |
...
| `ui.default_selected_permission` | `string` | `yes` | `user` | Preselected approval row on the first prompt of a session. Also CHUTES_BUILD_DEFAULT_SELECTED_PERMISSION. |
| `ui.display_refresh.auto_cadence_enabled` | `boolean` | `yes` | `user` | Match stream/scroll cadence to display refresh rate. Also CHUTES_BUILD_DISPLAY_REFRESH_AUTO_CADENCE. |
...
| `ui.group_tool_verbs` | `boolean` | `yes` | `user` | Fold consecutive read/search/list tool rows. Also CHUTES_BUILD_GROUP_TOOL_VERBS. |
...
| `ui.prompt_suggestions` | `boolean` | `yes` | `user` | Next-prompt ghost text after each turn. Also CHUTES_BUILD_PROMPT_SUGGESTIONS. |
...
| `ui.theme` | `string` | `yes` | `user` | Color theme name, or `auto`/`system` to follow the OS. Also `/theme` and CHUTES_BUILD_THEME. |
...
`requirements.toml` is an admin-enforced file. Locations: `$CHUTES_BUILD_HOME/requirements.toml` (signed cache) then `/etc/grok/requirements.toml`, then macOS MDM `ai.x.chutes-build`. The **Requirements** column on the `config.toml` tables lists every `config.toml` key this file accepts (`pin` or `yes`). Omitted keys stay unconstrained.
...
| Situation | What Chutes Build does |
| --- | --- |
| A developer sets a key you pinned | The pinned value applies. `chutes-build inspect` lists the requirements file that contributed. |
...
| `requirements.toml` is missing or its signature does not verify | The pins do not apply, and Chutes Build starts without them. Set `fail_closed = true` to refuse to start instead. |
...
Run `chutes-build inspect` on the developer's machine. It lists every config file that contributed, including requirements and managed layers, so a policy that is not applying is visible in one command.
