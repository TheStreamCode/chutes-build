...
Chutes Build asks before doing anything risky — and can plan before it codes.

## Permissions

When Chutes Build wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: Chutes Build explores the
...
`s` requests changes — Chutes Build iterates until you're happy, then implements.
...
background — Chutes Build keeps working and you're notified when it finishes
