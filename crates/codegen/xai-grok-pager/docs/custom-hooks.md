...
Hooks let you run custom scripts or HTTP requests at key moments during a Chutes Build session — for example, before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
2. Create a simple hook file, e.g. `~/.chutes-build/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.chutes-build/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.chutes-build/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match Chutes Build's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
Chutes Build injects the following variables into every hook process:

- `CHUTES_BUILD_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `CHUTES_BUILD_HOOK_NAME` — the full configured name of this hook
- `CHUTES_BUILD_SESSION_ID` — the current session identifier
- `CHUTES_BUILD_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `CHUTES_BUILD_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `CHUTES_BUILD_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `CHUTES_BUILD_PLUGIN_ROOT` and `CHUTES_BUILD_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`CHUTES_BUILD_PLUGIN_ROOT` and `CHUTES_BUILD_PLUGIN_DATA`. These keys override any user-declared
...
2. The current process environment (the env Chutes Build itself sees).
...
`${CHUTES_BUILD_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.chutes-build/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.chutes-build/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.chutes-build/logs`).
...
Copy them to `~/.chutes-build/hooks/` and customize.
