...
in `.chutes-build/agents/` (project-level) or `~/.chutes-build/agents/` (user-level).
...
let def = AgentDefinition::from_file(".chutes-build/agents/code-reviewer.md")?;
...
// Find all .md files in .chutes-build/agents/ directories
...
1. **Project-level** (highest priority): `.chutes-build/agents/*.md` — walk
...
|---|---|---|
| `chutes-build` | extend | Default agent for software engineering tasks |
