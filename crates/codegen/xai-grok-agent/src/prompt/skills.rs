...
///
/// Priority order: Local (cwd/.chutes-build/skills, cwd/.agents/skills, cwd/.claude/skills) → Intermediate dirs →
/// Repo (repo_root/.chutes-build/skills, repo_root/.agents/skills, repo_root/.claude/skills) → User (~/.chutes-build/skills, ~/.agents/skills, ~/.claude/skills)
/// → additional paths from `config.paths`
/// → Server (injected `config.server_skill_dirs`)
/// → Bundled (injected `config.bundled_skill_dirs` + `~/.chutes-build/bundled`; lowest precedence).
///
...
/// (e.g. `Repo` for `.chutes-build/plugins/`). Native skills always win bare-name
...
fn scope_for_config_dir(dir: &Path, cwd: Option<&Path>, git_root: Option<&Path>) -> SkillScope {
    // Home-level dirs (e.g. ~/.chutes-build/, ~/.agents/, ~/.claude/) are User scope.
    #[allow(deprecated)]
    if let Some(home) = std::env::home_dir()
...
mod tests {
    ...
    async fn server_skills_discovered_and_shadowed_by_local() {
        ...
        write_skill_md(
            &cwd.path().join(".chutes-build").join("skills").join("dup"),
            "dup",
        );
    ...
    fn find_skill_paths_no_skills_dir() {
        // .chutes-build exists but no skills/ subdirectory
        let tmp = tempfile::tempdir().unwrap();
        let grok_dir = tmp.path().join(".chutes-build");
    ...
    async fn list_skills_workspace_user_dir_with_recursive_skills() {
        ...
        let skills_base = user_dir.join(".chutes-build").join("skills");
    ...
    async fn list_skills_ignore_allows_lower_priority_same_name_fallback() {
        ...
        write_skill_md(
            &cwd.join(".chutes-build").join("skills").join("same"),
            "same",
        );
        let repo_skill_dir = repo_root.join(".chutes-build").join("skills").join("same");
    ...
    fn collect_skill_config_dirs_gates_vendor_dirs() {
        ...
        for name in [".chutes-build", ".agents", ".claude", ".cursor"] {
        ...
        assert!(
            ends_with(&dirs, ".chutes-build"),
            "grok must remain: {dirs:?}"
        );
    }
    ...
    fn dedupe_same_scope_cross_harness_loser_resurfaces() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "review",
                "/u/.chutes-build/skills/review/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "review",
    ...
    fn dedupe_frontmatter_owner_evicts_rekeyed_squatter() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.chutes-build/skills/japandi/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi",
                "/u/.chutes-build/skills/japandi2/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi2",
    ...
    fn dedupe_cross_scope_shadowing_unchanged() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/repo/.chutes-build/skills/japandi/SKILL.md",
                SkillScope::Repo,
            ),
            named_skill("japandi", "/u/skills/japandi2/SKILL.md", SkillScope::User),
        ]);
    ...
    fn dedupe_same_scope_same_basename_still_drops() {
        ...
        // (e.g. ~/.chutes-build/skills and ~/.agents/skills): first-seen wins.
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.chutes-build/skills/japandi/SKILL.md",
        ...
        assert_eq!(out.len(), 1);
        assert!(out[0].path.contains(".chutes-build"));
    }
    ...
    async fn copied_skill_dir_with_stale_frontmatter_name_surfaces_both() {
        ...
        let skills_dir = repo_root.join(".chutes-build").join("skills");
