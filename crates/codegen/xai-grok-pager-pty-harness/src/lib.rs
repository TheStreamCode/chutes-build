...
//! - **`scroll_matrix`**  — `CHUTES_BUILD_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// (XTVERSION probes and chutes-build wrap). Content-backed launches must use
    /// [`Self::new_in_sandbox`].
