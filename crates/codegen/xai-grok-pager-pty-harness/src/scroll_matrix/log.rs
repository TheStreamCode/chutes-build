//! `CHUTES_BUILD_SCROLL_LOG` JSONL parsing, per-stream grouping, and finalize
//! synchronization.
//!
...
/// Parse a `CHUTES_BUILD_SCROLL_LOG` JSONL file into records.
///
...
/// session (`CHUTES_BUILD_SCROLL_LOG` set at spawn): `stream_start` → `flush`* →
