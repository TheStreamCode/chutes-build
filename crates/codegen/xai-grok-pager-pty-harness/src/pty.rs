...
// `process_has_exited_without_reap` is `#[cfg(unix)]` at its definition and
// every call site here is gated the same way, but the import was not — which
...
impl PtyController {
    ...
    fn spawn_inner(
        ...
        let process_tree =
            process_pid.map(|pid| TestProcessTree::attach(pid, "chutes-build PTY child"));
...
mod tests {
    ...
    fn apply_child_env_strips_all_host_terminal_markers() {
        ...
        assert_eq!(
            cmd.get_env("CHUTES_BUILD_HOME").and_then(|v| v.to_str()),
    ...
    fn apply_child_env_uses_sandbox_baseline() {
        ...
        assert_eq!(cmd.get_env("CHUTES_BUILD_LEADER_SOCKET"), None);
    }
    ...
    fn apply_child_env_remove_deletes_sandbox_credential() {
        ...
        apply_child_env(&mut cmd, Some(&sandbox), &[EnvOp::remove("CHUTES_API_KEY")]);

        assert_eq!(cmd.get_env("CHUTES_API_KEY"), None);
        assert_eq!(
            cmd.get_env("CHUTES_BUILD_XAI_API_BASE_URL")
                .and_then(|v| v.to_str()),
    ...
    fn apply_child_env_caller_env_overrides_survive_strips() {
        ...
        apply_child_env(
            ...
            &[
                ...
                EnvOp::set("CHUTES_BUILD_OSC52_SINK", "1"),
            ],
        );
