...
//! as the active auth (`CHUTES_API_KEY` removed) and the rollout forced on via
//! `CHUTES_BUILD_PRIVACY_NOTICE_ROLLOUT=1`.
//!
//! ```bash
...
const ROWS: u16 = 50;
const COLS: u16 = 120;
const BANNER_TITLE: &str = "Help improve Chutes Build";
...
/// sandbox's fake `CHUTES_API_KEY` removed so the seeded opted-out OAuth entry
/// is the active auth — the banner's two preconditions.
fn banner_env_ops() -> [EnvOp<'static>; 2] {
    [
        EnvOp::set("CHUTES_BUILD_PRIVACY_NOTICE_ROLLOUT", "1"),
        EnvOp::remove("CHUTES_API_KEY"),
    ]
}
...
/// Poll `<home>/.chutes-build/config.toml` for the async `privacy_banner_acked`
...
fn wait_for_ack_on_disk(pager: &mut PtyHarness, home: &Path, timeout: Duration) -> Result<()> {
    let path = home.join(".chutes-build").join("config.toml");
