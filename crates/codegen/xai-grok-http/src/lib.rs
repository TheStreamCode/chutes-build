...
//! The sampler reads `CHUTES_BUILD_POOL_*` and `CHUTES_BUILD_CONNECT_TIMEOUT_SECS` once, when its shared client is first built.
//! `CHUTES_BUILD_SAMPLER_SHARED_CLIENT=0` falls back to a fresh client per `SamplingClient`.
//!
...
/// Construct an [`OriginClientInfo`] from the `CHUTES_BUILD_CLIENT_NAME` and `CHUTES_BUILD_CLIENT_VERSION` env vars.
/// Returns `None` when `CHUTES_BUILD_CLIENT_NAME` is unset.
...
/// Header telling cli-chat-proxy whether this process is a single-prompt (`chutes-build -p`) run or an interactive session.
/// The value feeds the `client_mode` metric label.
pub const CLIENT_MODE_HEADER: &str = "x-grok-client-mode";

/// Set to `"headless"` at startup by the non-TUI entry points (`run_single_turn` for `chutes-build -p`, `run_headless_inner` for `chutes-build agent [headless]`).
