...
//! `.chutes-build-plugin/plugin-index.json` (preferred), then
...
mod tests {
    ...
    fn load_catalog_parses_grok_plugin_dir() {
        ...
        write_catalog(dir.path(), ".chutes-build-plugin", BASIC);
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".chutes-build-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
