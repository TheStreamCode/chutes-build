...
//!
//! Catalog file lookup, in order: `.chutes-build-plugin/marketplace.json` (preferred),
//! `.chutes-build-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.chutes-build-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.chutes-build-plugin/plugin.json`
...
pub fn load_index(marketplace_root: &Path) -> Result<Option<MarketplaceIndex>, String> {
    let grok_dir = marketplace_root.join(".chutes-build-plugin");
