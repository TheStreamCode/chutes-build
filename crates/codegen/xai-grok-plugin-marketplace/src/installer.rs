...
mod tests {
    ...
    fn with_test_registry<T>(f: impl FnOnce(&mut InstallRegistry) -> T) -> T {
        ...
        // test in this binary can cache the real `~/.chutes-build` before this runs,
    ...
    fn transactional_update_save_failure_rolls_back_install_and_disk_registry() {
        with_test_registry(|registry| {
            ...
            unsafe {
                std::env::set_var(
                    "XAI_CHUTES_BUILD_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE",
                    "1",
                )
            };
            let result = update_from_marketplace_entry_transactional(
            ...
            unsafe {
                std::env::remove_var("XAI_CHUTES_BUILD_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE")
            };
