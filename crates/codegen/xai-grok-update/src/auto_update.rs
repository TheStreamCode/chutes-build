...
fn manual_install_cmd() -> &'static str {
    if cfg!(windows) {
        "irm https://chutes.ai/cli/install.ps1 | iex"
    } else {
        "curl -fsSL https://chutes.ai/cli/install.sh | bash"
    }
}
...
fn reinstall_hint(installer: &str) -> String {
  ...
  gh release download --repo {} \n             --pattern 'chutes-build-*' --output chutes-build && chmod +x chutes-build",
...
mod tests {
    ...
    fn test_user_facing_constants_are_stable() {
        ...
        // There is no `update` subcommand to run: self-update is off
        // (`product::AUTOMATIC_SELF_UPDATE`), so the message points at the
        // README instead of naming a command that does nothing.
        assert_eq!(
            MSG_RUN_UPDATE_MANUAL,
            "See README.md for supported update instructions."
        );
    }
