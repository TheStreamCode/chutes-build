...
fn manual_install_cmd(channel: &str) -> String {
    ...
    if channel == "enterprise" {
        ...
        return if cfg!(windows) {
            "irm https://chutes.ai/cli/enterprise-install.ps1 | iex".to_string()
        } else {
            "curl -fsSL https://chutes.ai/cli/enterprise-install.sh | bash".to_string()
        };
    }
    if is_stable_channel(channel) || !safe {
        return if cfg!(windows) {
            "irm https://chutes.ai/cli/install.ps1 | iex".to_string()
        } else {
            "curl -fsSL https://chutes.ai/cli/install.sh | bash".to_string()
        };
    }
    if cfg!(windows) {
        format!(
            "$env:CHUTES_BUILD_CHANNEL='{channel}'; irm https://chutes.ai/cli/install.ps1 | iex"
        )
    } else {
        format!(
            "curl -fsSL https://chutes.ai/cli/install.sh | CHUTES_BUILD_CHANNEL='{channel}' bash"
        )
    }
}
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of Chutes Build is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of Chutes Build is available.");
        }
        return Ok(());
    }
...
async fn run_update_subcommand(
    ...
    if let Some(mode) = xai_grok_telemetry::client::current_mode() {
        cmd.env("CHUTES_BUILD_TELEMETRY_ENABLED", mode.to_string());
    }
    match run_mode {
...
pub fn restart_grok() -> Result<()> {
    ...
    eprintln!("Restarting Chutes Build...");
    ...
    rename_result.map_err(|e| {
        anyhow::anyhow!(
            "cannot rename locked executable {}: {e}\n\
             Close all running chutes-build sessions and retry.",
            dest.display(),
        )
    })?;
    match tokio::fs::copy(src, dest).await {
...
fn warn_if_other_grok_processes_running() {
    ...
    if let Ok(output) = cmd.output() {
        ...
        if !other_pids.is_empty() {
            ...
            eprintln!("    Consider closing other chutes-build sessions before updating.");
            eprintln!();
        }
    }
}
...
pub async fn run_update(
    ...
    if let Some(version) = pinned_version {
        ...
        eprintln!(
            "Installing Chutes Build {} (current: {})...",
    ...
    let (latest_version, install_target) = match plan {
        UpdatePlan::Skip { latest } => {
            // Cache so an explicit `chutes-build update` doesn't re-prompt every run.
            let stable_ptr = try_fetch_stable_pointer().await;
    ...
    {
        eprintln!(
            "Forcing reinstall of Chutes Build {} (already up to date)",
            effective_current
        );
        &effective_current
    } else {
