...
fn test_reinstall_hint_internal_mentions_platform_installer() {
    ...
    if cfg!(windows) {
        ...
        assert!(
            !hint.contains("CHUTES_BUILD_CHANNEL"),
...
fn test_reinstall_hint_internal_alpha_sets_channel() {
    ...
    if cfg!(windows) {
        assert!(
            hint.contains("$env:CHUTES_BUILD_CHANNEL='alpha'"),
            "alpha should set CHUTES_BUILD_CHANNEL: {hint}"
        );
    } else {
        assert!(
            hint.contains("| CHUTES_BUILD_CHANNEL='alpha' bash"),
            "alpha must set CHUTES_BUILD_CHANNEL on bash (the process running \
...
fn test_reinstall_hint_enterprise_uses_enterprise_script() {
    ...
    // there), never install.sh + CHUTES_BUILD_CHANNEL.
