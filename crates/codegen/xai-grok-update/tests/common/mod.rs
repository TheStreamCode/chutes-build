...
//! its own `OnceLock<CHUTES_BUILD_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the CHUTES_BUILD_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes CHUTES_BUILD_HOME once per binary
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            ...
            std::env::remove_var("CHUTES_BUILD_INSTALLER");
            std::env::remove_var("CHUTES_BUILD_MANAGED_BY_NPM");
            std::env::remove_var("CHUTES_BUILD_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `CHUTES_BUILD_HOME` between tests so each test sees a clean home.
...
/// Backdate every file in `CHUTES_BUILD_HOME/downloads` by ~2 hours.
///
